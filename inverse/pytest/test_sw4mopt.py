...
#            run_cmd = sw4_mpi_run + ' ' + sw4_input_file + ' >& ' + sw4_stdout_file
            ...
            if result_file == 'energy.log':
                ...
                success = compare_one_line(ref_result , 'run1' + sep + result_file, 1e-5, 1e-10, -1, verbose)
                if success and 'attenuation' in test_dir:
                    # also compare the 3rd last line in the files
                    success = compare_one_line(ref_result , 'run1' + sep + result_file, 1e-5, 1e-10, -3, verbose)
            if success:        
