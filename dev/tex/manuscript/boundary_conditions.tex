...
To conserve space, we only describe the technique for imposing boudnary conditions with ghost points. It is also possible to impose the boundary conditions without ghost points, say SAT penalty method, see \cite{wang2018fourth}, but we only focus on the scheme with ghost points for the boundary conditions in this paper. We only impose the boundary conditions on the top/bottom surface (direction $3$) since other boundaries are periodic; besides, we only describe the top boundary here and omit the bottom boundary. In the rest of the paper, we define a discrete scalar product as
