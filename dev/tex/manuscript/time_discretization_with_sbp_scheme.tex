...
Let ${\bf c}^{n}$ and ${\bf f}^{n}$ denote the numerical approximations of ${\bf C}({\bf x},t_n), {\bf x}\in\Omega^c$ and ${\bf F}({\bf x},t_n), {\bf x}\in\Omega^f$ respectively. Here, $t_n = n\Delta t, n = 0,1,\cdots$ and $\Delta_t > 0$ is a constant time step. We present the fourth order time integrator with predictor and corrector in the Algorithm \ref{first_alg}.
...
\begin{breakablealgorithm}
	...
	Given initial conditions $\wt{{\bf c}}^{0}, \wt{{\bf c}}^{-1}$ and ${\bf f}^{0}, {\bf f}^{-1}$ that satisfies the discretized interface conditions.
	
	\begin{itemize}
		\item  {Compute the predictor at the interior grid points for both fine and coarse domains
			\begin{equation*}
			{\bf c}^{*,n+1} = 2{\bf c}^{n} - {\bf c}^{n-1} + \Delta_t^2(\varrho^{2h})^{-1}{\wt{L}}^{2h} \wt{\bf{c}}^{n},\ \ \ 
			{\bf f}^{*,n+1} = 2{\bf f}^{n} - {\bf f}^{n-1} + \Delta_t^2(\varrho^h)^{-1}\hat{L}^{h} {\bf{f}}^{n}
			\end{equation*}
		}
		...
		\item{For the continuity of traction on the interface $\Gamma$, assign the ghost points value in $\wt{\bf c}^{*,n+1}$ to satisfy
			...
			\left|\nabla^c r^{3}\right|^{-1}({\mathcal J}_{\Gamma}^{2h})^{-1}\wt{\mathcal{A}}_3^{2h}\wt{\bf c}^{*,n+1}\big|_\Gamma
			= \wt{\mathcal{R}}\left(|\nabla^f r^{3}|^{-1}({\mathcal J}^h_{\Gamma})^{-1}(\mathcal{A}_3^h{\bf f}^{*,n+1}\big|_\Gamma-h\omega_1{\bm \eta}^{\ast,n+1})\right)
			\end{equation}
		...
		\item{Compute the corrector at the interior grid points
			\begin{equation*}
			{\bf c}^{n+1} = {\bf c}^{*,n+1} + \frac{\Delta_t^4}{12}(\varrho^{2h})^{-1}\wt{L}^{2h}\wt{\bf{c}}^{n},\ \ \ 
			{\bf f}^{n+1} = {\bf f}^{*,n+1} + \frac{\Delta_t^4}{12}(\varrho^{h})^{-1}L^{h}{\bf{f}}^{n}
			\end{equation*}
		}
		...
		\item{For the continuity of traction on the interface $\Gamma$, assign the ghost point value in $\wt{\bf c}^{n+1}$ to satisfy
			...
			\left|\nabla^c r^{3}\right|^{-1}({\mathcal J}_{\Gamma}^{2h})^{-1}\wt{\mathcal{A}}_3^{2h}\wt{\bf c}^{n+1}\big|_\Gamma
			= \wt{\mathcal{R}}\left(|\nabla^f r^{3}|^{-1}({\mathcal J}^h_{\Gamma})^{-1}(\mathcal{A}_3^h{\bf f}^{n+1}\big|_\Gamma-h\omega_1{\bm \eta}^{n+1})\right)
			\end{equation}
			with the definition of $\wt{\mathcal{A}}_3^{2h}$ in (\ref{hatAc}), $\mathcal{A}_3^h$ in (\ref{hatAf})
		}
	\end{itemize}
\end{breakablealgorithm}
Here, we only give the steps to evolve the elatic wave equation with suitable initial and interface conditions and skip the detailed derivations of the fourth order predictor corrector time integator. One can refer to \cite{wang2018fourth} to get more details.
