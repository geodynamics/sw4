...
\end{equation}
As for the fine domian $\Omega_f$, it is chosen to be
...
\end{equation}
where, $n^{(i)}$ is the element of the unit outward normal ${\bf n} = (n^{(1)},n^{(2)},n^{(3)})$ for the top surface and $M_{ij}$ are given in (\ref{Mmatrices}). 