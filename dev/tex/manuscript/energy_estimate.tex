...
({\bf v}, {\bf u})_{2h} = 8h_1h_2h_3\sum_{i=1}^{n_1^{2h}}\sum_{j=1}^{n_2^{2h}}\sum_{k=1}^{n_3^{2h}}\omega_k{J}^{2h}_{ll}({\bf v}_{i,j,k}\cdot {\bf u}_{i,j,k})
\end{equation}
with $l = (k-1)n_1^{2h}n_2^{2h}+(j-1)n_1^{2h}+i$. Similarly, define the three dimensional discrete scalar product in $\Omega^f$ as
\begin{equation}\label{scalar_product_inner_f}
({\bf v}, {\bf u})_{h} = h_1h_2h_3\sum_{i=1}^{n_1^{h}}\sum_{j=1}^{n_2^{h}}\sum_{k=1}^{n_3^{h}}\omega_k{J}^{h}_{ll}({\bf v}_{i,j,k}\cdot {\bf u}_{i,j,k}),
\end{equation}
where $l = (k-1)n_1^{h}n_2^{h}+(j-1)n_1^{h}+i$, $\bf u$ and $\bf v$ are grid functions in the fine domain $\Omega^f$. Now, we are ready to state the energy estimate of the proposed schemes in Section \ref{semi_discrete_form}. 
...
\begin{proof}
	...
	&&8h_1h_2h_3\sum_{i = 1}^{n_1^{2h}}\sum_{j=1}^{n_2^{2h}}\sum_{k=1}^{n_3^{2h}}\omega_k{\bf c}_t\cdot\left((\rho^{2h}\otimes{\bf I})(J^{2h}\otimes{\bf I})\frac{d^2{\bf c}}{dt^2}\right) \\
	&=& 8h_1h_2h_3\sum_{i = 1}^{n_1^{2h}}\sum_{j=1}^{n_2^{2h}}\sum_{k=1}^{n_3^{2h}}\omega_kJ^{2h}_{ll}{\bf c}_t\cdot\left((\rho^{2h}\otimes{\bf I})\frac{d^2{\bf c}}{dt^2}\right), \quad  l = (k-1)n_1^{2h}n_2^{2h}+(j-1)n_1^{2h}+i \nonumber\\
	&=& ({\bf c}_t, (\rho^{2h}\otimes {\bf I}){\bf c}_{tt})_{2h} = ({\bf c}_t,(J^{2h}\otimes {\bf I})^{-1}\wt{\mathcal{L}}^{2h}{\bf c})_{2h} = -\mathcal{S}_{2h}({\bf c}_t,{\bf c}) + B_{2h}({\bf c}_t,{{\bf c}}),\nonumber
	\end{eqnarray}
	...
	\end{equation}
	Forming the inner product between (\ref{fine_scheme}) and $h_1h_2h_3\omega_k{\bf f}_t$, and summing over $i,j,k$, based on \eqref{scalar_product_inner_f}, we obtain
	...
	&&h_1h_2h_3\sum_{i = 1}^{n_1^{h}}\sum_{j=1}^{n_2^{h}}\sum_{k=1}^{n_3^{h}}\omega_k{\bf f}_t\cdot\left((\rho^{h}\otimes{\bf I})(J^{h}\otimes{\bf I})\frac{d^2{\bf f}}{dt^2}\right) \\
	&=&\! h_1h_2h_3\sum_{i = 1}^{n_1^{h}}\sum_{j=1}^{n_2^{h}}\sum_{k=1}^{n_3^{h}}\omega_kJ^{h}_{ll}{\bf f}_t\cdot\left((\rho^{h}\otimes{\bf I})\frac{d^2{\bf f}}{dt^2}\right), \quad  l = (k-1)n_1^{h}n_2^{h}+(j-1)n_1^{h}+i \nonumber\\
