...
\end{equation}
Here, ${\varrho}^{h}_{\Gamma}$ and ${\mathcal{ J}}^{h}_{\Gamma}$ are $3n_1^hn_2^h\times 3n_1^hn_2^h$ diagonal matrices with similar definitions as in (\ref{rho_j}) with fine grids on $\Gamma$. $\big(({\varrho}^{2h})^{-1}\wt{L}^{2h} \wt{{\bf c}}\big)\big|_{\Gamma}$ is a colume vector of size $3n_1^{2h} n_2^{2h}$ and its value corresponds to the coarse grids on $\Gamma$. In addition, the terms $\mathcal{G}_l^h({N}_{ll}^h), l = 1,2$ and $\mathcal{D}_l^h(\mathcal{N}_{lm}^h\mathcal{D}_m^h), l=1,2,3,m=1,2,3,l\neq m$ in (\ref{elastic_semi_f})--(\ref{elastic_semi_f_i}) have similar definitions as in (\ref{g1122})--(\ref{N12}). But $\mathcal{G}_3^h({N}_{33}^h)$ is defined by
...
Here, both $\mathcal{B}^{h}$ and $\mathcal{R}$ are $3n_1^hn_2^hn_3^h\times 3n_1^hn_2^hn_3^h$ matrices, both $\mathcal{B}^{2h}$ and $\mathcal{R}$ are $3n_1^{2h}n_2^{2h}n_3^{2h}\times 3n_1^{2h}n_2^{2h}n_3^{2h}$ matrics, and they have the following structures
...
. And
\begin{align}
\mathcal{B}^h = \left(\begin{array}{ccc}
{\bf B}^h& ~  & ~ \\
~ & {\bf B}^h & ~ \\
~ &~  &{\bf B}^h\end{array}\right), \ \ \mathcal{B}^{2h} = \left(\begin{array}{ccc}
{\bf B}^{2h}& ~  & ~ \\
~ & {\bf B}^{2h} & ~ \\
~ &~  &{\bf B}^{2h}\end{array}\right),
\end{align}
where ${\bf B}^{2h}$ is a $n_1^{2h}n_2^{2h}n_3^{2h}\times n_1^{2h}n_2^{2h}n_3^{2h}$ diagonal matrix and its diagonal value is $|\nabla_x^c r^{(3)}|$ evalued at the coarse grid points on interface $\Gamma$; ${\bf B}^{h}$ is a $n_1^{h}n_2^{h}n_3^{h}\times n_1^{h}n_2^{h}n_3^{h}$ diagonal matrix with diagonal value to be  $|\nabla_x^f r^{(3)}|$ evalued at the fine grid points on interface $\Gamma$.
...
(\mathcal{B}^{2h}{\mathcal J}_{\Gamma}^{2h})^{-1}\big(\wt{\mathcal{A}}_3^{2h}\wt{\bf c}\big)\big|_\Gamma
= \wt{\mathcal{R}}\left((\mathcal{B}^{h}{\mathcal J}^h_{\Gamma})^{-1}((\mathcal{A}_3^h{\bf f})\big|_\Gamma-h_3\omega_1{\mathcal J}^h{\bm \eta})\right)
\end{equation}
...
where ${\bf I}$ is a $3\times3$ identity matrix, ${\bf I}_l$, $l = 1,2$ are $n_l^{2h}\times n_l^{2h}$ identity matrices, $\wt{\mathfrak{D}}_3^{2h}$ is a $n_3^{2h}\times (n_3^{2h}+2)$ matrix defined in (\ref{sbp_1st_1}) for direction $r^{(3)}$ which is a sparse matirx with only the last row has $5$ non-zeros and the rest of elements are zeros. As for $\mathscr{D}_3^{h}$, it is defined as
\[\mathscr{D}_3^{h} = {\bf I}\otimes {\bf I}_1 \otimes {\bf I}_2 \otimes \wt{\mathfrak{D}}_3^{2h}\]
where ${\bf I}$ is a $3\times3$ identity matrix, ${\bf I}_l$, $l = 1,2$ are $n_l^{h}\times n_l^{h}$ identity matrices, $\mathfrak{D}_3^{h}$ is a $n_3^{h}\times n_3^{h}$ matrix defined in (\ref{sbp_1st_2}) for direction $r^{(3)}$. And $\mathfrak{D}_3^{h}$ is a sparse matirx with only the first row has $4$ non-zeros and the rest of elements are zeros. 
