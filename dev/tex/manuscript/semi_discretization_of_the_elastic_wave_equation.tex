...
where ${\bf u}$ can be either a scalar or vector. {\color{red}The essential idea of the spatial discretization is that we only use ghost points in the coarse domain and no ghost points in the fine domain. For the rest of the paper, the $\sim$ over an operator represents that the ghost points are used. Specifically,} we approximate the elastic wave equation (\ref{elastic_curvi}) in $\Omega^c$ by
...
\end{equation}
{\color{red}Note that we have an extra term $h_3\omega_1{\bm \eta}_{\bf i}$ in (\ref{continuous_traction}). Because it is on the order of the truncation error it does not affect the order of accuracy. As a consequence, (\ref{continuous_traction}) provides a valid way of enforcing the continuity of traction force at the interface. The Theorem \ref{thm1} illustrates why the ${\bm \eta}_{\bf i}$-term is important for energy stability. Now let's work on the terms in (\ref{continuous_traction})}. $\omega_1$ is the first entry in the scalar product (\ref{inner_product}), $\Lambda_{{\bf i}'}^c$ and $\Lambda_{\bf i}^f$ store the values of functions $|\nabla_x R^{c,(3)}|$ and $|\nabla_x R^{f,(3)}|$ evaluated at the grid points in $I_{\Gamma^c}$ and $I_{\Gamma^f}$, respectively. And we have used the notation
