...
then group different sets of grid points according to
\begin{equation*}
\begin{aligned}
...
where ${\bf u}$ can be either a scalar or vector. To distinguish between the continuous variables and the correspoding approximations on the grid, we use ${\bf c}_{\bf i}$ and ${\bf f}_{\bf i}$ to denote the grid functions for the approximations of ${\bf C}({\bf x}_{\bf i})$ and ${\bf F}({\bf x}_{\bf i})$, respectively. The components of the vectors $\bf c$ and $\bf f$ follows the rules:\\
	%\begin{description}
		...
		b). the grid points are ordered such that they first loop over $r^{(1)}$ direction ($i$), then $r^{(2)}$ direction ($j$), and finally $r^{(3)}$ direction ($k$). More precisely, the vector 
		${\bf c} = [c^{(1)}_{1,1,1}, c^{(2)}_{1,1,1}, c^{(3)}_{1,1,1},c^{(1)}_{2,1,1}, c^{(2)}_{2,1,1}, c^{(3)}_{2,1,1},\cdots]^T$, and the vector ${\bf f} = [f^{(1)}_{1,1,1}, f^{(2)}_{1,1,1}, f^{(3)}_{1,1,1},f^{(1)}_{2,1,1}, f^{(2)}_{2,1,1}, f^{(3)}_{2,1,1},\cdots]^T$.
	%\end{description}
...
\end{equation}
which does not use any ghost points. The term ${G}_3^{h}(N_{33}^{h}){\bf f}$ approximates $\bar{\partial}_3(N_{33}\bar{\partial}_3 {\bf F})$ is presented in Appendix \ref{appendix_cdomain} without using any ghost points; the terms $Q_l^{h}(N_{ll}^{h}){\bf f}$ and $D_{l}^{h}(N_{lm}^{2h}D_m^{h}{\bf f})$ approximate $\bar{\partial}_l(N_{ll}\bar{\partial}_l{\bf F})$ and $\bar{\partial}_l(N_{lm}\bar{\partial}_m {\bf F})$, respectively. Here, ${Q}_l^{h}({N}_{ll}^h){\bf f}$ and ${D}_l^{h}({N}_{lm}^{h}{D}_m^{h}{\bf f})$ are defined similar as those in (\ref{L_operator}), but with the grid points in $I_{\Omega^f}\backslash I_{{\Gamma^f}}$.
...
Here, $J_{\Gamma}^h$ and $\Lambda^h$ are $n_1^{h}n_2^{h}\times n_1^{h}n_2^{h}$ diagonal matrices, the diagonal elements of $J_{\Gamma}^h$ and $\Lambda^{h}$ are $J^f$ and $|\nabla_x R^{f,(3)}|$ evaluated at fine grid points in $I_{\Gamma^f}$, respectively; ${J}_{\Gamma}^{2h}$ and ${{\Lambda}^{2h}}$ are $n_1^{2h}n_2^{2h}\times n_1^{2h}n_2^{2h}$ diagonal matrices, the diagonal elements of $J_{\Gamma}^{2h}$ and $\Lambda^{2h}$ are $J^c$ and $|\nabla_x R^{c,(3)}|$ evaluated at coarse grid points in $I_{\Gamma^c}$, respectively; ${\bf I}$ is $3\times 3$ identity matrix. ${\bf P}$ is an interpolation operator of size $n_1^hn_2^h\times n_1^{2h}n_2^{2h}$ for scalar grid functions. Since the mesh refinement ratio is $1:2$, the stencils of the fourth order accurate interpolation operator ${\bf P}$ in two dimensions have four cases, see the illustration in  Figure \ref{interpolation}.
...
\end{equation}
where ${N}_{3l}^hD_l^h{\bf f}$ approximate $N_{3l}\bar{\partial}_l{\bf F}$ and ${N}_{33}^h\mathcal{D}_3^h{\bf f}$ approximates $N_{33}\bar{\partial}_3{\bf F}$ are given in Appendix \ref{appendix_cdomain}; and the other notation 
...
\end{equation}
Here, ${N}_{3l}^{2h}D_l^{2h}{\bf c}$ approximate $N_{3l}\bar{\partial}_l{\bf C}$ and they have similar definitions as those in (\ref{hatAf}), but correspond to the grid points in $I_{\Gamma^c}$; ${N}_{33}^{2h}\wt{\mathcal{D}}^{2h}_3{\bf c}$ approximates $N_{33}\bar{\partial}_3{\bf C}$ is given in Appendix \ref{appendix_cdomain}. The scaled restriction operator ${\mathcal{R}} $ in \eqref{continuous_traction} has the structure 
...
\left<{\bf u}, {\bf v}\right>_{2h} = 4h_1h_2\sum_{i=1}^{n_1^{2h}}\sum_{j=1}^{n_2^{2h}}{  J}_{i,j,n_3^{2h}}^{c}\Lambda^{c}_{i,j,n_3^{2h}}({\bf u}_{i,j,n_3^{2h}}^T\cdot {\bf v}_{i,j,n_3^{2h}}).
\end{equation}
...
\left<{\bf u}, {\bf v}\right>_{h} = h_1h_2\sum_{i=1}^{n_1^{h}}\sum_{j=1}^{n_2^{h}}{  J}_{i,j,1}^{f}\Lambda^{f}_{i,j,1}({\bf u}_{i,j,1}^T\cdot {\bf v}_{i,j,1})
\end{equation}
