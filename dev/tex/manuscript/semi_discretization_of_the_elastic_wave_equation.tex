...
where ${\bf u}$ can be either a scalar or vector. To distinguish between the continuous variables and the corresponding approximations on the grid, we use ${\bf c}_{\bf i}$ and ${\bf f}_{\bf i}$ to denote the grid functions for the approximations of ${\bf C}({\bf x}_{\bf i})$ and ${\bf F}({\bf x}_{\bf i})$, respectively. {\color{red} Let {\bf c} and {\bf f} be the vector representations of the grid functions ${\bf c_i}$ and ${\bf f_i}$ respectively.} The elements of $\bf c$ and $\bf f$ are ordered in the following way:\\
	%\begin{description}
		...
		${\bf c} = [c^{(1)}_{1,1,1}, c^{(2)}_{1,1,1}, c^{(3)}_{1,1,1},c^{(1)}_{2,1,1}, c^{(2)}_{2,1,1}, c^{(3)}_{2,1,1},\cdots]^T$ and ${\bf f} = [f^{(1)}_{1,1,1}, f^{(2)}_{1,1,1}, f^{(3)}_{1,1,1},f^{(1)}_{2,1,1}, f^{(2)}_{2,1,1}, f^{(3)}_{2,1,1},\cdots]^T$. {\color{red} We want to point out that $\bf c$ contains the ghost point values for $k = n_3^{2h}+1$, but ${\bf f}$ does not contain any ghost point values.}
	%\end{description}
...
\end{equation}
Here,  $(\Lambda^{c}_{{\bf i}'}J_{{\bf i}'}^{c})^{-1}\wt{\mathcal{A}}_3^{2h}{\bf c}_{{\bf i}'}$ and $(\Lambda^f_{\bf i}{ J}^f_{\bf i})^{-1}\mathcal{A}_3^h{\bf f}_{\bf i}$ are approximations of the traction at the interface on the coarse grid and fine grid, respectively. More precisely, $\wt{\mathcal{A}}_3^{2h}{\bf c}$ is a sum of three terms  $\wt{\mathcal{A}}_3^{2h}{\bf c}= {N}_{31}^{2h}{D}^{2h}_1{\bf c} + {N}_{32}^{2h}{D}^{2h}_2{\bf c} + {N}_{33}^{2h}\wt{\mathcal{D}}^{2h}_3{\bf c}$ given in Appendix \ref{appendix_cdomain}. The traction approximation $(\Lambda^f_{\bf i}{ J}^f_{\bf i})^{-1}\mathcal{A}_3^h{\bf f}_{\bf i}$ is defined analogously: ${\mathcal{A}}_3^{h}{\bf f}= {N}_{31}^{h}{D}^{h}_1{\bf f} + {N}_{32}^{h}{D}^{h}_2{\bf f} + {N}_{33}^{h}{\mathcal{D}}^{h}_3{\bf f}$ is given in Appendix \ref{appendix_cdomain}.   The operator $\mathcal{R}$ is a scaled restriction operator with the structure 
