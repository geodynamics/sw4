module problemsetup_new_3d
  ...
  real(dp),parameter :: l1 = 2000.d0, l2 = 2000.d0, l3 = 1000.d0 ! space interval

  integer,parameter :: n1_c = 201, n2_c = 201, n3_c = 101 ! number of grid points in direction-1 in the coarse domain
  ...
  real(dp),parameter :: amp = 0.d0, peak = 0.04d0

contains

  subroutine generate_grid(X1_c,X2_c,X3_c)
    integer i,j,k
  ...
  subroutine metric_derivative(r1,r2,r3,xi13,xi23,xi33,J)
  ...
  ! exact solution outside the doamin and the boundary of domain except the top bounday
  subroutine exact_solution(x1,x2,x3,t,u1,u2,u3,flag)
    implicit none
    real(dp) :: x1, x2, x3, t, u1, u2, u3
    integer flag
    u1 = 0.d0!cos(x1+0.3d0)*sin(x2+0.3d0)*sin(x3+0.2d0)*cos(t*t)
    u2 = 0.d0!sin(x1+0.3d0)*cos(x2+0.3d0)*sin(x3+0.2d0)*cos(t*t)
    u3 = 0.d0!sin(x1+0.2d0)*sin(x2+0.2d0)*cos(x3+0.2d0)*sin(t)
  ...
  subroutine forcing_tt(x1,x2,x3,t,f1tt,f2tt,f3tt)
    ...
    ! set the second time derivative of forcing function to be 0
    f1tt = 0.d0
    f2tt = 0.d0
    f3tt = 0.d0
  ...
  subroutine top_normal_data(x1,x2,x3,l1,l2,t,g)
    ...
    ! Gaussian style
    g(1) = 0.d0!exp(-((t-4.d0/44.2d0)/(1.d0/44.2d0))**2)&
       !*exp(-((x1-l1/2.d0)/(2.5d0*h1phy_c))**2)*exp(-((x2-l2/2.d0)/(2.5d0*h2phy_c))**2)
    !
    g(2) = 0.d0!exp(-((t-4.d0/44.2d0)/(1.d0/44.2d0))**2)&
       !*exp(-((x1-l1/2.d0)/(2.5d0*h1phy_c))**2)*exp(-((x2-l2/2.d0)/(2.5d0*h2phy_c))**2)
    !
    g(3) = 1e9*exp(-((t-4.d0/44.2d0)/(1.d0/44.2d0))**2)&
       *exp(-((x1-l1/2.d0)/(2.5d0*0.5d0*h1phy_c))**2)*exp(-((x2-l2/2.d0)/(2.5d0*0.5d0*h2phy_c))**2)
    !g(3) = 1e9*exp(-((t-10.d0/44.2d0)/(1.d0/44.2d0))**2)&
      !*exp(-((x1-l1/2.d0)/(2.5d0*h1phy_c))**2)*exp(-((x2-l2/2.d0)/(2.5d0*h2phy_c))**2)
