program energy_test_cartesian
  ...
  real(dp), dimension (1-nrg:n1_c+nrg,1-nrg:n2_c+nrg,1-nrg:n3_c+nrg,1:dim) :: vn
  ...
  ! estimate time step
  call kappa(tv)
  cfl=sqrt(3.d0/1.40/tv)*0.5d0
  dt0 = min(h1_c,h2_c,h3_c)*cfl*0.9d0
  nt = ceiling(tn/dt0)+1
  dt = tn/nt
  ...
  do time_index = 1, nt
	 ...
	 call discrete_energy(u_c(:,:,:,:,1),u_c(:,:,:,:,2),u_c(:,:,:,:,3),u_c(:,:,:,:,4),dt,energy_discrete(time_index))
...
contains
  ...
  ! compute the discrete energy
  subroutine discrete_energy(uc_oldold,uc_old,uc_star,uc_new,dt,energy_num)
