program energy_test_cartesian
  ...
  rate = real(cr)
  tv = 0.d0
  ! make sure the initial condition satisfy the homogeneous boundary conditions
  call Update_gp(1)
  ...
  do time_index = 1, nt
     ...
     call discrete_energy(u_c(:,:,:,:,1),u_c(:,:,:,:,2),u_c(:,:,:,:,3),dt,energy_discrete(time_index))
...
contains
  ...
  subroutine discrete_energy(uc_oldold,uc_old,uc_new,dt,energy_num)
    real(dp), dimension(1-nrg:n1_c+nrg,1-nrg:n2_c+nrg,1-nrg:n3_c+nrg,1:dim) :: uc_old,uc_new, uc_oldold
    ...
    Lh_old = (uc_new(1:n1_c,1:n2_c,1:n3_c,:) - 2.d0*uc_old(1:n1_c,1:n2_c,1:n3_c,:) &
