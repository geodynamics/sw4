module problemsetup_new_3d
  ...
  real(dp),parameter :: int_pos = 0.8d0 ! This is the position in r3 where the interface is located.

  integer,parameter :: n1_c = 101, n2_c = 101 ! number of grid points in direction-1,2 in the coarse domain
...
contains
  ...
  real(dp) function interface_cfx(x,y)
    real(dp) x,y
    interface_cfx = ampi*4.d0*pi*cos(4.d0*pi*x)
  end function interface_cfx

  real(dp) function interface_cfy(x,y)
    real(dp) x,y
    interface_cfy = -ampi*4.d0*pi*sin(4.d0*pi*y)
  ...
  subroutine top_normal_data(x1,x2,x3,l1,l2,t,g)
    ...
    g(3) = 1e9*exp(-((t-4.d0/44.2d0)/(1.d0/44.2d0))**2)&
      *exp(-((x1-l1/2.d0)/(2.5d0*0.5d0*h1phy_f))**2)*exp(-((x2-l2/2.d0)/(2.5d0*0.5d0*h2phy_f))**2)
    !g(3) = 1e9*exp(-((t-10.d0/44.2d0)/(1.d0/44.2d0))**2)&
      !*exp(-((x1-l1/2.d0)/(2.5d0*h1phy_f))**2)*exp(-((x2-l2/2.d0)/(2.5d0*h2phy_f))**2)
