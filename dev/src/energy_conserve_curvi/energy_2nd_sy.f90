...
contains
  ...
  subroutine Interface_system(Mass)
    ...
    do iset = 1,3
       do jj = 1,3
        ...
        Mass_p(-2:0,:,:,:) = 0.d0
        Mass_p(n1_f+1:n1_f+3,:,:,:) = 0.d0
        Mass_p(:,-2:0,:,:) = 0.d0
        Mass_p(:,n2_f+1:n2_f+3,:,:) = 0.d0
  ...
  subroutine discrete_energy(uf_oldold,uf_old,uf_new,uc_oldold,uc_old,uc_new,dt,energy_num,energy_num_bdry,difference_b)
    ...
    !do j = 1,n2_c
    !   do i = 1,n1_c
    !      Lh_old_f(2*i-1,2*j-1,1,:) = Lh_old_c(i,j,n3_c,:)
    !      Lh_old_f(2*i,2*j-1,1,:) = P(-1)*Lh_old_c(i-1,j,n3_c,:)+P(0)*Lh_old_c(i,j,n3_c,:)&
    !                               +P(1)*Lh_old_c(i+1,j,n3_c,:)+P(2)*Lh_old_c(i+2,j,n3_c,:)
    !      Lh_old_f(2*i-1,2*j,1,:) = P(-1)*Lh_old_c(i,j-1,n3_c,:)+P(0)*Lh_old_c(i,j,n3_c,:)&
    !                               +P(1)*Lh_old_c(i,j+1,n3_c,:)+P(2)*Lh_old_c(i,j+2,n3_c,:)
    !      Lh_old_f(2*i,2*j,1,:) = P(-1)*(P(-1)*Lh_old_c(i-1,j-1,n3_c,:)+P(0)*Lh_old_c(i,j-1,n3_c,:)&
    !                                       +P(1)*Lh_old_c(i+1,j-1,n3_c,:)+P(2)*Lh_old_c(i+2,j-1,n3_c,:))&
    !                                 +P(0)*(P(-1)*Lh_old_c(i-1,j,n3_c,:)+P(0)*Lh_old_c(i,j,n3_c,:)&
    !                                       +P(1)*Lh_old_c(i+1,j,n3_c,:)+P(2)*Lh_old_c(i+2,j,n3_c,:))&
    !                                 +P(1)*(P(-1)*Lh_old_c(i-1,j+1,n3_c,:)+P(0)*Lh_old_c(i,j+1,n3_c,:)&
    !                                       +P(1)*Lh_old_c(i+1,j+1,n3_c,:)+P(2)*Lh_old_c(i+2,j+1,n3_c,:))&
    !                                 +P(2)*(P(-1)*Lh_old_c(i-1,j+2,n3_c,:)+P(0)*Lh_old_c(i,j+2,n3_c,:)&
    !                                       +P(1)*Lh_old_c(i+1,j+2,n3_c,:)+P(2)*Lh_old_c(i+2,j+2,n3_c,:))
    !   end do
    !end do
         ...
         do i = 1,n1_c
           difference((k-1)*3*n1_c+(i-1)*3+iset) = boundary_c(i,k,iset)/int_cof_c(i,k)
            do j = -4,2
               do l = -4,2
                  difference((k-1)*3*n1_c+(i-1)*3+iset) = difference((k-1)*3*n1_c+(i-1)*3+iset) &
                        - Rop(j)*(Rop(l)*boundary_f(2*i+l,2*k+j,iset)/int_cof_f(2*i+l,2*k+j)) &
                        + h3_f*17.d0/48.d0*Rop(j)*(Rop(l)*eta(2*i+l,2*k+j,1,iset)/int_cof_f(2*i+l,2*k+j))
               end do
            end do
         end do
    ...
    print *, energy_num, energy_num_bdry
