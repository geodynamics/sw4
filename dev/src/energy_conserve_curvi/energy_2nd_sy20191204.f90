program energy_2nd_sy
  ...
  real(dp), dimension (1-nrg:n1_c+nrg,1-nrg:n2_c+nrg,1-nrg:n3_c+nrg,dim) :: force_c,force_tt_c,lh_c, lh_c_temp
  real(dp), dimension (1-nrg:n1_f+nrg,1-nrg:n2_f+nrg,1-nrg:n3_f+nrg,dim) :: force_f,force_tt_f,lh_f,eta,eta2,lh_f_temp
  ...
  real(dp), dimension (1-nrg:n1_f+nrg,1-nrg:n2_f+nrg,1-nrg:n3_f+nrg,1:dim,1:4) :: u_f_ej_injekt
  ...
  real(dp), dimension(1:n1_c,1:10000) :: eta_bc
  ...
  real(dp), dimension (-2:n1_f+3,-2:n2_f+3) :: Mass_f2 = 0.d0
  ...
  dt0 = min(h1_f,h2_f,h3_f)*cfl*0.9*0.5d0
  nt = ceiling(tn/dt0)+1
  dt = tn/nt
  ...
  do time_index = 1, nt
     ...
     call discrete_energy(u_f(:,:,:,:,1),u_f(:,:,:,:,2),u_f(:,:,:,:,3),&
        ...
        energy_discrete(time_index),energy_bdry(time_index),bdry(time_index),eta_bc(:,time_index))
  ...
  call print_array_to_file(1,n1_c,nt,eta_bc,'eta_bc.txt')
...
contains
    ...
    do i=1-nrg,n3_f+nrg
       do j=1-nrg,n2_f+nrg
          do k=1-nrg,n1_f+nrg
             ...
             rho_f(k,j,i,:) = 1.d0 !+ sin(Xgrid_f(k,j,i,1)+0.3d0)*sin(Xgrid_f(k,j,i,2)+0.3d0)*sin(Xgrid_f(k,j,i,3)-0.2d0)
          end do
       end do
    end do
  ...
  subroutine Interface_RHS(index)
    ...
    do iset = 1,3
      ...
      ! project
	  lh_c_temp = 0.d0 
	  lh_c_temp(2:n1_c-1,2:n2_c-1,2:n3_c,:) = lh_c(2:n1_c-1,2:n2_c-1,2:n3_c,:)
      Mass_f1 = 0.d0
      do k = -1,n2_c+1
         do i = -1,n1_c+1
            if (k .gt. -1 .and. i .gt. -1) then
                Mass_f1(2*i-1,2*k-1)=lh_c_temp(i,k,n3_c,iset)/rho_c(i,k,n3_c,1)
            end if
            if (k .ge. -1 .and. i .gt. -1) then
               do j = -1,2
                  Mass_f1(2*i-1,2*k)=Mass_f1(2*i-1,2*k)+P(j)*lh_c_temp(i,k+j,n3_c,iset)/rho_c(i,j+k,n3_c,1)
               end do
            end if
            if (k .gt. -1 .and. i .ge. -1) then
               do j = -1,2
                  Mass_f1(2*i,2*k-1)=Mass_f1(2*i,2*k-1)+P(j)*lh_c_temp(i+j,k,n3_c,iset)/rho_c(i+j,k,n3_c,1)
               end do
            end if
            do j = -1,2
               do l = -1,2
                   Mass_f1(2*i,2*k)=Mass_f1(2*i,2*k)+P(j)*(P(l)*lh_c_temp(i+l,k+j,n3_c,iset)/rho_c(i+l,k+j,n3_c,1))
                end do
             end do
           end do
       end do
	  ...
	  !  real(dp), dimension (-2:n1_f+3,-2:n2_f+3) :: Mass_f1 = 0.d0
	  Mass_f2 = 0.d0
	  Mass_f2(2:n1_f-1,2:n2_f-1) =  Mass_f1(2:n1_f-1,2:n2_f-1)
      ...
      do k = 1,n2_c
         do i = 1,n1_c
            do j = -4,2
               do l = -4,2
                  Vass((k-1)*3*n1_c+(i-1)*3+iset)=Vass((k-1)*3*n1_c+(i-1)*3+iset) &
                  -17.d0/48.d0*h3_f*Rop(j)*(Rop(l)*rho_f(2*i+l,2*k+j,1,1)*Mass_f2(2*i+l,2*k+j)*1.d0/int_cof_f(2*i+l,2*k+j))
               end do
            end do
         end do
      end do
	  ...
	  lh_f_temp = 0.d0
	  lh_f_temp(2:n1_f-1,2:n2_f-1,1,:) = lh_f(2:n1_f-1,2:n2_f-1,1,:)
      do k = 1,n2_c
         do i = 1,n1_c
            do j = -4,2
               do l = -4,2
                  Vass((k-1)*3*n1_c+(i-1)*3+iset) = Vass((k-1)*3*n1_c+(i-1)*3+iset) &
                        + Rop(j)*(Rop(l)*lh_f_temp(2*i+l,2*k+j,1,iset)/int_cof_f(2*i+l,2*k+j))
               end do
            end do
         end do
      end do
            ...
            do j = -4,2
               do l = -4,2
                  difference((k-1)*3*n1_c+(i-1)*3+iset) = difference((k-1)*3*n1_c+(i-1)*3+iset) &
                        ...
                        + h3_f*17.d0/48.d0*Rop(j)*(Rop(l)*eta(2*i+l,2*k+j,1,iset)/int_cof_f(2*i+l,2*k+j))
               end do
            end do
         end do
      end do
  ...
  subroutine discrete_energy(uf_oldold,uf_old,uf_new,uc_oldold,uc_old,uc_new,dt,energy_num,energy_num_bdry,difference_b,eta_bc1)
    ...
    real(dp), dimension(-2:n1_f+3,-2:n2_f+3,1:3) :: boundary_f, boundary_f2
    real(dp), dimension(1:n1_c) :: eta_bc1
    real(dp) :: dt,energy_num,energy_num_bdry,difference_b,energy_num_bdry1,energy_num_bdry2, energy_num1, energy_num2
	...
	Lh_old_c = 0.d0
	Lh_old_c(2:n1_c-1,2:n2_c-1,2:n3_c,:) = lh_c(2:n1_c-1,2:n2_c-1,2:n3_c,:)
	...
	Lh_old_f = 0.d0
	Lh_old_f(1:n1_f,1:n2_f,1:n3_f,:) = lh_f(1:n1_f,1:n2_f,1:n3_f,:)
	...
	energy_num2 = energy_num-energy_num1
    !
    call compute_eta(2)
    eta_bc1 = eta(1:n1_c,1,1,1)
    ...
    energy_num_bdry1 = 0.d0
    energy_num_bdry2 = 0.d0
    boundary_c = 0.d0
    boundary_f = 0.d0
    do iset = 1,3
       do j = 0,4
	  ...
	  boundary_f2(2:n1_f-1,2:n2_f-1,:) = boundary_f(2:n1_f-1,2:n2_f-1,:) 
	  eta2(2:n1_f-1,2:n2_f-1,1,:) = eta(2:n1_f-1,2:n2_f-1,1,:)
      do k = 2,n2_c-1
         do i = 2,n1_c-1
           difference((k-1)*3*n1_c+(i-1)*3+iset) = boundary_c(i,k,iset)/int_cof_c(i,k)
            do j = -4,2
               do l = -4,2
                  difference((k-1)*3*n1_c+(i-1)*3+iset) = difference((k-1)*3*n1_c+(i-1)*3+iset) &
                        - Rop(j)*(Rop(l)*boundary_f2(2*i+l,2*k+j,iset)/int_cof_f(2*i+l,2*k+j)) &
                        + h3_f*17.d0/48.d0*Rop(j)*(Rop(l)*eta2(2*i+l,2*k+j,1,iset)/int_cof_f(2*i+l,2*k+j))
				  !if abs(difference((k-1)*3*n1_c+(i-1)*3+iset))
               end do
            end do
         end do
      end do
      ...
      do l = 1,n2_c
         do i = 1,n1_c
            energy_num_bdry = energy_num_bdry + h1_c*h2_c*uc_new(i,l,n3_c,iset)*boundary_c(i,l,iset)/int_cof_c(i,l) !&
         end do
      end do

      do l = 1,n2_f
         do i = 1,n1_f
            energy_num_bdry = energy_num_bdry - (h1_f*h2_f*uf_new(i,l,1,iset)*boundary_f(i,l,iset) &
              - h3_f*17.d0/48.d0*h1_f*h2_f*uf_new(i,l,1,iset)*eta(i,l,1,iset))/int_cof_f(i,l) !&
         end do
      end do
	...
	print *, energy_num_bdry, energy_num, energy_num1, energy_num2

    difference_b = sum(difference)

  end subroutine discrete_energy

end program energy_2nd_sy
