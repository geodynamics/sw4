...
for line in lines:
    tokens = line.split()
    if not tokens or tokens[0] not in ("rec", "sac"):
        continue

    # SW4 initializes each receiver's omitted coordinates to zero.
    xyz = np.array([0.0,0.0,0.0])
    is_depth = 1
    if tokens[0] in ("rec", "sac"):
        has_sta = 0
        # Remove the "sac" or "rec" command token.
        llist = tokens[1:]
        # print(llist)
        nsew=0
