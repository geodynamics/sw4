...
void EW::consintp(Sarray& Uf, Sarray& Unextf, Sarray& Bf, Sarray& Muf,
  ...
  // given on i=1,i=Ni and at the ghost points i=-2,-1,0,Ni+1,Ni+2,Ni+3. The
  // arrays Bf,Unextf, etc are assumed to be computed with all z-ghost points
  // zero, i.e., Uf(i,Nkf+1) was zero for i=-2,-1,..,Ni+3 when Bf and Unextf
  // were evaluated from Uf (similarly for Unextc and Bc). Before this routine
  // is called, correct boundary values for Uf(-2,Nkf+1),..Uf(1,Nkf+1) (and
  // similarly at the upper i-boundary, and for Uc) must be imposed. In this way
  // the restriction and prolongation stencils can be computed without any
  // special treatment near the i-boundaries.
  ...
  while (jacerr > m_citol && it < m_cimaxiter) {
    ...
    for (int jc = jcb; jc <= jce; jc++)
      for (int ic = icb; ic <= ice; ic++) {
        ...
        a21 = nuf / Rhof(i, j, nkf) * Muf(i, j, nkf) *
        ...
        //	       {
        //	         cout << "i,j " << i << " " << j << " " << b1 << " " <<
        //b2 << " " << r1 << " " << r2 << endl; 		 cout << "   " << Uf(c,i,j,nkf+1)
        //<< " " << Uc(c,ic,jc,0) << " " <<
        //		    a21*Uf(c,i,j,nkf+1)+Unextf(c,i,j,nkf) << " " <<
        //-a22*Uc(c,ic,jc,0)+Unextc(c,ic,jc,1) << endl;
        //	       }
      ...
      for (int i = ifb; i <= ife; i++) {
               ...
               j % 2 == 1)))  // not both i and j are odd (handled above)
        {
          ...
          for (int c = 1; c <= 2; c++) {
            ...
            Uf(c, i, j, nkf + 1) =
                b1 * Rhof(i, j, nkf) / (nuf * m_ghcof[0] * Muf(i, j, nkf));
            Uf(c, i, j, nkf + 1) =
