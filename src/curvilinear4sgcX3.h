...
//__device__ inline double  acof(sw4_type i, sw4_type j, sw4_type k){ return cmem_acof[(i - 1)
//+ 6 * (j - 1) + 48 * (k - 1)];}
#endif
  ...
  //#pragma omp parallel
  {
    ...
    if (onesided[4] == 1) {
      ...
      forall3async<__LINE__>(
          tag1, I, J, K, [=] SW4_DEVICE(Tclass<1> t, sw4_type i, sw4_type j, sw4_type k) {
#else
      ...
      RAJA::kernel<CURV_POL>(
          ...
          [=] SW4_DEVICE(sw4_type k, sw4_type j, sw4_type i) {
#endif
    ...
    // forall3GS(IS,JS,KS, [=]SW4_DEVICE(sw4_type i,sw4_type j,sw4_type k){
    // Use 168 regissters , no spills
    Tclass<2> tag2;
#pragma forceinline
    forall3async<__LINE__>(
        tag2, I, J, K, [=] SW4_DEVICE(Tclass<2> t, sw4_type i, sw4_type j, sw4_type k) {
    // forall3X<256>(ifirst + 2, ilast - 1,jfirst + 2, jlast - 1,kstart, kend +
    // 1,
    //	      [=] SW4_DEVICE(sw4_type i, sw4_type j, sw4_type k) {
#else
...
#pragma forceinline
    forall3async<__LINE__>(
        tag3, I, J, K, [=] SW4_DEVICE(Tclass<3> t, sw4_type i, sw4_type j, sw4_type k) {
#else
...
#pragma forceinline
    forall3async<__LINE__>(
        tag4, I, J, K, [=] SW4_DEVICE(Tclass<4> t, sw4_type i, sw4_type j, sw4_type k) {
#else
    // RAJA::RangeSegment k_range(kstart,klast-1);
    // RAJA::RangeSegment j_range(jfirst+2,jlast-1);
    // RAJA::RangeSegment i_range(ifirst+2,ilast-1);
...
//    const sw4_type UNROLL_LEN=1;
    ...
    //     forall2async(II, JJ2,[=] SW4_DEVICE(sw4_type i, sw4_type j) {
    // #pragma unroll
...
#pragma forceinline
    forall3async<__LINE__>(
        tag5, II, JJ, KK, [=] SW4_DEVICE(Tclass<5> t, sw4_type i, sw4_type j, sw4_type k) {
...
#define ghcof(i) a_ghcof[i - 1]
  ...
  //#pragma omp parallel
  {
    ...
    if (onesided[4] == 1) {
      ...
      Range<4> J(jfirst + 2, jlast - 1);
      Range<4> K(1, 6 + 1);
      forall3async(I, J, K, [=] SW4_DEVICE(sw4_type i, sw4_type j, sw4_type k) {
#else
      RAJA::RangeSegment k_range(1, 6 + 1);
      RAJA::RangeSegment j_range(jfirst + 2, jlast - 1);
      RAJA::RangeSegment i_range(ifirst + 2, ilast - 1);
...
#pragma unroll UNROLL_LEN
    ...
    forall3GSasync(I, J, K, [=] SW4_DEVICE(sw4_type i, sw4_type j, sw4_type k) {
#else
    RAJA::RangeSegment k_range(kstart, klast - 1);
    RAJA::RangeSegment j_range(jfirst + 2, jlast - 1);
    RAJA::RangeSegment i_range(ifirst + 2, ilast - 1);
