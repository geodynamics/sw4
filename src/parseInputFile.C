...
bool EW::parseInputFile(vector<Source*>& a_GlobalUniqueSources,
  ...
  if (m_topography_exists) {
    ...
    {
      buildGaussianHillTopography(m_GaussianAmp, m_GaussianLx, m_GaussianLy,
                                  m_GaussianXc, m_GaussianYc);
  ...
  while (!inputFile.eof()) {
    ...
    if (strlen(buffer) > 0)  // empty lines produce this
    {
...
#else
        if (m_myRank == 0)
          cout << "Error: SW4 was not built with source/material optimization "
                  "support"
               << endl;
        return false;
#endif
      ...
      else if (startswith("randomize", buffer)) {  // processRandomize(buffer);
        if (m_myRank == 0)
...
void EW::processGrid(char* buffer) {
  ...
  //      // rounding of cube position to two decimals (prec=100), three
  //      (prec=1000) etc.. double prec = 100;

  //      if( found_latlon )
  //      {
  //         CHECK_INPUT( fabs(ibcaz - mGeoAz) < 1e-5, "Error: Az in Geodyn
  //         file, "
  ...
  // 	double x = metersPerDegree*( cos(phi)*(ibclat-gridLat) +
  // cos(ibclat*deg2rad)*(ibclon-gridLon)*sin(phi)); 	double y =
  // metersPerDegree*(-sin(phi)*(ibclat-gridLat) +
  // cos(ibclat*deg2rad)*(ibclon-gridLon)*cos(phi));
  //         x -= h*(x/h-round(x/h));
  //         y -= h*(y/h-round(y/h));
  //         gridLat = ibclat - (x*cos(phi) - y*sin(phi))/metersPerDegree;
  // 	gridLon = ibclon - (x*sin(phi) +
  // y*cos(phi))/(metersPerDegree*cos(ibclat*deg2rad)); mLatOrigin = gridLat;
  // mLonOrigin = gridLon;
  //         origin[0] = x;
  // 	origin[1] = y;
  //      }
  ...
  // 	else
  // 	{
  // 	   // h based on cube length and cube position, might be very
  // restrictive 	   CHECK_INPUT( false, "Error: cube position without
  // lat/long position must be adjustable");
  // 	}
  //      }
...
void EW::cleanUpRefinementLevels() {
  CHECK_INPUT(m_topo_zmax < m_global_zmax - m_h_base,
              "The topography is extending too deep into the ground and there "
              "is no space for the Cartesian grid.");
...
void EW::processTopography(char* buffer) {
  ...
  while (token != NULL) {
      ...
      if (m_grid_interpolation_order < 2 || m_grid_interpolation_order > 7) {
        if (m_myRank == 0)
          cout << "order needs to be 2,3,4,5,6,or 7 not: "
               << m_grid_interpolation_order << endl;
        MPI_Abort(MPI_COMM_WORLD, 1);
      }

      //        if (m_myRank==0)
      // 	 cout << "Setting interpolation order to=" <<
      // m_grid_interpolation_order << endl;
    }
    //                          123456789
    else if (startswith(
                 ...
                 token))  // developer option: not documented in user's guide
    {
      ...
      CHECK_INPUT(m_zetaBreak > 0 && m_zetaBreak <= 1,
                  "Error: zetabreak must be in [0,1], not " << m_zetaBreak);
...
void EW::processTestLamb(char* buffer) {
  ...
  // float_sw4 cs = 1.0, rho=1.0, cp=sqrt(3.0), fz=1.0, freq=1.0, f0=1.0; // the
  // exact solution assumes freq = 1
...
// {
//    // At this point, the geodyn file has already been read into
//    m_geodyn_filename char* token = strtok(buf, " \t");
//    CHECK_INPUT(strcmp("geodynbc", token) == 0, "ERROR: not a geodynbc
//    line...: " << token); CHECK_INPUT(m_geodynbc_found,"Error: geodynbc not
//    obtained"<<token);

//    ifstream geodynfile(m_geodynbc_filename.c_str());
//    CHECK_INPUT(geodynfile.is_open(), "Error: opening geodyn file " <<
//    m_geodynbc_filename );
...
//    }
//    geodynfile.close();
//    CHECK_INPUT( nx == ny, "Geodyn file error: x-y Cube dimensions must be
//    equal, not "
//                   <<nx << " " << ny );
//    CHECK_INPUT( faces == 5 || faces == 6 , "Geodyn file error: Faces must be
//    5 or 6, not" << faces ); CHECK_INPUT( timestepset, "Geodyn file error: No
//    time step given"); CHECK_INPUT( nstepsset, "Geodyn file error: Number of
//    steps not given"); mSimulation->set_geodyn_data( m_geodynbc_filename, nx,
//    nz, h, m_ibc_origin, timestep,
// 				 nsteps, faces );
// }
...
//    y0found=false, z0found=false; bool latfound=false, lonfound=false,
//    azfound=false, hfound=false, elevfound=false; while (!geodynfile.eof() &&
//    !done )
//    {
//       geodynfile.getline(buffer,256);
//       if (startswith("#", buffer) || startswith("\n", buffer) || buffer ==
...
//       nzfound = true;
//    }
//    CHECK_INPUT( (nxfound || nyfound || nzfound) && hfound, "Error in geodyn
//    file: dimensions not specified");
...
void EW::processCheckPoint(char* buffer) {
  ...
  if (restartFileGiven) {
    ...
    if (restartPathGiven) m_check_point->set_restart_path(restartPath);
  }
}
...
//-----------------------------------------------------------------------
int EW::getNumberOfSteps() const { return mNumberOfTimeSteps; }
...
void EW::allocateCartesianSolverArrays(float_sw4 a_global_zmax) {
  ...
  if (mVerbose >= 2 && proc_zero())
    printf(
        "allocateCartesianSolverArrays: #ghost points=%i, #parallel padding "
        "points=%i\n",
  ...
  //    m_topography_exists tells if there was a topography command in the input
  //    file?
  ...
  // used, for avoid segfault in parameter list with mMuVE[g], etc...
  ...
  // Cartesian grid where they meet
  if (m_topography_exists) {
  ...
  for (int g = nCartGrids - 1; g >= 0; g--) {
    ...
    //      cout << "defining " << ifirst << " " << ilast << " " << jfirst << "
    //      " << jlast << " " << kfirst << " " << klast << endl; cout << " zmin
    //      " << m_zmin[g] << endl;
    ...
    if (mVerbose >= 3 && proc_zero()) {
      printf(
          "Grid #%d, iInterior=[%d,%d], jInterior=[%d,%d], kInterior=[%d,%d]\n",
          g, m_iStartInt[g], m_iEndInt[g], m_jStartInt[g], m_jEndInt[g],
          m_kStartInt[g], m_kEndInt[g]);
    }
  ...
  if (m_topography_exists) {
    ...
    mTopo.define(ifirst, ilast, jfirst, jlast, 1,
                 1);  // true topography/bathymetry, read directly from etree
    m_ext_ghost_points = 2;
    // smoothed version of true topography, with an extended number (4 instead
    // of 2 ) of ghost points.
    mTopoGridExt.define(ifirst - m_ext_ghost_points, ilast + m_ext_ghost_points,
                        jfirst - m_ext_ghost_points, jlast + m_ext_ghost_points,
                        1, 1);

    // At this point, we don't know the number of grid points in the k-direction
    // of the curvi-linear grid. the arrays mX, mY, mZ must be allocated by the
    // grid generator
  }
...
void EW::allocateCurvilinearArrays() {
  ...
  // note that the z-coordinate points downwards, so positive topography (above
  // sea level) gets negative z-values
  ...
  int Nz = 1 + (int)((zMaxCart - 0.5 * (zMaxGlobal + zMinGlobal)) /
                     mGridSize[gTop]);  // on average the same gridsize in z
  ...
  if (mVerbose && proc_zero())
    printf(
        "allocateCurvilinearArrays: Number of grid points in curvilinear grid "
        "= %i, kStart = %i, kEnd = %i\n",
        Nz, m_kStart[gTop], m_kEnd[gTop]);
  // allocate mX, mY, and mZ arrays
  mX.define(m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop], m_jEnd[gTop],
            m_kStart[gTop], m_kEnd[gTop]);
  mY.define(m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop], m_jEnd[gTop],
            m_kStart[gTop], m_kEnd[gTop]);
  mZ.define(m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop], m_jEnd[gTop],
            m_kStart[gTop], m_kEnd[gTop]);
  // Allocate array for the metric
  mMetric.define(4, m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop], m_jEnd[gTop],
                 m_kStart[gTop], m_kEnd[gTop]);
  // and the Jacobian of the transformation
  mJ.define(m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop], m_jEnd[gTop],
            m_kStart[gTop], m_kEnd[gTop]);
  // and material properties, initialize to -1
  mRho[gTop].define(m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop], m_jEnd[gTop],
                    m_kStart[gTop], m_kEnd[gTop]);
  mRho[gTop].set_to_minusOne();

  if (m_anisotropic) {
    mC[gTop].define(21, m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop],
                    m_jEnd[gTop], m_kStart[gTop], m_kEnd[gTop]);
    mCcurv.define(45, m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop],
                  m_jEnd[gTop], m_kStart[gTop], m_kEnd[gTop]);
    mC[gTop].set_to_minusOne();
    mCcurv.set_to_minusOne();
  } else {
    mMu[gTop].define(m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop], m_jEnd[gTop],
                     m_kStart[gTop], m_kEnd[gTop]);
    mMu[gTop].set_to_minusOne();
    mLambda[gTop].define(m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop],
                         m_jEnd[gTop], m_kStart[gTop], m_kEnd[gTop]);
  ...
  if (m_use_attenuation) {
    // initialize the viscoelastic material coefficients to -1
    mQs[gTop].define(m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop], m_jEnd[gTop],
                     m_kStart[gTop], m_kEnd[gTop]);
    mQs[gTop].set_to_minusOne();
    mQp[gTop].define(m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop], m_jEnd[gTop],
                     m_kStart[gTop], m_kEnd[gTop]);
    mQp[gTop].set_to_minusOne();
    for (int a = 0; a < m_number_mechanisms;
         a++)  // the simplest attenuation model has m_number_mechanisms = 0
    {
      mMuVE[gTop][a].define(m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop],
                            m_jEnd[gTop], m_kStart[gTop], m_kEnd[gTop]);
      mMuVE[gTop][a].set_to_minusOne();
      mLambdaVE[gTop][a].define(m_iStart[gTop], m_iEnd[gTop], m_jStart[gTop],
                                m_jEnd[gTop], m_kStart[gTop], m_kEnd[gTop]);
...
void EW::processReceiver(char* buffer,
  ...
  if (!((inCurvilinear || z >= 0) && x >= 0 && x <= m_global_xmax && y >= 0 &&
    ...
    TimeSeries* ts_ptr =
        new TimeSeries(this, fileName, staName, mode, sacformat, usgsformat, x,
                       y, depth, topodepth, writeEvery, !nsew);
...
void EW::processRandomize(char* buffer) {
  ...
  while (token != NULL) {
    ...
    if (startswith("#", token) || startswith(" ", buffer))
      ...
      CHECK_INPUT(m_random_amp > 0,
                  "Error randomize, amplitude must be > 0, not " << token);
      ...
      lengthscaleset = true;
      CHECK_INPUT(m_random_dist > 0,
                  "Error randomize, dist must be > 0, not " << token);
    } else if (startswith("lengthscalez=", token)) {
      token += 13;
      m_random_distz = atof(token);
      lengthscalezset = true;
      CHECK_INPUT(m_random_distz > 0,
                  "Error randomize, distz must be > 0, not " << token);
  ...
  if (lengthscaleset && !lengthscalezset) m_random_distz = m_random_dist;
