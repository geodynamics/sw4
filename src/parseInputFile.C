...
// deal with topography
  if (m_topography_exists)
  {
	...
	extractTopographyFromUCVM( m_topoFileName );
     /* else if( m_topoInputStyle == EW::CUSVM) */
	/* extractTopographyFromCUSVM( m_topoFileName ); */
...
// grid, fileio, and topography, since we have already processed those commands.
	 ...
	 processMaterialUCVM( buffer );
       else if (startswith("cusvm", buffer))
	 processMaterialCUSVM( buffer );
...
// new keyword: geographic, but keeping grid for backwards compatibility with WPP
	  ...
	  else if (strcmp("cusvm", token) == 0)
	  {
	     m_topoInputStyle=CUSVM;
	     m_topography_exists=false;
	     /* m_topography_exists=true; */
	     needFileName=true; // we require the file name to be given on the topography command line
	  }
...
//-----------------------------------------------------------------------
void EW::processMaterialCUSVM(char* buffer)
{
   string name = "ucvm";
   string filename = "NONE";
   string directory = "NONE";
   ...
   while (token != NULL)
   {
	...
	// Ignore commented lines and lines with just a space.
	 ...
	 directory = token;
      ...
      {
	 cout << token << " is not a CUSVM option " << endl;
      }
      token = strtok(NULL, " \t");
   }
  // End parsing...
  ...
  if (m_myRank == 0)
     cout << "*** Using CUSVM " << filename << " in directory " << directory << endl;

  MaterialCUSVM* sf = new MaterialCUSVM(this, filename, directory);
  add_mtrl_block( sf  );
}
