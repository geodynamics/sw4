...
GridGeneratorGeneral::GridGeneratorGeneral(float_sw4 topo_zmax, bool always_new,
                                           sw4_type grid_interpolation_order,
                                           float_sw4 zetaBreak)
...
bool GridGeneratorGeneral::inverse_grid_mapping(EW* a_ew, float_sw4 x,
  ...
  if (m_always_new || ncurv > 1) {
    ...
    // 0. Check z
    if (!(bbox[4] - 3 * h < z && z < bbox[5] + 3 * h)) return false;
    return inverse_grid_mapping_old(a_ew, x, y, z, g, q, r, s,
...
//-----------------------------------------------------------------------
void GridGeneratorGeneral::generate_grid_and_met_old(EW* a_ew, Sarray& a_x,
                                                     Sarray& a_y, Sarray& a_z,
                                                     Sarray& a_jac,
                                                     Sarray& a_met) {
  SW4_MARK_FUNCTION;
...
#pragma omp parallel for
  for (sw4_type k = a_x.m_kb; k <= a_x.m_ke; k++) {
    float_sw4 s = (k - 1) * izb;
    float_sw4 omsm = (1 - s);
    for (sw4_type l = 2; l <= m_grid_interpolation_order; l++) omsm *= (1 - s);
...
bool GridGeneratorGeneral::grid_mapping_old(float_sw4 q, float_sw4 r,
  ...
  // points) 2) There is no curvilinear grid. Still compute (X0, Y0) based on
  ...
  if (s - 1 < m_zetaBreak * (Nz - 1)) {
    ...
    float_sw4 omsm = omra;
    for (sw4_type l = 2; l <= m_grid_interpolation_order; l++) omsm *= omra;
  ...
  if (a_ew->interior_point_in_proc(i, j, g)) {
    ...
    if (z >= zlim) {
      ...
      while (er > tol && it < maxit) {
        ...
        float_sw4 omsm = omra;
        for (sw4_type l = 2; l <= m_grid_interpolation_order - 1; l++) omsm *= omra;
...
void GridGeneratorGeneral::grid_mapping_diff_old(
  ...
  if (zpar >= 1) {
    ...
    sw4_type order = m_grid_interpolation_order;
    ...
    for (sw4_type j = jc - 3; j <= jc + 4; j++)
      for (sw4_type i = ic - 3; i <= ic + 4; i++) {
        ...
        tauq +=
            d6cofi[i - (ic - 3)] * a6cofj[j - (jc - 3)] * TopoGridExt(i, j, 1);
        taur +=
            a6cofi[i - (ic - 3)] * d6cofj[j - (jc - 3)] * TopoGridExt(i, j, 1);
        tauqq +=
            dd6cofi[i - (ic - 3)] * a6cofj[j - (jc - 3)] * TopoGridExt(i, j, 1);
        tauqr +=
            d6cofi[i - (ic - 3)] * d6cofj[j - (jc - 3)] * TopoGridExt(i, j, 1);
        taurr +=
            a6cofi[i - (ic - 3)] * dd6cofj[j - (jc - 3)] * TopoGridExt(i, j, 1);
      }
    zq = (-tauq) * powo;
    zr = (-taur) * powo;
    zqq = (-tauqq) * powo;
    ...
    if (analytic_derivative) {
      ...
      zss = -c1 * order * (order - 1) * pow(1 - zpar, order - 2) /
            (m_zetaBreak * m_zetaBreak * (Nz - 1) * (Nz - 1));
    } else {
      zs = 0;
      zss = 0;
      float_sw4 z1d = 0;
...
void GridGeneratorGeneral::generate_z_and_j(EW* a_ew, sw4_type g, Sarray& z,
  ...
  if (g > ncg) {
    //      scaleFact  = (m_topo_zmax -
    //      a_ew->m_curviRefLev[g-1-ncg])/m_topo_zmax;
    scaleRatio = (m_topo_zmax - a_ew->m_curviRefLev[g - 1 - ncg]) /
                 (m_topo_zmax - a_ew->m_curviRefLev[g - ncg]);
  }
  //   if( a_ew->getRank() == 4 )
