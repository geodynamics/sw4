...
// Cencalvm currently found at:
// http://earthquake.usgs.gov/research/structure/3dgeologic/download.php
using namespace std;
...
//-----------------------------------------------------------------------
EtreeFile::EtreeFile(EW* ew, const string& accessmode, const string& file,
                     const string& xfile, const string& model,
                     const string& logfile, const string& query,
                     const double res)
    : mEw(ew),
      mAccess(accessmode),
      mFileName(file),
  ...
  // we could check the vertical extent, but the horizontal extent is currently
  // not very precise (only uses the min and max (lat,lon)). Therefore, we make
  // the conservative guess that not all points are covered.
  mCoversAllPoints = false;
}

//-----------------------------------------------------------------------
EtreeFile::~EtreeFile() { delete[] mPayload; }
...
void EtreeFile::initialize(const string& model) {
  ...
  if (model == "SF") {
    ...
    VERIFY2(0, "Efile model must be SF not: " << model << endl);
  ...
  //   m_latmin = min( c1.getLatitude(), c2.getLatitude(), c3.getLatitude(),
  //   c4.getLatitude() ); m_latmax = max( c1.getLatitude(), c2.getLatitude(),
  //   c3.getLatitude(), c4.getLatitude() ); m_lonmin = min( c1.getLongitude(),
  //   c2.getLongitude(), c3.getLongitude(), c4.getLongitude() ); m_lonmax =
  //   max( c1.getLongitude(), c2.getLongitude(), c3.getLongitude(),
  //   c4.getLongitude() );
}

//-----------------------------------------------------------------------
void EtreeFile::getbox(double& latmin, double& latmax, double& lonmin,
                       double& lonmax) const {
  latmin = m_latmin;
  latmax = m_latmax;
  lonmin = m_lonmin;
...
//-----------------------------------------------------------------------
void EtreeFile::getcorners(double& latse, double& lonse, double& latsw,
                           double& lonsw, double& latne, double& lonne,
                           double& latnw, double& lonnw) const {
  latse = m_latse;
  lonse = m_lonse;
  latsw = m_latsw;
  lonsw = m_lonsw;
...
void EtreeFile::setupEFile() {
  ...
  if (mXFileName != "NONE") mQuery.filenameExt(mXFileName.c_str());

  // For anything other than the SF model
  if (mQueryGeom != 0) mQuery.geometry(mQueryGeom);
...
//-----------------------------------------------------------------------
void EtreeFile::readEFile(std::vector<Sarray>& rho, std::vector<Sarray>& cs,
                          std::vector<Sarray>& cp, std::vector<Sarray>& qs,
                          std::vector<Sarray>& qp, int& outside,
                          int& material) {
  int myRank;
  MPI_Comm_rank(MPI_COMM_WORLD, &myRank);
  ...
  double x, y, z, lon, lat, elev, density, vp, vs, elevation, qup, qus,
      faultBlock;
  ...
  // the topoMat array is needed for "squishing" the material properties near
  // the free surface when a Cartesian grid is used on top
  topoMat.define(mEw->m_iStart[topLevel], mEw->m_iEnd[topLevel],
                 mEw->m_jStart[topLevel], mEw->m_jEnd[topLevel], 1, 1);
  // topoMat holds the highest elevation where the etree returns solid material
  // properties start by figuring out the max elevation where the etree returns
  // solid material
  for (int j = mEw->m_jStart[topLevel]; j <= mEw->m_jEnd[topLevel]; ++j)
    for (int i = mEw->m_iStart[topLevel]; i <= mEw->m_iEnd[topLevel]; ++i) {
      ...
      if (mQuery.errorHandler()->status() !=
          cencalvm::storage::ErrorHandler::OK) {
        // If query generated an error, then bail out, otherwise reset status
        mQuery.errorHandler()->resetStatus();
        if (verbose)
          printf(
              "WARNING: Etree query failed for topoMat elevation at grid point "
              "(i,j)= (%i,%i) at (x,y)=(%e,%e) in grid g=%i\n"
      ...
      // get the max elevation where the etree will return solid material (for
      // assigning material properties in the curvilinear grid)
      elev = mPayload[3];
      int maxIter = 20;
      for (int iter = 0; iter < maxIter; iter++) {
        ...
        if (mPayload[1] != NODATAVAL &&
            mPayload[2] !=
                NODATAVAL)  // bail out if we have valid Vp and Vs values
          break;
        elev -= elevDelta;  // otherwise reduce the elevation and try again
      }
      // make sure Vp and Vs are defined
      CHECK_INPUT(mPayload[1] != NODATAVAL && mPayload[2] != NODATAVAL,
                  "Vp and/or Vs are not defined for grid point (i,j)= ("
                      << i << ", " << j
                      << ") in curvilinear grid g = " << topLevel << endl
                      << " lat= " << lat << " lon= " << lon
                      << " query elevation= " << elev
                      << " topo/bathy elevation= " << mPayload[3]
                      << " Vp= " << mPayload[1] << " Vs= " << mPayload[2]);
  ...
  if (mEw->topographyExists()) {
    ...
    for (int j = mEw->m_jStart[g]; j <= mEw->m_jEnd[g]; ++j)
      for (int i = mEw->m_iStart[g]; i <= mEw->m_iEnd[g]; ++i) {
        for (int k = 1; k <= mEw->m_kEnd[g];
             ++k)  // don't attempt querying the etree above the topography
                   // (start at k=1)
        {
          x = mEw->mX(i, j, k);
          y = mEw->mY(i, j, k);
          // zeta is a linear function of k which should be zeta=1 for k=1 and
          // zeta=0 for k=m_kEnd - m_ghost_points
          zeta = ((double)(mEw->m_kEnd[g] - ghost_points - k)) /
                 (mEw->m_kEnd[g] - ghost_points - 1.);
          // modify the z coordinate to account for the difference between raw
          // and smoothed topography.
          z = mEw->mZ(i, j, k) +
              (mEw->mTopoGridExt(i, j, 1) - topoMat(i, j, 1)) * zeta;
          mEw->computeGeographicCoord(x, y, lon, lat);
          elev = -z;
          ...
          if (mQuery.errorHandler()->status() !=
              cencalvm::storage::ErrorHandler::OK) {
            mQuery.errorHandler()
                ->resetStatus();  // C.B> If we have an error, we are outside.
            outside++;
            continue;
          }
          ...
          // Vp, Vs, Density = NODATAVAL (-999) means the material properties
          // are undefined
          if (vp == NODATAVAL || vs == NODATAVAL || density == NODATAVAL) {
            if (k == 1)  // game over...
            {
              CHECK_INPUT(vp != NODATAVAL,
                          "Vp undefined for grid point (i,j,k)= ("
                              << i << ", " << j << ", " << k
                              << ") in curvilinear grid g = " << g << endl
                              << " lat= " << lat << " lon= " << lon
                              << " elevation= " << elev << endl
                              << "Density=" << mPayload[0]
                              << " Vp=" << mPayload[1] << " Vs=" << mPayload[2]
                              << " Elevation=" << mPayload[3]
                              << " Qp=" << mPayload[4] << " Qs=" << mPayload[5]
                              << " Faultblock= " << mPayload[6]);
              CHECK_INPUT(
                  vs != NODATAVAL,
                  "Water material properties detected for grid point (i,j,k)= ("
                      << i << ", " << j << ", " << k
                      << ") in curvilinear grid g = " << g << endl
                      << " lat= " << lat << " lon= " << lon
                      << " elevation= " << elev << endl
                      << "Density=" << mPayload[0] << " Vp=" << mPayload[1]
                      << " Vs=" << mPayload[2] << " Topo Elevation="
                      << mPayload[3] << " Qp=" << mPayload[4]
                      << " Qs=" << mPayload[5] << " Faultblock= " << mPayload[6]
                      << " mZ(i,j,k)= " << mEw->mZ(i, j, k)
                      << " topoGrid= " << mEw->mTopoGridExt(i, j, 1)
                      << " topoMat= " << topoMat(i, j, 1));

              CHECK_INPUT(density != NODATAVAL,
                          "Density undefined for grid point (i,j,k)= ("
                              << i << ", " << j << ", " << k
                              << ") in curvilinear grid g = " << g << endl
                              << " lat= " << lat << " lon= " << lon
                              << " elevation= " << elev << endl
      ...
      for (int i = mEw->m_iStart[g]; i <= mEw->m_iEnd[g]; ++i) {
        for (int k = kLow; k <= mEw->m_kEnd[g]; ++k) {
          ...
          if (g == topLevel) {
            ...
            zeta = ((double)(mEw->m_kEnd[g] - ghost_points - k)) /
                   (mEw->m_kEnd[g] - ghost_points - 1.);
            // modify the z coordinate to account for the difference between the
            // z=0 flat grid and the top elevation where material data is
            // available
            z = mEw->m_zmin[g] + (k - 1) * mEw->mGridSize[g] +
                (0.0 - topoMat(i, j, 1)) * zeta;
          } else {
            z = mEw->m_zmin[g] + (k - 1) * mEw->mGridSize[g];
          }
          mEw->computeGeographicCoord(x, y, lon, lat);
          ...
          //---------------------------------------------------------
          mQuery.query(&mPayload, mPayloadSize, lon, lat, elev);
          if (cencalvm::storage::ErrorHandler::OK !=
              mQuery.errorHandler()->status()) {
            mQuery.errorHandler()
                ->resetStatus();  // C.B> If we have an error, we are outside.
            outside++;
            continue;
          ...
          // NOTE: COPIED THE LOGIC FOR USING THE PREVIOUS k-VALUE IF WATER IS
          // DETECTED (AS ABOVE) Vp, Vs, Density = NODATAVAL (-999) means the
          // material properties are undefined
          if (vp == NODATAVAL || vs == NODATAVAL || density == NODATAVAL) {
            if (k == kLow)  // game over...
            {
              CHECK_INPUT(vp != NODATAVAL,
  ...
  if (myRank == 0) cout << "Done reading etree properties..." << endl;
  mQuery.close();
}  // end readEFile
...
void EtreeFile::set_material_properties(std::vector<Sarray>& rho,
  ...
  if (mAccess == "serial") {
    for (int procID = 0; procID < comm_size; ++procID) {
      ...
      //-------------------------------------------------------
      if (myRank == procID) {
        readEFile(rho, cs, cp, qs, qp, outside, material);
      }
      MPI_Barrier(MPI_COMM_WORLD);
    }
  } else if (mAccess == "parallel") {
    if (myRank == 0) cout << "Reading efile..." << endl;

    readEFile(rho, cs, cp, qs, qp, outside, material);
  }
  VERIFY2(
      mQuery.errorHandler()->status() != cencalvm::storage::ErrorHandler::ERROR,
  ...
  //
  //       vp >= sqrt(2)*vs
  // DUE TO ROUNDOFF WHEN CONVERTING TO mu, lambda, the ratio needs to be
  // slightly larger than sqrt(2). sqrt(2)=1.4142135...   We use 1.42 to be on
  // the safe side
  // ----------------------------------------------------------------
      ...
      for (int j = mEw->m_jStart[g]; j <= mEw->m_jEnd[g]; ++j)
        for (int k = mEw->m_kStart[g]; k <= mEw->m_kEnd[g]; ++k) {
          if (cp[g](i, j, k) < sqrt2 * cs[g](i, j, k)) {
            warningCount++;
            cp[g](i, j, k) = cs[g](i, j, k) *
                             sqrt2;  // better to increase cp than decrease cs
          }
        }

  // Summarize the warnings
  int warningSum = 0;
  MPI_Reduce(&warningCount, &warningSum, 1, MPI_INT, MPI_SUM, 0,
             MPI_COMM_WORLD);
