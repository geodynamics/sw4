...
void Mopt::processMaterialParCart(char* buffer) {
  ...
  VERIFY2(
      nx <= m_ew->m_global_nx[0] && ny <= m_ew->m_global_ny[0] &&
          nz <= m_ew->m_global_nz[0],
  ...
  if (!shared)
    m_mp =
        new MaterialParCart(m_ew, nx, ny, nz, init, varcase, file, amp, omega);
  else {
    if (vel)
      m_mp = new MaterialParCartesianVels(m_ew, nx, ny, nz, init, file);
    else if (vponly)
      m_mp = new MaterialParCartesianVp(m_ew, nx, ny, nz, init, file, ratio,
                                        gamma, true);
    else if (vsvp)
