...
Mopt::Mopt(EW* a_ew) {
  ...
  m_wave_mode =
      2;  // default to both P and S waves otherwise 0 for P and 1 for S only
...
bool Mopt::parseInputFileOpt(std::string filename) {
  ...
  if (m_ew->getVerbosity() >= 3 && m_myrank == 0)
    cout << "********parseInputFileOpt: Done reading the input file*********"
         << endl;
  return true;
}
...
void Mopt::processMaterialParCart(char* buffer) {
  ...
  while (token != NULL) {
    ...
    if (startswith("#", token) || startswith(" ", buffer))
      ...
      CHECK_INPUT(strcmp(token, "mpar") == 0 || strcmp(token, "mpc") == 0,
                  "ERROR: processing mparcart, file type " << token
                                                           << "not recognized");
      ...
      token += 7;
      shared = strcmp(token, "1") == 0 || strcmp(token, "yes") == 0 ||
               strcmp(token, "on") == 0;
    } else {
  ...
  int nztot = 0;
  for (int g = 0; g < m_ew->mNumberOfGrids; g++) nztot += m_ew->m_global_nz[g];

  VERIFY2(
      nx <= m_ew->m_global_nx[0] && ny <= m_ew->m_global_ny[0] && nz <= nztot,
      "MaterialParCart: The material grid must be coarser than the global grid")
  ...
  if (m_ew->topographyExists())
    m_mp = new MaterialParCurv(m_ew, nx, ny, nz, init, varcase, file, amp,
                               omega, shared);
  else
    m_mp = new MaterialParCart(m_ew, nx, ny, nz, init, varcase, file, amp,
                               omega, shared);
  //   if( !shared )
  //      m_mp = new MaterialParCart( m_ew, nx, ny, nz, init, varcase, file,
  //      amp, omega );
  //   else
  //   {
  //      if (vel)
  ...
  //         );
  //      else if( vponly )
  //         m_mp = new MaterialParCartesianVp( m_ew, nx, ny, nz, init, file,
  //         ratio, gamma, true );
  //      else if( vsvp )
...
void Mopt::processMrun(char* buffer) {
  ...
  while (token != NULL) {
    ...
    if (startswith("#", token) || startswith(" ", buffer))
      ...
      token += 7;
      CHECK_INPUT(strcmp("on", token) == 0 || strcmp("off", token) == 0,
                  "ERROR, mrun mcheck= " << token << " not understood" << endl);
      m_mcheck = strcmp("on", token) == 0;
    } else if (startswith("tsoutput=", token)) {
      token += 9;
      CHECK_INPUT(
          strcmp("on", token) == 0 || strcmp("off", token) == 0,
          "ERROR, mrun tsoutput= " << token << " not understood" << endl);
      m_output_ts = strcmp("on", token) == 0;
    } else if (startswith("quiet=", token)) {
      token += 6;
      CHECK_INPUT(strcmp("yes", token) == 0 || strcmp("no", token) == 0,
                  "ERROR, mrun quiet= " << token << " not understood" << endl);
      ...
      if (strcmp(token, "l2") == 0)
        ...
        CHECK_INPUT(false, "ERROR, mrun misfit= " << token << " not understood"
                                                  << endl);
      ...
      int n = strlen(token);
      bool filtergrad = strncmp("yes", token, n) == 0 ||
                        strncmp("on", token, n) == 0 ||
                        strncmp("1", token, n) == 0;
      m_ew->set_filtergrad();
    ...
    } else if (startswith("twinscale=", token))  // scale of win size
    {
      ...
      token += 9;
      int n = strlen(token);
      if (strncmp("yes", token, n) == 0 || strncmp("1", token, n) == 0 ||
          strncmp("on", token, n) == 0)
        m_write_dfm = true;
...
void Mopt::processMregularize(char* buffer) {
  ...
  char* token = strtok(buffer, " \t");
  int n;
  CHECK_INPUT(strcmp("regularize", token) == 0,
              "ERROR: not a regularize line...: " << token);
  token = strtok(NULL, " \t");
  ...
  while (token != NULL) {
    ...
    if (startswith("coeff=", token)) {
      ...
      token += 9;
      n = strlen(token);
      if (strncmp(token, "yes", n) == 0 || strncmp(token, "on", n) == 0 ||
          strncmp(token, "1", n) == 0)
...
void Mopt::set_sscalefactors() {
  ...
  if (m_nstot > 0) {
    ...
    if (!m_scales_file_given)
      m_mp->set_scalefactors(my_nmpars, sfs, m_rhoscale, m_muscale,
                             m_lambdascale, m_vsscale, m_vpscale);
    else {
      int errflag = 0;
      if (m_myrank == 0) {
        cout << "Reading scale factor file "
             << " my_nmpars = " << my_nmpars << endl;
        string fname = m_path + m_scales_fname;
        //	 int fd=open(m_scales_fname.c_str(),O_RDONLY );
        int fd = open(fname.c_str(), O_RDONLY);
        VERIFY2(fd != -1, "ERROR reading scale factors. File "
                              << m_scales_fname << " could not be opened"
                              << endl);
        int nmpars_read;
        size_t nr = read(fd, &nmpars_read, sizeof(int));
        if (nmpars_read == my_nmpars && nr == sizeof(int)) {
          nr = read(fd, sfs, my_nmpars * sizeof(double));
          if (nr != my_nmpars * sizeof(double)) errflag = 2;
        ...
        double imf = 1 / sqrt(m_misfitscale);
        for (int i = 0; i < my_nmpars; i++) sfs[i] *= imf;
      }
      MPI_Bcast(sfs, my_nmpars, MPI_DOUBLE, 0, m_ew->m_1d_communicator);
      VERIFY2(errflag == 0,
              "Error no " << errflag << " in Mopt::set_sscalefactors");
    }
  }
}
...
void Mopt::set_baseMat(double* xs, double* xm) {
  ...
  for (int i = 0; i < my_nmpars; i++) m_xs0[i] = xs[i];
  for (int i = 0; i < nmpard; i++) m_xm0[i] = xm[i];
}
...
void Mopt::set_dscalefactors() {
  ...
  if (nmpard > 0) {
    ...
    if (!m_scales_file_given)
      m_mp->set_scalefactors(nmpard, m_sfm, m_rhoscale, m_muscale,
                             m_lambdascale, m_vsscale, m_vpscale);
    else {
      // Not yet implemented
      for (int i = 0; i < nmpard; i++) m_sfm[i] = 1;
    }
  }
}
...
void Mopt::set_sourcescalefactors(int nspar, double* sfs) {
  ...
  if (nspar == 11)
    for (int i = 0; i < 11; i++) sfs[i] = sfall[i];
  else if (nspar == 10)
    for (int i = 0; i < 10; i++) sfs[i] = sfall[i];
  else if (nspar == 9)
    for (int i = 0; i < 9; i++) sfs[i] = sfall[i];
  else if (nspar == 6)
    for (int i = 0; i < 6; i++) sfs[i] = sfall[i + 3];
  else if (nspar != 0)
  ...
  while (token != NULL) {
    ...
    if (startswith("#", token) || startswith(" ", buffer))
      ...
      CHECK_INPUT(m_nbfgs_vectors > 0,
                  "ERROR: lbfgs, nvectors must be positive, not "
                      << m_nbfgs_vectors << endl);
      ...
      if (strcmp(token, "scale-factors") == 0)
        ...
        CHECK_INPUT(false, "ERROR: lbfgs ihess0=" << token << " not understood "
                                                  << endl);
...
void Mopt::processMfsurf(char* buffer) {
  ...
  while (token != NULL) {
    ...
    if (startswith("#", token) || startswith(" ", buffer))
      ...
      token += 4;
      CHECK_INPUT(strcmp(token, "rho") == 0 || strcmp(token, "mu") == 0 ||
      ...
      token += 5;
      CHECK_INPUT(
          strcmp(token, "rho") == 0 || strcmp(token, "mu") == 0 ||
              strcmp(token, "lambda") == 0,
      ...
      token += 10;
      m_misfit1d_images =
          (strcmp(token, "1") == 0) || (strcmp(token, "yes") == 0);
    } else
...
void Mopt::processMimage(char* buffer, bool use_hdf5) {
  ...
  while (token != NULL) {
    ...
    if (startswith("#", token) || startswith(" ", buffer))
      ...
      if (strcmp(token, "rho") == 0)
        ...
        CHECK_INPUT(
            false,
            "Processing mimage command: mode must be one of the following: "
                << endl
                << "rho|mu|lambda|p|s|gradrho|gradmu|gradlambda|gradp|grads"
                << endl
                << "not " << token << endl);
    } else if (startswith("precision=", token)) {
      token += 10;
      CHECK_INPUT(strcmp(token, "double") == 0 || strcmp(token, "float") == 0,
                  "Processing mimage command: "
                      << " precision must be float or double, not " << token
                      << endl);
  ...
  CHECK_INPUT(coordWasSet,
              "ERROR: Processing image command: one of the coordinate (x,y,z) "
              "option must be set "
                  << " to determine the image's 2D plane" << endl);
  double time = -1, timeInterval = -1;
  Image* i = new Image(m_ew, time, timeInterval, iter, iterInterval, filePrefix,
                       mode, locationType, coordValue, use_double, use_hdf5);
...
void Mopt::processSfileoutput(char* buffer) {
  ...
  SfileOutput* sfile =
      new SfileOutput(m_ew, time, timeInterval, cycle, cycleInterval, tStart,
...
void Mopt::processM3Dimage(char* buffer) {
  ...
  char* token = strtok(buffer, " \t");
  CHECK_INPUT(strcmp("m3dimage", token) == 0,
              "ERROR: Not a m3dimage line...: " << token);
  ...
  while (token != NULL) {
    ...
    if (startswith("iter=", token)) {
      ...
      CHECK_INPUT(atoi(token) >= 0.,
                  "Processing m3dimage command: iter must be a non-negative "
                  "integer, not: "
                      << token);
      iter = atoi(token);
      timingSet = true;
      ...
      token += 13;  // skip iterInterval=
      CHECK_INPUT(atoi(token) >= 0.,
                  "Processing m3dimage command: iterInterval must be a "
      ...
      if (strcmp(token, "rho") == 0)
        ...
        CHECK_INPUT(
            0,
            "Processing m3dimage command: "
                << "mode must be one of the following: " << endl
                << "\t rho|p|s|mu|lambda|gradrho|gradp|grads|gradmu|gradlambda "
                   "*not: "
                << token);
      }
    } else if (startswith("precision=", token)) {
      ...
      CHECK_INPUT(
          ...
          "Processing m3dimage command: precision must be float or double, not "
          "'" << token);
      use_double = startswith("double", token);
    } else {
  ...
  CHECK_INPUT(
      timingSet,
      "Processing m3dimage command: "
          << "at least one timing mechanism must be set: iter, iterInterval"
          << endl);
...
void Mopt::init_pseudohessian(vector<Sarray>& ph) {
  ...
  if (m_use_pseudohessian) {
    ...
    for (int g = 0; g < m_ew->mNumberOfGrids; g++) {
      ph[g].define(3, m_ew->m_iStart[g], m_ew->m_iEnd[g], m_ew->m_jStart[g],
