...
float_sw4 CheckPoint::getDt()
{
   ...
   if( mEW->getRank() == 0 )
   {
	 ...
	 s << mEW->getPath();
      ...
      if (mUseHDF5) {
#ifdef USE_HDF5
         hid_t fid = H5Fopen( const_cast<char*>(s.str().c_str()), H5F_ACC_RDONLY, H5P_DEFAULT); 
...
void CheckPoint::write_header_hdf5( hid_t fid, float_sw4 a_time, int a_cycle)
{
   ...
   int prec = m_double ? 8 : 4;
   attr = H5Acreate(fid, "prec", H5T_NATIVE_INT, attr_space, H5P_DEFAULT, H5P_DEFAULT);
   ret = H5Awrite(attr, H5T_NATIVE_INT, &prec);
   CHECK_INPUT( ret >= 0,"CheckPoint::write_header_hdf5: Error writing precision" );
   H5Aclose(attr);

   int ng = mEW->mNumberOfGrids;
   attr = H5Acreate(fid, "ngrid", H5T_NATIVE_INT, attr_space, H5P_DEFAULT, H5P_DEFAULT);
   ...
   attr = H5Acreate(fid, "cycle", H5T_NATIVE_INT, attr_space, H5P_DEFAULT, H5P_DEFAULT);
   ...
   int nmech = mEW->getNumberOfMechanisms();
   attr = H5Acreate(fid, "nmesh", H5T_NATIVE_INT, attr_space, H5P_DEFAULT, H5P_DEFAULT);
   ...
   attr = H5Acreate(fid, "nrank", H5T_NATIVE_INT, attr_space, H5P_DEFAULT, H5P_DEFAULT);
   ...
   for(int g = 0; g < ng ;g++ )
   {
      ...
      globalSize[4] = 1;
      globalSize[5] = mGlobalDims[g][5]-mGlobalDims[g][4]+1;
      attr = H5Acreate(fid, name, H5T_NATIVE_INT, attr_space6, H5P_DEFAULT, H5P_DEFAULT);
      ret = H5Awrite(attr, H5T_NATIVE_INT, globalSize);
      CHECK_INPUT( ret >= 0,"CheckPoint::write_header_hdf5: Error writing global sizes" );
      H5Aclose(attr);
   }
   H5Sclose(attr_space);
...
void CheckPoint::read_header_hdf5( hid_t fid, float_sw4& a_time, int& a_cycle)
{
   ...
   H5Aclose(attr);
   CHECK_INPUT( ret >= 0,"CheckPoint::read_header_hdf5: Error reading precision" );
   CHECK_INPUT( (m_double && prec==8) || (!m_double && prec==4), 
   ...
   CHECK_INPUT( ng == mEW->mNumberOfGrids, "CheckPoint::read_header_hdf5: Error number of grids on restart file"
		<< " does not match number of grids in solver" );
   ...
   CHECK_INPUT( nmech == mEW->getNumberOfMechanisms(), "CheckPoint::read_header_hdf5: Error number "
		<< "of attenuation mechanisms on restart file"
		<< " does not match number of attenuation mechanisms in solver" );
   ...
   CHECK_INPUT( nrank == nrank_restart, "CheckPoint::read_header_hdf5: Error number "
		<< "of ranks on restart file does not match number of ranks in currrent run" );
   ...
   for(int g = 0; g < ng ;g++ )
   {
      ...
      H5Aclose(attr);
      CHECK_INPUT( ret >= 0,"CheckPoint::read_header_hdf5: Error reading global sizes" );
      CHECK_INPUT( globalSize[0] == 1, "CheckPoint::read_checkpoint: Error in global sizes, " <<
		   "low i-index is "<< globalSize[0]);
      CHECK_INPUT( globalSize[1] == mGlobalDims[g][1]-mGlobalDims[g][0]+1, "CheckPoint::read_checkpoint: Error in global sizes, "
		   << "upper i-index is " << globalSize[1]);
...
void CheckPoint::create_hdf5_dset(hid_t fid, char *dset_name, hid_t dtype, hid_t dspace, hid_t dcpl)
{
  hid_t dset;
#ifdef USE_HDF5_ASYNC
  dset = H5Dcreate_async(fid, dset_name, dtype, dspace, H5P_DEFAULT, dcpl, H5P_DEFAULT, m_es_id);
  H5Dclose_async(dset, m_es_id);
#else
  dset = H5Dcreate(fid, dset_name, dtype, dspace, H5P_DEFAULT, dcpl, H5P_DEFAULT);
  H5Dclose(dset);
#endif
}

void CheckPoint::write_hdf5_dset(hid_t fid, char *dset_name, hid_t dtype, hid_t mspace, hid_t mydspace, hid_t dxpl, void *buf)
{
  hid_t dset;
#ifdef USE_HDF5_ASYNC
				  ...
				  vector<Sarray*>& a_AlphaVEm, vector<Sarray*>& a_AlphaVE )
{
   ...
   for( int g = 0 ; g < mEW->mNumberOfGrids ; g++ ) {
      ...
      H5Dclose(dset);
      if( m_kji_order )
         a_Um[g].insert_subarrayIK( mWindow[g][0], mWindow[g][1], mWindow[g][2], mWindow[g][3], mWindow[g][4], mWindow[g][5], doubleField );
      else
         a_Um[g].insert_subarray( mWindow[g][0], mWindow[g][1], mWindow[g][2], mWindow[g][3], mWindow[g][4], mWindow[g][5], doubleField );
      ...
      H5Dclose(dset);
      if( m_kji_order )
         a_U[g].insert_subarrayIK( mWindow[g][0], mWindow[g][1], mWindow[g][2], mWindow[g][3], mWindow[g][4], mWindow[g][5], doubleField );
      else
         a_U[g].insert_subarray( mWindow[g][0], mWindow[g][1], mWindow[g][2], mWindow[g][3], mWindow[g][4], mWindow[g][5], doubleField);
      ...
      for( int m=0 ; m < mEW->getNumberOfMechanisms() ; m++ ) {
         ...
         H5Dclose(dset);
         if( m_kji_order )
            a_AlphaVEm[g][m].insert_subarrayIK( mWindow[g][0], mWindow[g][1], mWindow[g][2], mWindow[g][3], mWindow[g][4], mWindow[g][5], doubleField );
         else
            a_AlphaVEm[g][m].insert_subarray( mWindow[g][0], mWindow[g][1], mWindow[g][2], mWindow[g][3], mWindow[g][4], mWindow[g][5], doubleField );
         ...
         H5Dclose(dset);
         if( m_kji_order )
            a_AlphaVE[g][m].insert_subarrayIK(  mWindow[g][0], mWindow[g][1], mWindow[g][2], mWindow[g][3], mWindow[g][4], mWindow[g][5], doubleField );
         else
            a_AlphaVE[g][m].insert_subarray(  mWindow[g][0], mWindow[g][1], mWindow[g][2], mWindow[g][3], mWindow[g][4], mWindow[g][5], doubleField );
      }
      H5Sclose(dspace);
      H5Sclose(mspace);
      H5Sclose(mydspace);
