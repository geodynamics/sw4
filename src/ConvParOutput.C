...
ConvParOutput::ConvParOutput(EW& ew, int n, int nvar, int myrank, int verbose,
  ...
  if (m_stepfileio && m_myrank == 0) {
    ...
    if (m_cg) {
      fprintf(m_fd,
              "it  sub-it  max-nrm-gradient  max-nrm-model-update  misfit\n");
    } else  // lbfgs
    {
...
//-----------------------------------------------------------------------
void ConvParOutput::print_xmsg(double* x, double rnorm, double dxnorm, int j,
                               int k) {
  if (m_myrank == 0) {
    cout << "------------------------------------------------------------------"
            "-----"
         << endl;
    if (m_cg) {
      cout << " it=" << j << " " << k << " max-norm scaled gradient= " << rnorm
...
void ConvParOutput::print_scalar(double s, const char* msg, int m) {
  if (m_myrank == 0 && m_verbose > m) cout << msg << " " << s << endl;
}

//-----------------------------------------------------------------------
void ConvParOutput::save_step(double f, double* df, double* sf, double* x,
                              double rnorm, double dxnorm, int nreductions,
                              int j, int k) {
  if (m_myrank == 0) {
    if (m_stepfileio) {
      if (m_cg) {
      ...
      for (int i = 0; i < m_nvar; i++) fprintf(m_fdx, " %15.7g ", x[i]);
      for (int i = 11; i < m_n; i++) fprintf(m_fdx, " %15.7g ", x[i]);
      fprintf(m_fdx, "\n");
      fflush(m_fd);
      fflush(m_fdx);
    } else {
      ...
      for (int i = 0; i < m_nvar; i++) m_paradata.push_back(x[i]);
      //	    m_paradata[m_nsize*m_ind+i] = x[i];
      for (int i = 11; i < m_n; i++) m_paradata.push_back(x[i]);
      //	    m_paradata[m_nsize*m_ind+i] = x[i];
      m_ind++;
    }
  }
  ...
  if (m_myrank == 0) {
    ...
    } else  // what case is this?
    {
      ...
      for (int i = 0; i < m_ind; i++) {
        if (m_cg) {
          fprintf(m_fd, "%i %i %15.7g %15.7g %15.7g %i \n", m_iconvdata[3 * i],
                  m_iconvdata[3 * i + 1], m_convdata[3 * i],
                  m_convdata[3 * i + 1], m_convdata[3 * i + 2],
                  m_iconvdata[3 * i + 2]);
          fprintf(m_fdx, "%i %i ", m_iconvdata[3 * i], m_iconvdata[3 * i + 1]);
        } else {
          fprintf(m_fd, "%i %15.7g %15.7g %15.7g %i \n", m_iconvdata[3 * i],
                  m_convdata[3 * i], m_convdata[3 * i + 1],
                  m_convdata[3 * i + 2], m_iconvdata[3 * i + 2]);
