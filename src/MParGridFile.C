...
//-----------------------------------------------------------------------
MParGridFile::MParGridFile( std::string fname )
{
   read_mparcartfile(fname);
}
			    ...
			    int update )
{
   ...
   for( int i=0 ; i < nc*m_nxr*m_nyr*m_nzr ; i++ )
      m_xms[i] = xms[i];
}

//-----------------------------------------------------------------------
MParGridFile::~MParGridFile( )
{
   delete[] m_xms;
}
					 ...
					 float_sw4 hy, float_sw4 hz, float_sw4 xmin, float_sw4 ymin, float_sw4 zmin )
{
   ...
   else
   {
	 ...
	 if( vars == 1 )
	 {
	    if( m_vars == 2 )
	    {
	       ...
	       xmsrc[3*ind+1] = m_xms[3*ind]*m_xms[3*ind+1]*m_xms[3*ind+1]; // mu=rho*cs*cs
	       xmsrc[3*ind+1] = m_xms[3*ind]*m_xms[3*ind+2]*m_xms[3*ind+2]-2*xmsrc[3*ind+1]; // la=rho*cp*cp-2*mu
	    }
	    else
	       fail = true;
	 }
	 ...
	 {
	    if( m_vars == 1 )
	       xmsrc[ind] = sqrt((2*m_xms[3*ind+1]+m_xms[3*ind+2])/m_xms[3*ind]);
	    else if( m_vars == 2 )
   ...
   if( fail )
   {
      std::cout << "ERROR: MParGridFile, attempt to extract too many components" << std::endl;
      if( vars != m_vars )
	 delete[] xmsrc;
      return;
   }
