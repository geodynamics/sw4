...
//-----------------------------------------------------------------------
void interpolatemtrl(
    int nx, int ny, int nz, float_sw4 xmin, float_sw4 ymin, float_sw4 zmin,
    float_sw4 hx, float_sw4 hy, float_sw4 hz, float_sw4* __restrict__ a_rho,
    float_sw4* __restrict__ a_mu, float_sw4* __restrict__ a_lambda, int ib,
    int ie, int jb, int je, int kb, int ke, int ibact, int ieact, int jbact,
    int jeact, int kbact, int keact, float_sw4* __restrict__ a_rhogrid,
...
#define lambdagrid(i, j, k) \
  a_lambdagrid[i - ib + nig * (j - jb) + nijg * (k - kb)]
    ...
    for (int k = kbact; k <= keact; k++) {
      ...
      for (int j = jbact; j <= jeact; j++) {
        ...
        for (int i = ibact; i <= ieact; i++) {
          ...
          rhogrid(i, j, k) =
              rhogrid(i, j, k) +
              (1 - wgz) * ((1 - wgy) * ((1 - wgx) * rho(ic, jc, kc) +
                                        wgx * rho(ic + 1, jc, kc)) +
                           wgy * ((1 - wgx) * rho(ic, jc + 1, kc) +
                                  wgx * rho(ic + 1, jc + 1, kc))) +
              wgz * ((1 - wgy) * ((1 - wgx) * rho(ic, jc, kc + 1) +
                                  wgx * rho(ic + 1, jc, kc + 1)) +
                     wgy * ((1 - wgx) * rho(ic, jc + 1, kc + 1) +
                            wgx * rho(ic + 1, jc + 1, kc + 1)));
          mugrid(i, j, k) =
              mugrid(i, j, k) +
              (1 - wgz) * ((1 - wgy) * ((1 - wgx) * mu(ic, jc, kc) +
                                        wgx * mu(ic + 1, jc, kc)) +
                           wgy * ((1 - wgx) * mu(ic, jc + 1, kc) +
                                  wgx * mu(ic + 1, jc + 1, kc))) +
              wgz * ((1 - wgy) * ((1 - wgx) * mu(ic, jc, kc + 1) +
                                  wgx * mu(ic + 1, jc, kc + 1)) +
                     wgy * ((1 - wgx) * mu(ic, jc + 1, kc + 1) +
                            wgx * mu(ic + 1, jc + 1, kc + 1)));
          lambdagrid(i, j, k) =
              lambdagrid(i, j, k) +
              (1 - wgz) * ((1 - wgy) * ((1 - wgx) * lambda(ic, jc, kc) +
                                        wgx * lambda(ic + 1, jc, kc)) +
                           wgy * ((1 - wgx) * lambda(ic, jc + 1, kc) +
                                  wgx * lambda(ic + 1, jc + 1, kc))) +
              wgz * ((1 - wgy) * ((1 - wgx) * lambda(ic, jc, kc + 1) +
                                  wgx * lambda(ic + 1, jc, kc + 1)) +
                     wgy * ((1 - wgx) * lambda(ic, jc + 1, kc + 1) +
                            wgx * lambda(ic + 1, jc + 1, kc + 1)));
        }
      }
    }
  }
...
void gradients(int nx, int ny, int nz, float_sw4 xmin, float_sw4 ymin,
               ...
               float_sw4* __restrict__ a_gradrho,
               float_sw4* __restrict__ a_gradmu,
               float_sw4* __restrict__ a_gradlambda, int ib, int ie, int jb,
               int je, int kb, int ke, int ibact, int ieact, int jbact,
               int jeact, int kbact, int keact,
               float_sw4* __restrict__ a_gradrhogrid,
               float_sw4* __restrict__ a_gradmugrid,
...
#define gradmugrid(i, j, k) \
  a_gradmugrid[i - ib + nig * (j - jb) + nijg * (k - kb)]
#define gradlambdagrid(i, j, k) \
  a_gradlambdagrid[i - ib + nig * (j - jb) + nijg * (k - kb)]
...
#define gradlambdal(i, j, k, t) \
  _gradlambda[i + nx2 * (j) + nxy2 * (k) + nxyz2 * (t)]
  ...
  for (int k = kbact; k <= keact; k++) {
    ...
    for (int j = jbact; j <= jeact; j++) {
      ...
      for (int i = ibact; i <= ieact; i++) {
        ...
        //	      float_sw4 wgh3 =
        //((1-l3)*(1-wgz)+l3*wgz)*((1-l2)*(1-wgy)+l2*wgy)*((1-l1)*(1-wgx)+l1*wgx);
      ...
      for (int j = jbact; j <= jeact; j++) {
        ...
        for (int i = ibact; i <= ieact; i++) {
          ...
          if (ic < 0) {
            ...
            wgx = (x - xmin) / hx - ic;
          gradrho(ic, jc, kc) = gradrho(ic, jc, kc) + (1 - wgz) * (1 - wgy) *
                                                          (1 - wgx) *
                                                          gradrhogrid(i, j, k);
          gradrho(ic + 1, jc, kc) =
              gradrho(ic + 1, jc, kc) +
          ...
          gradrho(ic, jc + 1, kc) =
              gradrho(ic, jc + 1, kc) +
              (1 - wgz) * wgy * (1 - wgx) * gradrhogrid(i, j, k);
          gradrho(ic + 1, jc + 1, kc) =
              gradrho(ic + 1, jc + 1, kc) +
              (1 - wgz) * wgy * wgx * gradrhogrid(i, j, k);
          gradrho(ic, jc, kc + 1) =
              gradrho(ic, jc, kc + 1) +
              wgz * (1 - wgy) * (1 - wgx) * gradrhogrid(i, j, k);
          gradrho(ic + 1, jc, kc + 1) =
              gradrho(ic + 1, jc, kc + 1) +
              wgz * (1 - wgy) * wgx * gradrhogrid(i, j, k);
          gradrho(ic, jc + 1, kc + 1) =
              gradrho(ic, jc + 1, kc + 1) +
              wgz * wgy * (1 - wgx) * gradrhogrid(i, j, k);
          gradrho(ic + 1, jc + 1, kc + 1) =
              gradrho(ic + 1, jc + 1, kc + 1) +
              wgz * wgy * wgx * gradrhogrid(i, j, k);
          gradmu(ic, jc, kc) = gradmu(ic, jc, kc) + (1 - wgz) * (1 - wgy) *
                                                        (1 - wgx) *
                                                        gradmugrid(i, j, k);
          gradmu(ic + 1, jc, kc) =
              gradmu(ic + 1, jc, kc) +
          ...
          gradmu(ic, jc + 1, kc + 1) =
              gradmu(ic, jc + 1, kc + 1) +
              wgz * wgy * (1 - wgx) * gradmugrid(i, j, k);
          gradmu(ic + 1, jc + 1, kc + 1) =
              gradmu(ic + 1, jc + 1, kc + 1) +
              wgz * wgy * wgx * gradmugrid(i, j, k);
          gradlambda(ic, jc, kc) =
              gradlambda(ic, jc, kc) +
              (1 - wgz) * (1 - wgy) * (1 - wgx) * gradlambdagrid(i, j, k);
          gradlambda(ic + 1, jc, kc) =
              gradlambda(ic + 1, jc, kc) +
              (1 - wgz) * (1 - wgy) * wgx * gradlambdagrid(i, j, k);
          gradlambda(ic, jc + 1, kc) =
              gradlambda(ic, jc + 1, kc) +
              (1 - wgz) * wgy * (1 - wgx) * gradlambdagrid(i, j, k);
          gradlambda(ic + 1, jc + 1, kc) =
              gradlambda(ic + 1, jc + 1, kc) +
              (1 - wgz) * wgy * wgx * gradlambdagrid(i, j, k);
          gradlambda(ic, jc, kc + 1) =
              gradlambda(ic, jc, kc + 1) +
              wgz * (1 - wgy) * (1 - wgx) * gradlambdagrid(i, j, k);
          gradlambda(ic + 1, jc, kc + 1) =
              gradlambda(ic + 1, jc, kc + 1) +
              wgz * (1 - wgy) * wgx * gradlambdagrid(i, j, k);
          gradlambda(ic, jc + 1, kc + 1) =
              gradlambda(ic, jc + 1, kc + 1) +
              wgz * wgy * (1 - wgx) * gradlambdagrid(i, j, k);
          gradlambda(ic + 1, jc + 1, kc + 1) =
              gradlambda(ic + 1, jc + 1, kc + 1) +
              wgz * wgy * wgx * gradlambdagrid(i, j, k);
        }
      }
...
//-----------------------------------------------------------------------
void EW::interpolate(int nx, int ny, int nz, double xmin, double ymin,
                     double zmin, double hx, double hy, double hz, Sarray& rho,
                     Sarray& mu, Sarray& lambda, int grid, Sarray& rhogrid,
                     Sarray& mugrid, Sarray& lambdagrid, bool update)
//
// Computes the material on computational grid from a given material
// perturbation on a parameter grid.
//
// Computes: rhogrid := I(rho)+mRho[grid]  (similar for mu, lambda)
//           where I(rho) interpolates input perturbation `rho' onto the
//           computational grid.
//
...
//
//   Output: rhogrid, mugrid, lambdagrid - Material (perturbation+base material)
//   on computational grid: g=`grid'
//
{
  ...
  if (topographyExists() && grid >= mNumberOfCartesianGrids) {
    interpolatemtrlc(nx, ny, nz, xmin, ymin, zmin, hx, hy, hz, rhop, mup,
                     lambdap, ifirst, ilast, jfirst, jlast, kfirst, klast,
                     ifirstact, ilastact, jfirstact, jlastact, kfirstact,
                     klastact, rhogp, mugp, lambdagp, mGridSize[grid],
                     mZ[grid].c_ptr());
  } else {
    interpolatemtrl(nx, ny, nz, xmin, ymin, zmin, hx, hy, hz, rhop, mup,
                    lambdap, ifirst, ilast, jfirst, jlast, kfirst, klast,
                    ifirstact, ilastact, jfirstact, jlastact, kfirstact,
                    klastact, rhogp, mugp, lambdagp, mGridSize[grid],
                    m_zmin[grid]);
...
void EW::interpolate_to_coarse(int nx, int ny, int nz, double xmin, double ymin,
  ...
  //          xmin, ymin, zmin - Origin of parameter grid.
  //          hx, hy, hz       - Spacing of parameter grid.
  //          rhogrid, mugrid, lambdagrid - The material on the computational
  //          grids.
  //
  ...
  for (int k = rho.m_kb; k <= rho.m_ke; k++)
    for (int j = rho.m_jb; j <= rho.m_je; j++)
      for (int i = rho.m_ib; i <= rho.m_ie; i++) {
        ...
        if (m_iStart[g] <= ig && ig + 1 <= m_iEnd[g] && m_jStart[g] <= jg &&
            jg + 1 <= m_jEnd[g] && m_kStart[g] <= kg && kg + 1 <= m_kEnd[g]) {
          ...
          //               mugrid[g](ig,jg,kg)-mMu[g](ig,jg,kg); lambda(i,j,k) =
...
//-----------------------------------------------------------------------
void EW::interpolate_to_coarse_vel(int nx, int ny, int nz, double xmin,
  ...
  //          xmin, ymin, zmin - Origin of parameter grid.
  //          hx, hy, hz       - Spacing of parameter grid.
  //          rhogrid, mugrid, lambdagrid - The material (rho,mu,lambda) on the
  //          computational grids.
  //
  ...
  for (int g = 0; g < mNumberOfGrids; g++) done[g] = false;
            ...
            for (int k1 = rhogrid[g].m_kb; k1 <= rhogrid[g].m_ke; k1++)
              for (int j1 = rhogrid[g].m_jb; j1 <= rhogrid[g].m_je; j1++)
                for (int i1 = rhogrid[g].m_ib; i1 <= rhogrid[g].m_ie; i1++) {
                  ...
                  cpdiff[g](i1, j1, k1) =
                      sqrt((2 * mugrid[g](i1, j1, k1) +
                            lambdagrid[g](i1, j1, k1)) /
                           rhogrid[g](i1, j1, k1)) -
                      sqrt((2 * mMu[g](i1, j1, k1) + mLambda[g](i1, j1, k1)) /
                           mRho[g](i1, j1, k1));
                }
            done[g] = true;
          }
  ...
  Sarray tmp;
  tmp.copy(rho);
  MPI_Allreduce(tmp.c_ptr(), rho.c_ptr(), rho.npts(), MPI_DOUBLE, MPI_SUM,
                m_1d_communicator);
  tmp.copy(cs);
  MPI_Allreduce(tmp.c_ptr(), cs.c_ptr(), cs.npts(), MPI_DOUBLE, MPI_SUM,
                m_1d_communicator);
  tmp.copy(cp);
  MPI_Allreduce(tmp.c_ptr(), cp.c_ptr(), cp.npts(), MPI_DOUBLE, MPI_SUM,
                m_1d_communicator);
}

//-----------------------------------------------------------------------
void EW::interpolate_base_to_coarse(int nx, int ny, int nz, double xmin,
                                    double ymin, double zmin, double hx,
                                    double hy, double hz, Sarray& rho,
  ...
  //          hx, hy, hz       - Spacing of parameter grid.
  //
  //   Output: rho, mu, lambda  - Material velocity perturbation on parameter
  //   grid.
  //
  int ig, jg, kg, g;
  rho.set_to_zero();
  ...
  MPI_Allreduce(tmp.c_ptr(), rho.c_ptr(), rho.npts(), MPI_DOUBLE, MPI_SUM,
                m_1d_communicator);
  tmp.copy(mu);
  MPI_Allreduce(tmp.c_ptr(), mu.c_ptr(), mu.npts(), MPI_DOUBLE, MPI_SUM,
                m_1d_communicator);
  tmp.copy(lambda);
  MPI_Allreduce(tmp.c_ptr(), lambda.c_ptr(), lambda.npts(), MPI_DOUBLE, MPI_SUM,
                m_1d_communicator);
}

//-----------------------------------------------------------------------
void EW::interpolate_base_to_coarse_vel(int nx, int ny, int nz, double xmin,
                                        double ymin, double zmin, double hx,
                                        double hy, double hz, Sarray& rho,
                                        Sarray& cs, Sarray& cp) {
  // Compute material perturbation on parameter grid from the background
          ...
          rho(i, j, k) = ((1 - wghy) * (1 - wghz) *
                          ...
                          (wghy) * (wghz) *
                              ((1 - wghx) * mRho[g](ig, jg + 1, kg + 1) +
                               wghx * mRho[g](ig + 1, jg + 1, kg + 1)));
          cs(i, j, k) = (1 - wghy) * (1 - wghz) *
                            ((1 - wghx) * csgrid[g](ig, jg, kg) +
                             wghx * csgrid[g](ig + 1, jg, kg)) +
                        (wghy) * (1 - wghz) *
                            ((1 - wghx) * csgrid[g](ig, jg + 1, kg) +
                             wghx * csgrid[g](ig + 1, jg + 1, kg)) +
                        (1 - wghy) * (wghz) *
                            ((1 - wghx) * csgrid[g](ig, jg, kg + 1) +
                             wghx * csgrid[g](ig + 1, jg, kg + 1)) +
                        (wghy) * (wghz) *
                            ((1 - wghx) * csgrid[g](ig, jg + 1, kg + 1) +
                             wghx * csgrid[g](ig + 1, jg + 1, kg + 1));
          cp(i, j, k) = (1 - wghy) * (1 - wghz) *
                            ((1 - wghx) * cpgrid[g](ig, jg, kg) +
                             wghx * cpgrid[g](ig + 1, jg, kg)) +
                        (wghy) * (1 - wghz) *
                            ((1 - wghx) * cpgrid[g](ig, jg + 1, kg) +
                             wghx * cpgrid[g](ig + 1, jg + 1, kg)) +
                        (1 - wghy) * (wghz) *
                            ((1 - wghx) * cpgrid[g](ig, jg, kg + 1) +
                             wghx * cpgrid[g](ig + 1, jg, kg + 1)) +
                        (wghy) * (wghz) *
                            ((1 - wghx) * cpgrid[g](ig, jg + 1, kg + 1) +
                             wghx * cpgrid[g](ig + 1, jg + 1, kg + 1));
        }
      }
  Sarray tmp;
  tmp.copy(rho);
...
//-----------------------------------------------------------------------
void EW::interpolation_gradient(int nx, int ny, int nz, double xmin,
                                double ymin, double zmin, double hx, double hy,
                                double hz, Sarray& gradrho, Sarray& gradmu,
  ...
  int kfirstact = m_kStartAct[grid];
  int klastact = m_kEndAct[grid];
  if (ifirstact < m_iStartInt[grid]) ifirstact = m_iStartInt[grid];
  if (ilastact > m_iEndInt[grid]) ilastact = m_iEndInt[grid];
  if (jfirstact < m_jStartInt[grid]) jfirstact = m_jStartInt[grid];
  if (jlastact > m_jEndInt[grid]) jlastact = m_jEndInt[grid];
  ...
  if (topographyExists() && grid >= mNumberOfCartesianGrids) {
    gradientsc(nx, ny, nz, xmin, ymin, zmin, hx, hy, hz, grhop, gmup, glambdap,
               ifirst, ilast, jfirst, jlast, kfirst, klast, ifirstact, ilastact,
               jfirstact, jlastact, kfirstact, klastact, grhogp, gmugp,
               glambdagp, mGridSize[grid], mZ[grid].c_ptr());
  } else {
    gradients(nx, ny, nz, xmin, ymin, zmin, hx, hy, hz, grhop, gmup, glambdap,
              ifirst, ilast, jfirst, jlast, kfirst, klast, ifirstact, ilastact,
              jfirstact, jlastact, kfirstact, klastact, grhogp, gmugp,
              glambdagp, mGridSize[grid], m_zmin[grid]);
  }
...
void EW::transform_gradient(Sarray& rho, Sarray& mu, Sarray& lambda,
  ...
  // the
  //  grid points.
  // This routine returns the gradient with respect to (rho,cs,cp) at the grid
  // points. NOTE: input (grho,gmu,glambda) are overwritten with the transformed
  // gradient.
  ...
  for (size_t ind = 0; ind < rho.m_npts; ind++) {
    ...
    grhop[ind] = cs * cs * gmup[ind] + (cp * cp - 2 * cs * cs) * glambdap[ind] +
                 grhop[ind];
    gmup[ind] =
        2 * rhop[ind] * cs * gmup[ind] - 4 * rhop[ind] * cs * glambdap[ind];
