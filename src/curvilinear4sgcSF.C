...
#define ghcof_no_gp(i) a_ghcof_no_gp[i - 1]
  ...
  //#pragma omp parallel
  {
    ...
    if (onesided[4] == 1) {
      ...
      // BEST 16 4 6
#ifdef ENABLE_HIP
      // std::cout<<"IRANNGE "<<ilast-1-ifirst-2<<" JRANGE
      // "<<jlast-1-jfirst-2<<"\n";
      ...
      RAJA::kernel<CURV_POL>(
          ...
          [=] RAJA_DEVICE(
            ...
            // ijac*=strx(i) * stry(j);
            {
              ...
              float_sw4 tmp[8][6];
          // Cuda best with no pragmas. Hip #pragma unroll 8 on both is fastest
          // August 18 2021
#ifdef ENABLE_HIP
#pragma unroll 8
#endif
                ...
                tmp[mm][2] =
                    ...
                    ((2 * mu(i, j, m) + la(i, j, m)) * met(4, i, j, m) *
                     ...
                     mu(i, j, m) * (met(2, i, j, m) * strx(i) *
                                    ...
                                    met(3, i, j, m) * stry(j) *
                                        met(3, i, j, m) * stry(j)));
                tmp[mm][3] = (mu(i, j, m) + la(i, j, m)) * met(2, i, j, m) *
                             met(3, i, j, m);
                tmp[mm][4] = (mu(i, j, m) + la(i, j, m)) * met(2, i, j, m) *
                             met(4, i, j, m);
                tmp[mm][5] = (mu(i, j, m) + la(i, j, m)) * met(3, i, j, m) *
                             met(4, i, j, m);
              }
#ifdef ENABLE_HIP
#pragma unroll 8  // HIP SA 62
              ...
              // divergence goes from 7.73ms to 23ms
                ...
                r1 += istrxy * mucofu2 * u(1, i, j, q) +
                      mucofuv * u(2, i, j, q) + istry * mucofuw * u(3, i, j, q);
                r2 += mucofuv * u(1, i, j, q) +
                      istrxy * mucofv2 * u(2, i, j, q) +
              ...
              float_sw4 mucofw2 =
                  ...
                  ((2 * mu(i, j, 1) + la(i, j, 1)) * met(4, i, j, 1) *
                   ...
                   mu(i, j, 1) *
                       (met(2, i, j, 1) * strx(i) * met(2, i, j, 1) * strx(i) +
                        met(3, i, j, 1) * stry(j) * met(3, i, j, 1) * stry(j)));
              float_sw4 mucofuv = ghcof(k) * (mu(i, j, 1) + la(i, j, 1)) *
                                  met(2, i, j, 1) * met(3, i, j, 1);
              float_sw4 mucofuw = ghcof(k) * (mu(i, j, 1) + la(i, j, 1)) *
                                  met(2, i, j, 1) * met(4, i, j, 1);
              float_sw4 mucofvw = ghcof(k) * (mu(i, j, 1) + la(i, j, 1)) *
          ...
          [=] RAJA_DEVICE(Tclass<11> t, double* sma, sw4_type i, sw4_type j,
            ...
            // float_sw4 istrxy = istry * istrx;
            {
              ...
              // pq-derivatives (u-eq)
              // 38 ops., tot=4049
              r1 += c2 * (mu(i, j + 2, k) * met(1, i, j + 2, k) *
                              met(1, i, j + 2, k) *
                              (c2 * (u(2, i + 2, j + 2, k) -
                                     u(2, i - 2, j + 2, k)) +
                               c1 * (u(2, i + 1, j + 2, k) -
              ...
              // qp-derivatives (u-eq)
              // 38 ops. tot=4087
              r1 += c2 * (la(i + 2, j, k) * met(1, i + 2, j, k) *
                              met(1, i + 2, j, k) *
                              (c2 * (u(2, i + 2, j + 2, k) -
                                     u(2, i + 2, j - 2, k)) +
                               c1 * (u(2, i + 2, j + 1, k) -
              ...
              // pq-derivatives (v-eq)
              // 38 ops. , tot=4125
              r2 += c2 * (la(i, j + 2, k) * met(1, i, j + 2, k) *
                              met(1, i, j + 2, k) *
                              (c2 * (u(1, i + 2, j + 2, k) -
                                     u(1, i - 2, j + 2, k)) +
                               c1 * (u(1, i + 1, j + 2, k) -
              ...
              //* qp-derivatives (v-eq)
              // 38 ops., tot=4163
              r2 += c2 * (mu(i + 2, j, k) * met(1, i + 2, j, k) *
                              met(1, i + 2, j, k) *
                              (c2 * (u(1, i + 2, j + 2, k) -
                                     u(1, i + 2, j - 2, k)) +
                               c1 * (u(1, i + 2, j + 1, k) -
          ...
          float_sw4 mucofw2 =
              ...
              ((2 * mu(i, j, nk) + la(i, j, nk)) * met(4, i, j, nk) *
               ...
               mu(i, j, nk) *
                   (met(2, i, j, nk) * strx(i) * met(2, i, j, nk) * strx(i) +
                    met(3, i, j, nk) * stry(j) * met(3, i, j, nk) * stry(j)));
          float_sw4 mucofuv = ghcof_no_gp(nk - k + 1) *
                              (mu(i, j, nk) + la(i, j, nk)) * met(2, i, j, nk) *
                              met(3, i, j, nk);
          float_sw4 mucofuw = ghcof_no_gp(nk - k + 1) *
                              (mu(i, j, nk) + la(i, j, nk)) * met(2, i, j, nk) *
                              met(4, i, j, nk);
          float_sw4 mucofvw = ghcof_no_gp(nk - k + 1) *
                              (mu(i, j, nk) + la(i, j, nk)) * met(3, i, j, nk) *
                              met(4, i, j, nk);
