...
void curvilinear4sg_ci(
    ...
    float_sw4* __restrict__ a_ghcof, float_sw4* __restrict__ a_acof_no_gp,float_sw4* __restrict__ a_ghcof_no_gp, 
...
#define ghcof_no_gp(i) a_ghcof_no_gp[i - 1]
  ...
  //#pragma omp parallel
  {
    ...
    if (onesided[4] == 1) {
      ...
      // BEST 16 4 6
#ifdef ENABLE_HIP
      //std::cout<<"IRANNGE "<<ilast-1-ifirst-2<<" JRANGE "<<jlast-1-jfirst-2<<"\n";
...
#pragma forceinline
      forall3asyncSF<__LINE__>(
			     tag1, I, J, K, [=] RAJA_DEVICE(Tclass<11> t, double *sma, int i, int j,int k) { // LAMBDA 1 -28 registers StandAlone 80 HIP STANDALOIE 114
	...
	sma[0] = r1;
        ...
        // 43 ops, tot=144
	{
        float_sw4 cof1 = (mu(i - 2, j, k)) * met(1, i - 2, j, k) * met(1, i - 2, j, k) *
               strx(i - 2);
        float_sw4 cof2 = (mu(i - 1, j, k)) * met(1, i - 1, j, k) * met(1, i - 1, j, k) *
               strx(i - 1);
        float_sw4 cof3 = (mu(i, j, k)) * met(1, i, j, k) * met(1, i, j, k) * strx(i);
        float_sw4 cof4 = (mu(i + 1, j, k)) * met(1, i + 1, j, k) * met(1, i + 1, j, k) *
               strx(i + 1);
        float_sw4 cof5 = (mu(i + 2, j, k)) * met(1, i + 2, j, k) * met(1, i + 2, j, k) *
               strx(i + 2);
	...
	sma[2] = r3;
		    ...
		    },[=] RAJA_DEVICE(Tclass<11> t, double *sma, int i, int j, int k) {  // LAMBDA 2 28 regsiters in CUDA Standalon 166, 72 with #pragma unroll 1 on both loops 
            ...
            // ijac*=strx(i) * stry(j);
		      ...
		      r2 = sma[1];
		      r3 = sma[2];
		      //static const double acoff[6][8][8]={384*1.0};
	...
	float_sw4 tmp[8][6];
	// Cuda best with no pragmas. Hip #pragma unroll 8 on both is fastest August 18 2021
#ifdef ENABLE_HIP
#pragma unroll 8 
#endif
	for (int m = 1; m <= 8; m++) { // without this loop the funcion takes 6.8s
	  // mu and lambda seem to vary from cycle to cycle. So mucofs cannot be computed and stored
	    //if (acof(k,q,m)!=0.0){ // Massive increase runtime due to divergence goes from 7.73ms to 23ms
	  int mm=m-1;
            tmp[mm][0] = 
            ...
            tmp[mm][3]= (mu(i, j, m) + la(i, j, m)) *
                       met(2, i, j, m) * met(3, i, j, m);
            tmp[mm][4]= (mu(i, j, m) + la(i, j, m)) *
                       met(2, i, j, m) * met(4, i, j, m);
            tmp[mm][5]= (mu(i, j, m) + la(i, j, m)) *
...
#pragma unroll 8  //HIP SA 62
	  ...
	  //#pragma unroll 8 // HIP SA 176
	    ...
	    int mm=m-1;
            ...
            mucofuv += acof(k, q, m) *tmp[mm][3];
            mucofuw += acof(k, q, m) *tmp[mm][4];
            mucofvw += acof(k, q, m) *tmp[mm][5];
#ifdef SW4_USE_CMEM
                }
#endif
	...
	sma[2] = r3;
		    ...
		    },[=] RAJA_DEVICE(Tclass<11> t, double *sma, int i, int j,int k) {  // LAMBDA 3 0 regs standalon 40 HIPSA 39
		      //float_sw4 ijac = strx(i) * stry(j) / jac(i, j, k);
            // float_sw4 ijac = 1 / jac(i, j, k);
            //float_sw4 istry = 1 / (stry(j));
            //float_sw4 istrx = 1 / (strx(i));
        ...
        // 72 ops., tot=4011
		      ...
		      float_sw4   mucofu2 = ghcof(k) *
                  ((2 * mu(i, j, 1) + la(i, j, 1)) * met(2, i, j, 1) * strx(i) *
                       met(2, i, j, 1) * strx(i) +
                   mu(i, j, 1) *
                       (met(3, i, j, 1) * stry(j) * met(3, i, j, 1) * stry(j) +
                        met(4, i, j, 1) * met(4, i, j, 1)));
		      float_sw4 mucofv2 = ghcof(k) *
                  ((2 * mu(i, j, 1) + la(i, j, 1)) * met(3, i, j, 1) * stry(j) *
                       met(3, i, j, 1) * stry(j) +
                   mu(i, j, 1) *
                       (met(2, i, j, 1) * strx(i) * met(2, i, j, 1) * strx(i) +
                        met(4, i, j, 1) * met(4, i, j, 1)));
		      float_sw4 mucofw2 = ghcof(k) *
                  ((2 * mu(i, j, 1) + la(i, j, 1)) * met(4, i, j, 1) *
                       met(4, i, j, 1) +
                   mu(i, j, 1) *
                       (met(2, i, j, 1) * strx(i) * met(2, i, j, 1) * strx(i) +
                        met(3, i, j, 1) * stry(j) * met(3, i, j, 1) * stry(j)));
        float_sw4 mucofuv = ghcof(k) * (mu(i, j, 1) + la(i, j, 1)) * met(2, i, j, 1) *
                  met(3, i, j, 1);
        float_sw4 mucofuw = ghcof(k) * (mu(i, j, 1) + la(i, j, 1)) * met(2, i, j, 1) *
                  met(4, i, j, 1);
        float_sw4 mucofvw = ghcof(k) * (mu(i, j, 1) + la(i, j, 1)) * met(3, i, j, 1) *
                  met(4, i, j, 1);
	...
	sma[2] = r3;
		    ...
		    },[=] RAJA_DEVICE(Tclass<11> t, double *sma, int i, int j,int k) {  // LAMBDA 4 2 regs standalone 68 HIP SA 86
	...
	//sma[2] = r3;
		    ...
		    },[=] RAJA_DEVICE(Tclass<11> t, double *sma, int i, int j,int k ) {  // LAMBDA 5 2 regs standalone 80 HIPSA 86
	...
	sma[2] = r3;
		    ...
		    },[=] RAJA_DEVICE(Tclass<11> t, double *sma, int i, int j,int k) {  // LAMBDA 6 99 HIPSA 87
        // rq - derivatives
	...
	},[=] RAJA_DEVICE(Tclass<11> t, double *sma, int i, int j,int k) {  // LAMBDA 7 HIP SA 78
