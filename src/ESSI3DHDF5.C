...
ESSI3DHDF5::ESSI3DHDF5(const std::string& filename, int (&global)[3],
  ...
  m_global_dims[0] = H5S_UNLIMITED;  // Extendible dimension for cycle
...
void ESSI3DHDF5::write_header(double h, double (&lonlat_origin)[2], double az,
  ...
  hid_t dataset_id =
      H5Dcreate2(m_file_id, "Grid lon-lat origin", H5T_IEEE_F64LE, dataspace_id,
  ...
  ierr =
      H5Dwrite(dataset_id, H5T_IEEE_F64LE, H5S_ALL, H5S_ALL, m_mpiprop_id, &az);
  ierr = H5Dclose(dataset_id);
  ierr = H5Sclose(dataspace_id);
  ...
  dataset_id = H5Dcreate2(m_file_id, "ESSI xyz grid spacing", H5T_IEEE_F64LE,
                          dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ierr =
      H5Dwrite(dataset_id, H5T_IEEE_F64LE, H5S_ALL, H5S_ALL, m_mpiprop_id, &h);
  ierr = H5Dclose(dataset_id);
  ierr = H5Sclose(dataspace_id);
  ...
  dataset_id = H5Dcreate2(m_file_id, "ESSI xyz origin", H5T_IEEE_F64LE,
                          dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ierr = H5Dwrite(dataset_id, H5T_IEEE_F64LE, H5S_ALL, H5S_ALL, m_mpiprop_id,
                  &origin);
  ierr = H5Dclose(dataset_id);
  ierr = H5Sclose(dataspace_id);
  ...
  dim = 1;
  dataspace_id = H5Screate_simple(1, &dim, NULL);
  dataset_id = H5Dcreate2(m_file_id, "time start", H5T_IEEE_F64LE, dataspace_id,
                          H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ierr =
      H5Dwrite(dataset_id, H5T_IEEE_F64LE, H5S_ALL, H5S_ALL, m_mpiprop_id, &t);
  ierr = H5Dclose(dataset_id);
  ierr = H5Sclose(dataspace_id);
  ...
  dim = 1;
  dataspace_id = H5Screate_simple(1, &dim, NULL);
  dataset_id = H5Dcreate2(m_file_id, "timestep", H5T_IEEE_F64LE, dataspace_id,
                          H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ierr =
      H5Dwrite(dataset_id, H5T_IEEE_F64LE, H5S_ALL, H5S_ALL, m_mpiprop_id, &dt);
  ierr = H5Dclose(dataset_id);
  ierr = H5Sclose(dataspace_id);
...
void ESSI3DHDF5::write_topo(double* window_array) {
  ...
  */
  ...
  hid_t dataset_id =
      H5Dcreate2(m_file_id, "z coordinates", H5T_IEEE_F64LE, dataspace_id,
...
void ESSI3DHDF5::init_write_vel(int ntimestep) {
  ...
  for (int c = 0; c < num_vel; c++) {
    ...
    m_vel_dataset_id[c] =
        H5Dcreate2(m_file_id, var, H5T_IEEE_F64LE, m_vel_dataspace_id[c],
                   H5P_DEFAULT, prop_id, H5P_DEFAULT);
  }
  H5Pclose(prop_id);
  if (debug) cout << "Rank" << myRank << "Done creating vel" << endl;
#endif
}
...
void ESSI3DHDF5::write_vel(double* window_array, int comp, int cycle) {
  ...
  */
  ...
  if (debug) cout << "Rank" << myRank << "Done writing vel" << endl;
  // H5Sclose(slice_id);
