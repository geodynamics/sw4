...
/*< output-checking allocation >*/
{
  ...
  if (0 >= size)
    fprintf(stderr, "%s: illegal allocation (%ld bytes)", __FILE__, size);
...
/* find new traveltime at gridpoint i */
{
  ...
  for (j = 0; j < 3; j++) {
    ...
    if (order > 1) {
      if (a < b && ix - 2 >= 0) {
        k = i - 2 * s[j];
        if (in[k] != SF_OUT && a >= (t = ttime[k])) stencil(t, xj);
      }
      if (a > b && ix + 2 <= n[j] - 1) {
        k = i + 2 * s[j];
        if (in[k] != SF_OUT && b >= (t = ttime[k])) stencil(t, xj);
      }
    }
  }
  ...
  if (v[2]->value < SF_HUGE) { /* ALL THREE DIRECTIONS CONTRIBUTE */
    if (updaten(3, &res, v) || updaten(2, &res, v) || updaten(1, &res, v))
      return res;

  } else if (v[1]->value < SF_HUGE) { /* TWO DIRECTIONS CONTRIBUTE */
    if (updaten(2, &res, v) || updaten(1, &res, v)) return res;
...
/* find new traveltime at gridpoint i */
{
  ...
  if (v[2]->value > 0) { /* ALL THREE DIRECTIONS CONTRIBUTE */
    if (updaten2(3, &res, v) || updaten2(2, &res, v) || updaten2(1, &res, v))
      return res;
  } else if (v[1]->value > 0) { /* TWO DIRECTIONS CONTRIBUTE */
    if (updaten2(2, &res, v) || updaten2(1, &res, v)) return res;
...
/*< initialize distance computation >*/
{
  ...
  for (ip = 0; ip < np; ip++) {
    ...
    if (x[0] < 0. || ix[0] >= n[0] || x[1] < 0. || ix[1] >= n[1] || x[2] < 0. ||
        ix[2] >= n[2])
      continue;
    k = 0;
    for (j = 0; j < 3; j++) {
    ...
    if (ix[0] != n[0] - 1) {
      ...
      if (ix[1] != n[1] - 1) {
        ...
        if (ix[2] != n[2] - 1)
          n123 -= dist(k + s[0] + s[1] + s[2], d[0] - x[0], d[1] - x[1],
                       d[2] - x[2]);
      }
      if (ix[2] != n[2] - 1)
...
/*< initialize the source >*/
{
  ...
  ic = (start[0] + endx[0]) / 2 +
       n[0] * ((start[1] + endx[1]) / 2 + n[1] * (start[2] + endx[2]) / 2);
...
/*< Run fast marching eikonal solver >*/
{
  ...
  for (npoints = sf_neighbors_nearsource(xs, b, d, v, plane); npoints > 0;
