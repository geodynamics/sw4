...
//	 simulation.solve( src, GlobalTimeSeries[e], mu, lambda, rho, U, Um, upred_saved, ucorr_saved, false, e );
      ...
      simulation.solve_mopt( GlobalSources[e], GlobalTimeSeries[e], mu, lambda, rho, U, Um, upred_saved, ucorr_saved, true, e, mopt->m_nsteps_in_memory, 0, ph );
   ...
   else
   {
	 ...
	 for( int jper = activeg[2] ; jper <= activeg[3] ; jper++ )
	    for( int iper = activeg[0] ; iper <= activeg[1] ; iper++ )
		  ...
		  pio->write_array( &fid, 3, xmi, offset, (char*)"double");
...
// Output source initial guess
	   if( myRank == 0 && nspar > 0 )
	   {
	      cout << "Initial source guess : \n";
              const char* names[11] = {" x0 = "," y0 = ", " z0 = "," mxx = ", " mxy = ", " mxz = ",
			   " myy = ", " myz = ", " mzz = ", " t0 = ", " freq = " };
