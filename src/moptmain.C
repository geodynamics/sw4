...
void gradient_test(EW& simulation, vector<vector<Source*> >& GlobalSources,
  ...
  if (par_grad) {
    ...
    std::vector<Sarray> rho0(1), mu0(1), lambda0(1), rho1(1), mu1(1),
        lambda1(1);
    ...
    for (size_t indg = 0; indg < npar_global; indg++) {
      ...
      if (varcase > 1) getcscp(rho1[0], mu1[0], lambda1[0], cs1[0], cp1[0]);
      double dfnum;
      if (varcase == 1) {
        if (var == 0)
      ...
      //      for( int jg=32 ; jg <= 95 ; jg++ )
      {
        ...
        if (ind >= 0 && myRank < procevent) {
          cout << myRank << " " << setw(12) << h << " " << setw(12) << dfan
               ...
               << fabs((dfan - dfnum) / dfnorm) << endl;
          dftest << indg << " " << fp << " " << h << " " << dfan << " " << dfnum
                 << " " << dfan - dfnum << endl;
        }
        if (ind >= 0) xm[ind] = x0;
        if (indg > 0 && (indg % 100 == 0) &&
            (myRank == 0 && myRank < procevent))
...
#pragma omp parallel
        ...
        if (mopt->m_opttest == 2) {
          ...
          CHECK_INPUT(mopt->m_mpcart0 != NULL,
                      "ERROR, there is no Cartesian material parameterization "
                      "defined\n");
          mopt->m_mpcart0->project_and_write(simulation.mRho, simulation.mMu,
                                             simulation.mLambda,
                                             "projmtrl.mpc");
        } else if (mopt->m_opttest == 8) {
          // Material parameterization test. Compute material from parameters
          // and output images
          mp->get_parameters(nmpard, xm, nmpars, &xs[nspar], simulation.mRho,
                             simulation.mMu, simulation.mLambda);
          int ng = simulation.mNumberOfGrids;
