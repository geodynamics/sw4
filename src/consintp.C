...
#pragma omp simd
  ...
  Sarray UcNew(3, m_iStart[gc], m_iEnd[gc], m_jStart[gc], m_jEnd[gc], 0,
               0);  // only one k-index
  Sarray UfNew(3, m_iStart[gf], m_iEnd[gf], m_jStart[gf], m_jEnd[gf], nkf + 1,
               nkf + 1);  // the k-index is arbitrary,
                          // Start iteration
  while (jacerr > m_citol && it < m_cimaxiter) {
    ...
    oddIoddJinterpJacobiOpt(
        ...
        m_kEnd.data(), m_iStartInt.data(), m_iEndInt.data(), m_jStartInt.data(),
        m_jEndInt.data(), gf, gc, nkf, mDt, hf, hc, cof, relax, m_sbop,
        m_ghcof);
    // FTNC      else
    // FTNC	oddIoddJinterpJacobi(rmax, Uf, UfNew, Uc, UcNew,
    // FTNC			     m_Mufs[gf], m_Mlfs[gf], m_Morc[gc],
    // m_Mlrc[gc], FTNC			     m_Mucs[gc], m_Mlcs[gc], m_Morf[gf],
    // m_Mlrf[gf], FTNC			     Unextf, BfRestrict, Unextc, Bc,
    // FTNC			     m_iStart.data(), m_iEnd.data(), m_jStart.data(),
    // m_jEnd.data(), m_kStart.data(), m_kEnd.data(), FTNC
    // m_iStartInt.data(), m_iEndInt.data(), m_jStartInt.data(),
    // m_jEndInt.data(), FTNC			     gf, gc, nkf, mDt, hf, hc,
    // cof, relax, FTNC			     m_sbop, m_ghcof);
...
#pragma omp parallel for
  ...
  if (usingSupergrid()) {
    ...
    twfrsurfzsg_wind_ci(m_iStart[gc], m_iEnd[gc], m_jStart[gc], m_jEnd[gc],
                        ...
                        i2, j1, j2);
    // FTNC      else
    // FTNC	 twfrsurfzsg_wind( m_iStart[gc], m_iEnd[gc], m_jStart[gc],
    // m_jEnd[gc], m_kStart[gc], m_kEnd[gc],
    ...
    if (m_use_attenuation)  // only 1 mechanism with twilight forcing
    {
      ...
      twfrsurfzsg_att_wind_ci(m_iStart[gc], m_iEnd[gc], m_jStart[gc],
                              ...
                              time, om, cv, ph, omstrx, omstry, b_ptr, mua_ptr,
                              laa_ptr, m_zmin[gc], i1, i2, j1, j2);
      // FTNC	 else
      // FTNC	    twfrsurfzsg_att_wind( m_iStart[gc], m_iEnd[gc],
      // m_jStart[gc], m_jEnd[gc], m_kStart[gc], m_kEnd[gc], FTNC
      // hc, kic, time, om, cv, ph, omstrx, omstry, FTNC
    ...
    // FTNC      else
    // FTNC	 twfrsurfz_wind( &m_iStart[gc], &m_iEnd[gc], &m_jStart[gc],
    // &m_jEnd[gc], &m_kStart[gc], &m_kEnd[gc],
    ...
    {
      // FTNC	 if( m_croutines )
      twfrsurfz_att_wind_ci(m_iStart[gc], m_iEnd[gc], m_jStart[gc], m_jEnd[gc],
                            m_kStart[gc], m_kEnd[gc], hc, kic, time, om, cv, ph,
                            b_ptr, mua_ptr, laa_ptr, m_zmin[gc], i1, i2, j1,
                            j2);
      // FTNC	 else
      // FTNC	    twfrsurfz_att_wind( m_iStart[gc], m_iEnd[gc], m_jStart[gc],
      // m_jEnd[gc], m_kStart[gc], m_kEnd[gc],
      // FTNC				hc, kic, time, om, cv, ph,
      // FTNC				b_ptr, mua_ptr, laa_ptr, m_zmin[gc], i1,
      // i2, j1, j2 );
    }
  }
  ...
  for (int jc = jcb; jc <= jce; jc++)
    for (int ic = icb; ic <= ice; ic++) {
      ...
      a11 = 0.25 * Mufs(i, j, nkf) * m_sbop[0] *
      ...
      a11 =
          0.25 * Mlfs(i, j, nkf) * m_sbop[0] * ihf;  // Mlfs contains stretching
