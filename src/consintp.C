...
void EW::conssw4_typep(Sarray &Uf, Sarray &Unextf, Sarray &Bf, Sarray &Muf,
  ...
  const sw4_type miStartgc = m_iStart[gc];
  const sw4_type mjStartgc = m_jStart[gc];
#define strc_x(i) a_strc_x[(i - miStartgc)]
#define strc_y(j) a_strc_y[(j - mjStartgc)]

  // stretching on the fine side
  const sw4_type miStartgf = m_iStart[gf];
  const sw4_type mjStartgf = m_jStart[gf];
...
#define strf_y(j) a_strf_y[(j - mjStartgf)]
  ...
  SW4_MARK_BEGIN("CONSSW4_TYPEP_LOOP1");
  RAJA::kernel<CONSSW4_TYPEP_EXEC_POL1>(
...
#pragma unroll
  ...
  SYNC_STREAM;
  SW4_MARK_END("CONSSW4_TYPEP_LOOP1");
  SView &BcV = Bc.getview();
  Bc.prefetch();
  RAJA::RangeSegment jc_range(m_jStart[gc], m_jEnd[gc] + 1);
  RAJA::RangeSegment ic_range(m_iStart[gc], m_iEnd[gc] + 1);
  SW4_MARK_BEGIN("CONSSW4_TYPEP_LOOP2");
  RAJA::kernel<CONSSW4_TYPEP_EXEC_POL1>(
      RAJA::make_tuple(jc_range, ic_range), [=] RAJA_DEVICE(sw4_type jc, sw4_type ic) {
...
#pragma unroll
  ...
  SW4_MARK_END("CONSSW4_TYPEP_LOOP2");
  ...
  SW4_MARK_BEGIN("CONSSW4_TYPEP_LOOP3");
  ...
  // for (sw4_type c=1; c<=3; c++)
  {
    ...
    RAJA::kernel<CONSSW4_TYPEP_EXEC_POL1>(
        ...
        [=] RAJA_DEVICE(sw4_type j, sw4_type i) {
          // #pragma omp for
          //       for( sw4_type j=jfeven; j <= jfe ; j+=2 ) // odd-i, even-j
          // #pragma omp simd
          //          for( sw4_type i=ifodd ; i <= ife ; i+=2 )
          //          {
          sw4_type ic = (i + 1) / 2;
          sw4_type jc = j / 2;
          for (sw4_type c = 1; c <= 3; c++)
    ...
    RAJA::kernel<CONSSW4_TYPEP_EXEC_POL1>(
        ...
        [=] RAJA_DEVICE(sw4_type j, sw4_type i) {
          // #pragma omp for
          //       for( sw4_type j=jfodd; j <= jfe ; j+=2 ) // odd-j
          // #pragma omp simd
          //          for( sw4_type i=ifeven ; i <= ife ; i+=2 ) // even-i
          //          {
          sw4_type ic = i / 2;
          sw4_type jc = (j + 1) / 2;
          for (sw4_type c = 1; c <= 3; c++)
    ...
    RAJA::kernel<CONSSW4_TYPEP_EXEC_POL1>(
        ...
        [=] RAJA_DEVICE(sw4_type j, sw4_type i) {
          // #pragma omp for
          //       for( sw4_type j=jfeven; j <= jfe ; j+=2 ) // even-j
          // #pragma omp simd
          //          for( sw4_type i=ifeven ; i <= ife ; i+=2 ) // even-i
          //          {
          sw4_type ic = i / 2;
  ...
  // SYNC_STREAM;
  SW4_MARK_END("CONSSW4_TYPEP_LOOP4");
  // Allocate space for the updated values of Uf and Uc (ghost points only)
  SW4_MARK_BEGIN("CONSSW4_TYPEP_SARRAY_ALLOCATION");
  // Sarray
  ...
  SW4_MARK_END("CONSSW4_TYPEP_SARRAY_ALLOCATION");
  // Start iteration
  SW4_MARK_BEGIN("CONSSW4_TYPEP ITERATION");
  while (jacerr > m_citol && it < m_cimaxiter) {
    ...
    if (m_croutines)  // tmp
      ...
      oddIoddJinterpJacobi(rmax, Uf, UfNew, Uc, UcNew, m_Mufs[gf], m_Mlfs[gf],
                           ...
                           m_iStartSw4_Type.data(), m_iEndSw4_Type.data(),
                           m_jStartSw4_Type.data(), m_jEndSw4_Type.data(), gf, gc, nkf,
    ...
    SW4_MARK_BEGIN("CONSSW4_TYPEP::MPI_ALLREDUCE");
#if defined(SW4_TRACK_MPI)
    {
      std::chrono::high_resolution_clock::time_point t1, t2;
      t1 = SW4_CHRONO_NOW;
...
#endif
    SW4_MARK_END("CONSSW4_TYPEP::MPI_ALLREDUCE");
  ...
  SW4_MARK_END("CONSSW4_TYPEP ITERATION");
...
//-----------------------------------------------------------------------
void EW::checksw4_typep(Sarray &Uf, Sarray &Unextf, Sarray &Bf, Sarray &Muf,
                   Sarray &Lambdaf, Sarray &Rhof, float_sw4 hf, Sarray &Uc,
                   Sarray &Unextc, Sarray &Bc, Sarray &Muc, Sarray &Lambdac,
                   Sarray &Rhoc, float_sw4 hc, float_sw4 cof, sw4_type gc, sw4_type gf,
