...
//						   int nms, double* xms )
//{
//   VERIFY2( nmd== m_nmd && nms == m_nms, "MP::constrain_material: Error in
//   sizes"); int ng = m_ew->mNumberOfGrids; vector<Sarray> rho, mu, lambda;
//   rho.resize(ng);
//   mu.resize(ng);
//   lambda.resize(ng);
//   for( int g=0; g < ng ; g++ )
...
void MaterialParameterization::write_parameters(const char* filename, int nms,
  ...
  if (m_myrank == 0 && nms > 0) {
    ...
    if (write_ascii) {
      ...
      for (int i = 0; i < nms; i++) fut << i + 1 << " " << xms[i] << endl;
      fut.close();
    }
  }
}
...
//      //      VERIFY2( fd != -1, "Error opening file " << m_filename << " in
//      MaterialParameterization::read_parameters"<<endl); VERIFY2( fd != -1,
//      "Error opening file " << fname << " in
//      MaterialParameterization::read_parameters"<<endl); int nms_read; size_t
//      nr = read(fd,&nms_read,sizeof(int)); if( nms_read == nms && nr ==
//      sizeof(int) && nms_read == m_nms )
//      {
...
//      {
//	 errflag = 3;
//      std::cout << "nms_read = " << nms_read << " m_nms= " << m_nms << " nms=
//      " << nms << std::endl;
//      }
//      else
//	 errflag = 1;
...
//   VERIFY2( errflag == 0, "Error no " << errflag << " in
//   MaterialParameterization::read_parameters"<<endl);
//}
...
//-----------------------------------------------------------------------
void MaterialParameterization::get_regularizer(
