...
#define ghcof(i) a_ghcof[i - 1]
  ...
  float_sw4 lap2mu, u3zip2, u3zip1, u3zim1, u3zim2, lau3zx, mu3xz, u3zjp2,
      u3zjp1, u3zjm1, u3zjm2;
  ...
  if (op == '=') {
    a1 = 0;
    for (size_t i = 0; i < static_cast<size_t>((ilast - ifirst + 1)) *
                               (jlast - jfirst + 1) * (klast - kfirst + 1) * 3;
         i++)
      a_lu[i] = 0;
  } else if (op == '+')
    a1 = 1;
...
#pragma omp parallel private(                                              \
    k, i, j, mux1, mux2, mux3, mux4, muy1, muy2, muy3, muy4, r1, r2, r3,   \
    mucof, mu1zz, mu2zz, mu3zz, lap2mu, q, u3zip2, u3zip1, u3zim1, u3zim2, \
    lau3zx, mu3xz, u3zjp2, u3zjp1, u3zjm1, u3zjm2, lau3zy, mu3yz, mu1zx,   \
    u1zip2, u1zip1, u1zim1, u1zim2, u2zjp2, u2zjp1, u2zjm1, u2zjm2, mu2zy, \
    lau1xz, lau2yz, kb, qb, mb, muz1, muz2, muz3, muz4)
  {
#pragma omp for
    for (k = k1; k <= k2; k++)
...
#pragma ivdep
  ...
  for (size_t i = 0; i < npts; i++) {
    ...
    up[i + npts] =
        2 * u[i + npts] - um[i + npts] + dt2orh * (lu[i + npts] + fo[i + npts]);
...
void ve_bndry_stress_curvi_ci(
    ...
    float_sw4* __restrict__ a_lave, float_sw4* __restrict__ a_bforcerhs,
    float_sw4* __restrict__ a_met, int side, float_sw4 sbop[6], int usesg,
    float_sw4* __restrict__ a_strx, float_sw4* __restrict__ a_stry) {
#define alphap(c, i, j, k) \
  a_alphap[base3 + i + ni * (j) + nij * (k) + nijk * (c)]
#define muve(i, j, k) a_muve[base + i + ni * (j) + nij * (k)]
...
void att_free_curvi_ci(int ifirst, int ilast, int jfirst, int jlast, int kfirst,
                       int klast, float_sw4* __restrict__ a_u,
                       float_sw4* __restrict__ a_mu,
                       float_sw4* __restrict__ a_lambda,
                       float_sw4* __restrict__ a_bforcerhs,
                       float_sw4* __restrict__ a_met, float_sw4 sbop[6],
                       int usesg, float_sw4* __restrict__ a_strx,
                       float_sw4* __restrict__ a_stry) {
#define u(c, i, j, k) a_u[base3 + i + ni * (j) + nij * (k) + nijk * (c)]
#define mu(i, j, k) a_mu[base + i + ni * (j) + nij * (k)]
#define la(i, j, k) a_lambda[base + i + ni * (j) + nij * (k)]
