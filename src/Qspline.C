...
Qspline::Qspline(int npts, float_sw4* fun, float_sw4 tmin, float_sw4 dt,
  ...
  //   cout << "m_tmin = " << m_tmin << " m_dt= " << m_dt << " m_npts= " <<
  //   m_npts << endl;
  // }
  // end tmp
...
#undef A
  ...
  for (int i = 0; i < npts - 1; i++) {
    ...
    m_polcof[3 + 6 * i] = 10 * (fun[i + 1] - fun[i]) - 4 * rhs[2 * i + 2] -
                          6 * rhs[2 * i] - 1.5 * rhs[2 * i + 1] +
                          0.5 * rhs[2 * i + 3];
    m_polcof[4 + 6 * i] = -15 * (fun[i + 1] - fun[i]) + 7 * rhs[2 * i + 2] +
                          8 * rhs[2 * i] + 1.5 * rhs[2 * i + 1] -
                          rhs[2 * i + 3];
    m_polcof[5 + 6 * i] = 6 * (fun[i + 1] - fun[i]) -
                          3 * (rhs[2 * i] + rhs[2 * i + 2]) +
                          0.5 * (rhs[2 * i + 3] - rhs[2 * i + 1]);
  }
  delete[] rhs;
}
...
//    f = m_polcof[6*k] + m_polcof[1+6*k]*arg + m_polcof[2+6*k]*arg*arg +
//    m_polcof[3+6*k]*arg*arg*arg +
...
//    m_polcof[3+6*k]*arg*arg*arg +
//        m_polcof[4+6*k]*arg*arg*arg*arg + m_polcof[5+6*k]*arg*arg*arg*arg*arg;
//    f1= (m_polcof[1+6*k] + 2*m_polcof[2+6*k]*arg + 3*m_polcof[3+6*k]*arg*arg +
//    4*m_polcof[4+6*k]*arg*arg*arg+
// 	5*m_polcof[5+6*k]*arg*arg*arg*arg)*m_dti;
//    f2 = (2*m_polcof[2+6*k] + 6*m_polcof[3+6*k]*arg +
//    12*m_polcof[4+6*k]*arg*arg + 20*m_polcof[5+6*k]*arg*arg*arg)*m_dti*m_dti;
//    f3 = (6*m_polcof[3+6*k] + 24*m_polcof[4+6*k]*arg +
//    60*m_polcof[5+6*k]*arg*arg)*m_dti*m_dti*m_dti; f4 = (24*m_polcof[4+6*k] +
//    120*m_polcof[5+6*k]*arg)*m_dti*m_dti*m_dti*m_dti;
// }

//-----------------------------------------------------------------------
Qspline::~Qspline() {
  if (m_polcof != NULL) delete[] m_polcof;
}
