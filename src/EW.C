...
void energy4c(int*, int*, int*, int*, int*, int*, int*, int*, int*, int*, int*,
              int*, int*, float_sw4*, float_sw4*, float_sw4*, float_sw4*,
              float_sw4*, float_sw4*);
void lambexact(int*, int*, int*, int*, int*, int*, double*, double*, double*,
...
void curvilinear4sg_ci(
    ...
    float_sw4* a_u, float_sw4* a_u1, float_sw4* a_u2, float_sw4* a_u3,
...
EW::EW(const string& fileName, vector<vector<Source*> >& a_GlobalSources,
  ...
  if (!m_myRank) std::cout << "Using MPI buffers in device memory\n";
    ...
    //  std::cerr << "SW4 must be run using the -M -gpu flag with Device
    //  buffers\n";
    // abort();
    //}
...
EW::~EW() {
  ...
  for (int m = 0; m < mNumberOfGrids; m += 4) {
    // std::cout<<"MPI BUFFER TYPE IS "<<as_int(mpi_buffer_space)<<"
    // "<<std::get<0>(bufs_type1[4 * m])<<" \n";
...
void EW::computeNearestTopoGridPoint(int& iNear, int& jNear, float_sw4 a_x,
  ...
  if (a_x - (iNear - 0.5) * h > 0.) iNear++;

  jNear = static_cast<int>(floor(a_y / h)) + 1;
  if (a_y - (jNear - 0.5) * h > 0.) jNear++;
}
...
void EW::print_execution_times(double times[10]) {
  ...
  if (!mQuiet && proc_zero()) {
    ...
    for (int p = 0; p < no_of_procs(); p++)
      for (int c = 0; c < nt; c++) avgs[c] += time_sums[nt * p + c];
    for (int c = 0; c < nt; c++) avgs[c] /= no_of_procs();
    cout << "\n----------------------------------------" << endl;
...
void EW::normOfDifference(vector<Sarray>& a_Uex, vector<Sarray>& a_U,
  ...
  for (g = 0; g < mNumberOfGrids; g++) {
    ...
    if (topographyExists() && g >= mNumberOfCartesianGrids) {
      ...
      // &h,
      // FTNC		   uex_ptr, u_ptr, &linfLocal, &l2Local, &xInfGrid,
      // &m_zmin[g], &x0, FTNC		   &y0, &z0, &radius,
...
void EW::bndryInteriorDifference(vector<Sarray>& a_Uex, vector<Sarray>& a_U,
  ...
  for (g = 0; g < mNumberOfGrids; g++) {
    ...
    // &nz, &h,
    // FTNC		   uex_ptr, u_ptr, &lowZ[3*g], &interiorZ[3*g],
    // &highZ[3*g]
    // );
  }
}
    ...
    if (m_anisotropic) {
      ...
      for (g = 0; g < mNumberOfCartesianGrids; g++) {
        ...
        if (usingSupergrid()) {
          ...
          // FTNC			      &klast, f_ptr, &a_t, &om, &cv, &ph, &omm,
          // &phm,
          // &amprho, &ampmu, &ampla, FTNC			      &h, &zmin
          // );
          if (m_use_attenuation) {
            // FTNC		 if( m_croutines )
      ...
      for (g = mNumberOfCartesianGrids; g < mNumberOfGrids; g++) {
        ...
        if (usingSupergrid()) {
          ...
          if (m_use_attenuation) {
            ...
            // &jlast, &kfirst, FTNC
            // &klast, f_ptr, &a_t, &om, &cv, &ph, &omm, FTNC
            // &phm, &amprho, &ampmu, &ampla,
            // FTNC				       mX.c_ptr(), mY.c_ptr(),
            // mZ.c_ptr(), FTNC				       &omstrx, &omstry,
            // &omstrz );
          }
        } else {
          // FTNC	      if( m_croutines )
          ...
          // &kfirst,
          // FTNC			       &klast, f_ptr, &a_t, &om, &cv,
          // &ph, &omm, FTNC			       &phm, &amprho, &ampmu,
          // &ampla, FTNC			       mX.c_ptr(), mY.c_ptr(),
          // mZ.c_ptr()
          // );
          if (m_use_attenuation) {
            // FTNC		 if( m_croutines )
...
#pragma unroll
        ...
        // for (int i = 0; i < 3; i++)  Fnorm+=*ForceAddress_copy[index +
        // i]**ForceAddress_copy[index + i];
#endif
          }
        });
...
#ifdef SW4_NORM_TRACE
    norm_trace_file << "FORCE NORM " << a_t << " " << norm0.get() << " "
                    << norm1.get() << " " << norm2.get() << " " << Fnorm.get()
                    << " " << a_F[0].norm() << "\n";  // <<Anorm.get()<<"\n";
    // cout.precision(3);
#endif
  }
        ...
        if (usingSupergrid()) {
          ...
          if (m_use_attenuation) {
            ...
            forcingttfortsgatt_ci(ifirst, ilast, jfirst, jlast, kfirst, klast,
                                  ...
                                  omstrz);
            // FTNC		 else
            // FTNC		    forcingttfortsgatt( &ifirst, &ilast, &jfirst,
            // &jlast, &kfirst, FTNC
            // &klast, f_ptr, &a_t, &om, &cv, &ph, &omm, &phm, &amprho, &ampmu,
            // &ampla,
            // FTNC					&h, &zmin, &omstrx, &omstry,
          ...
          // &jlast, &kfirst,
          // FTNC				&klast, f_ptr, &a_t, &om, &cv, &ph,
          // &omm, &phm, FTNC				&amprho, &ampmu, &ampla,
          // &h, &zmin
          // );
          if (m_use_attenuation) {
            // FTNC		 if( m_croutines )
            ...
            // &jlast, &kfirst, FTNC
            // &klast, f_ptr, &a_t, &om, &cv, &ph, &omm, &phm, FTNC
        ...
        if (usingSupergrid()) {
          ...
          if (m_use_attenuation) {
            ...
            forcingttfortsgattc_ci(ifirst, ilast, jfirst, jlast, kfirst, klast,
                                   ...
                                   mZ[g].c_ptr(), omstrx, omstry, omstrz);
            // FTNC		 else
            // FTNC		    forcingttfortsgattc( &ifirst, &ilast, &jfirst,
            // &jlast, &kfirst, FTNC
            // &klast, f_ptr, &a_t, &om, &cv, &ph, &omm, &phm, &amprho, &ampmu,
            // &ampla, FTNC					 mX.c_ptr(),
            // mY.c_ptr(), mZ.c_ptr(),
...
void EW::evalRHS(vector<Sarray>& a_U, vector<Sarray>& a_Mu,
  ...
  for (g = 0; g < mNumberOfCartesianGrids; g++) {
                    ...
                    // FTNC    if( m_croutines )
    {
      if (usingSupergrid())
        rhs4th3fortsgstr_ci(
            ifirst, ilast, jfirst, jlast, kfirst, klast, nz, onesided_ptr,
            m_acof, m_bope, m_ghcof, uacc_ptr, u_ptr + base3 + nijk,
            u_ptr + base3 + 2 * nijk, u_ptr + base3 + 3 * nijk, mu_ptr + base,
            la_ptr + base, h, m_sg_str_x[g], m_sg_str_y[g], m_sg_str_z[g], op);
      else
      ...
      for (int a = 0; a < m_number_mechanisms; a++) {
        ...
        // FTNC	  if( m_croutines )
        {
          if (usingSupergrid())
            rhs4th3fortsgstr_ci(
                ifirst, ilast, jfirst, jlast, kfirst, klast, nz, onesided_ptr,
                m_acof_no_gp, m_bope, m_ghcof_no_gp, uacc_ptr,
                alpha_ptr + base3 + nijk, alpha_ptr + base3 + 2 * nijk,
                alpha_ptr + base3 + 3 * nijk, mua_ptr + base,
        ...
        // m_sg_str_x[g], m_sg_str_y[g], m_sg_str_z[g], &op ); FTNC
        // else FTNC		rhs4th3fort(&ifirst, &ilast, &jfirst, &jlast,
        // &kfirst, FTNC			    &klast, &nz, onesided_ptr,
        // m_acof_no_gp, m_bope, m_ghcof_no_gp, FTNC uacc_ptr, alpha_ptr,
        // mua_ptr, lambdaa_ptr, &h, &op ); FTNC	  }
      }
  ...
  for (g = mNumberOfCartesianGrids; g < mNumberOfGrids; g++) {
    ...
    if (m_croutines) {
      ...
      //                   m_ghcof_no_gp, m_sg_str_x[g], m_sg_str_y[g], nkg,
      //                   op);
      curvilinear4sg_ci(ifirst, ilast, jfirst, jlast, kfirst, klast,
                        u_ptr + base3, u_ptr + base3 + nijk,
                        u_ptr + base3 + 2 * nijk, u_ptr + base3 + 3 * nijk,
                        mu_ptr, la_ptr, met_ptr + base4, jac_ptr, uacc_ptr,
                        onesided_ptr, m_acof, m_bope, m_ghcof, m_acof_no_gp,
    ...
    if (m_use_attenuation && m_number_mechanisms > 0) {
      ...
      for (int a = 0; a < m_number_mechanisms; a++) {
        ...
        // FTNC	  if( m_croutines )
        {
          ...
          // curvilinear4sg_ci(ifirst, ilast, jfirst, jlast, kfirst, klast,
          //                   alpha_ptr, mua_ptr, lambdaa_ptr, met_ptr,
          //                   jac_ptr, uacc_ptr, onesided_ptr, m_acof_no_gp,
          //                   m_bope, m_ghcof_no_gp, m_acof_no_gp,
          //                   m_ghcof_no_gp, m_sg_str_x[g], m_sg_str_y[g], nkg,
          //                   op);
          curvilinear4sg_ci(
              ifirst, ilast, jfirst, jlast, kfirst, klast, alpha_ptr + base3,
              alpha_ptr + base3 + nijk, alpha_ptr + base3 + 2 * nijk,
              alpha_ptr + base3 + 3 * nijk, mua_ptr, lambdaa_ptr,
              met_ptr + base4, jac_ptr, uacc_ptr, onesided_ptr, m_acof_no_gp,
              m_bope, m_ghcof_no_gp, m_acof_no_gp, m_ghcof_no_gp, m_sg_str_x[g],
              m_sg_str_y[g], nkg, op);
        }
#endif
          // FTNC	  else
          ...
          // mua_ptr, lambdaa_ptr, met_ptr, jac_ptr, FTNC uacc_ptr,
          // onesided_ptr, m_acof_no_gp, m_bope, m_ghcof_no_gp, FTNC
          // m_sg_str_x[g], m_sg_str_y[g], &op ); FTNC	     else FTNC
          // curvilinear4(&ifirst, &ilast, &jfirst, &jlast, &kfirst, &klast,
          // FTNC alpha_ptr, mua_ptr, lambdaa_ptr, met_ptr, jac_ptr, FTNC
  ...
  void EW::evalRHSanisotropic(vector<Sarray> & a_U, vector<Sarray> & a_C,
    ...
    for (g = 0; g < mNumberOfCartesianGrids; g++) {
      ...
      innerloopanisgstrvc_ci(ifirst, ilast, jfirst, jlast, kfirst, klast, nz,
       ...
       // FTNC    else
    // FTNC       innerloopanisgstrvc( &ifirst, &ilast, &jfirst, &jlast,
    // &kfirst, FTNC			    &klast, &nz, u_ptr, uacc_ptr, c_ptr,
    ...
    for (g = mNumberOfCartesianGrids; g < mNumberOfGrids; g++) {
      ...
      // m_onesided[g],
      // FTNC		    m_acof, m_bope, m_ghcof, m_sg_str_x[g],
      // m_sg_str_y[g], FTNC		    m_sg_str_z[g] );
    }
  }
  ...
  void EW::updateMemVarPred(vector<Sarray*> & a_AlphaVEp,
    ...
    for (int g = 0; g < mNumberOfGrids; g++) {
      ...
      for (int a = 0; a < m_number_mechanisms; a++) {
        ...
        // klast, alp_ptr, alm_ptr, FTNC			  u_ptr,
      ...
      if (m_twilight_forcing) {
        ...
        if (topographyExists() && g >= mNumberOfCartesianGrids)
          ...
          //  It  works with SG stretching because no spatial derivatives occur
          //  in the forcing
  ...
  //-----------------------------------------------------------------------
  void EW::updateMemVarCorrNearInterface(
      Sarray & a_AlphaVEp, Sarray & a_AlphaVEm, Sarray & a_Up, Sarray & a_U,
      Sarray & a_Um, double a_t, int a_mech, int a_grid) {
    SW4_MARK_FUNCTION;
    ...
    // klast, alp_ptr, FTNC                         d1b, d1e, d2b, d2e, d3b,
    // d3e, alm_ptr, up_ptr, u_ptr, um_ptr, mOmegaVE[a_mech], mDt, domain );
  ...
  void EW::update_images(int currentTimeStep, float_sw4 time,
    ...
    for (unsigned int fIndex = 0; fIndex < mImageFiles.size(); ++fIndex) {
      ...
      // dminus is set. Use (up-u)/dt assumed centered at t, when dminus is
      // true.
      int td = 0;
        ...
        //            img->computeImageQuantity(mF, 3);
          ...
          if (topographyExists())
            img->copy2DArrayToImage(
                mTopo);  // save the raw topography; the smoothed is saved by
                         // the mode=grid with z=0
        } else if (img->mMode == Image::UZEXACT ||
                   img->mMode == Image::UXEXACT ||
          ...
          // Note: this is inefficient, the exact solution is computed
          // everywhere, and once for each
...
#pragma omp parallel for
        ...
        // accumulate?
        // // save the file
        //        img->writeImagePlane_2(currentTimeStep, mPath); // save the
        //        grid image
        // 	}
  ...
  void EW::set_sg_thickness(int n_gp) {
    ...
    m_use_sg_width = false;  // will be changed to true once the number of gp
                             // has been converted to a physical width
    if (m_myRank == 0)
  ...
  void EW::set_global_bcs(boundaryConditionType bct[6]) {
    ...
    //  cout << "mbcGlobalType = " << mbcGlobalType[0] << "," <<
    //  mbcGlobalType[1]
  ...
  void EW::layered_speeds(vector<float_sw4> & cp, vector<float_sw4> & z) {
    ...
    float_sw4 x0 = 0.5 * ((m_iStart[0] - 1) * mGridSize[0] +
                          (m_iEnd[0] - 1) * mGridSize[0]);
    float_sw4 y0 = 0.5 * ((m_jStart[0] - 1) * mGridSize[0] +
                          (m_jEnd[0] - 1) * mGridSize[0]);
    int i0, j0, k0, g0;
  ...
  void EW::compute_energy(float_sw4 dt, bool write_file, vector<Sarray>& Um,
    ...
    for (int g = 0; g < mNumberOfGrids; g++) {
      ...
      if (topographyExists() && g >= mNumberOfCartesianGrids) {
        ...
        // &m_kStart[g], &m_kEnd[g], FTNC		     &istart, &iend,
        // &jstart, &jend, &kstart, &kend, onesided_ptr, FTNC
        ...
        // &m_kStart[g], &m_kEnd[g], FTNC		    &istart, &iend,
        // &jstart, &jend, &kstart, &kend, onesided_ptr, FTNC
        // um_ptr, u_ptr, up_ptr, rho_ptr, &mGridSize[g], m_sg_str_x[g],
        // m_sg_str_y[g], FTNC		    m_sg_str_z[g], &locenergy );
      }
      energy += locenergy;
    }
    energy /= (dt * dt);
  ...
  void EW::print_utc(int e) {
    if (proc_zero()) {
      printf("EW reference UTC is  %02i/%02i/%i:%i:%i:%i.%i\n", m_utc0[e][1],
             m_utc0[e][2], m_utc0[e][0], m_utc0[e][3], m_utc0[e][4],
             m_utc0[e][5], m_utc0[e][6]);
    }
  }
  ...
  void EW::extractTopographyFromGridFile(string a_topoFileName) {
    ...
    //----------------------------------------------
    // Check user specified file name. Abort if they are not there or not
    // readable
    //----------------------------------------------
    CHECK_INPUT(
        access(a_topoFileName.c_str(), R_OK) == 0,
    ...
    // 2. interpolate in the grid file to get elevations on the computational
    // grid
    ...
    for (int i = m_iStart[topLevel]; i <= m_iEnd[topLevel]; ++i) {
      for (int j = m_jStart[topLevel]; j <= m_jEnd[topLevel]; ++j) {
        ...
        if (lat > latMax || lat < latMin || lon > lonMax || lon < lonMin) {
          printf(
              "ERROR: x=%e, y=%e corresponds to lon=%e, lat=%e which are "
              "outside "
              "the topography grid\n",
              x, y, lon, lat);
          MPI_Abort(MPI_COMM_WORLD, 1);
  ...
  void EW::extractTopographyFromCartesianFile(string a_topoFileName) {
    ...
    float_sw4 hp =
        1.01 * mGridSize[topLevel];  // change this to 2 grid sizes because
                                     // there are double ghost points?
...
#pragma omp parallel for
  ...
  void EW::extractTopographyFromImageFile(string a_topoFileName) {
    ...
    if (proc_zero()) {
      printf(
          "TopoImage header: prec=%i, npatches=%i, time=%e, plane=%i, "
          "coord=%e, "
          "mode=%i, gridinfo=%i\n",
...
#pragma omp parallel for
  ...
  void EW::extractTopographyFromRfile(std::string a_topoFileName) {
    ...
    if (fd != -1) {
      ...
      if (nr != sizeof(int)) {
        cout << rname << " Error reading att, nr= " << nr << "bytes read"
             << endl;
        close(fd);
        return;
      }
      ...
      if (nr != sizeof(int)) {
        cout << rname << " Error reading len, nr= " << nr << "bytes read"
             << endl;
        close(fd);
        return;
      }
...
#pragma omp parallel for reduction(max : topomax) reduction(min : topomin)
      for (int i = m_iStart[topLevel]; i <= m_iEnd[topLevel]; ++i) {
        for (int j = m_jStart[topLevel]; j <= m_jEnd[topLevel]; ++j) {
          ...
          if (!extrapol) {
            ...
            float_sw4 Qim1, Qi, Qip1, Qip2, Rjm1, Rj, Rjp1, Rjp2, tjm1, tj,
                tjp1, tjp2;
            Qim1 = (q) * (q - 1) * (q - 2) / (-6.);
            Qi = (q + 1) * (q - 1) * (q - 2) / (2.);
            Qip1 = (q + 1) * (q) * (q - 2) / (-2.);
            ...
            tjm1 = Qim1 * gridElev(i0 - 1, j0 - 1, 1) +
                   ...
                   Qip2 * gridElev(i0 + 2, j0 - 1, 1);
            tj = Qim1 * gridElev(i0 - 1, j0, 1) + Qi * gridElev(i0, j0, 1) +
                 Qip1 * gridElev(i0 + 1, j0, 1) +
                 Qip2 * gridElev(i0 + 2, j0, 1);
            tjp1 = Qim1 * gridElev(i0 - 1, j0 + 1, 1) +
                   Qi * gridElev(i0, j0 + 1, 1) +
                   Qip1 * gridElev(i0 + 1, j0 + 1, 1) +
                   Qip2 * gridElev(i0 + 2, j0 + 1, 1);
  ...
  //     if (file_id < 0) {
  //       cout << "Could not open hdf5 file: " << a_topoFileName.c_str() <<
  //       endl; MPI_Abort(MPI_COMM_WORLD, file_id);
  //     }
  //   }
  ...
  //   if (m_myRank == 0) {
  //     attr_id =
  //         H5Aopen(file_id, "Origin longitude, latitude, azimuth",
  //         H5P_DEFAULT);
  //     ASSERT(attr_id >= 0);
  ...
  //               "ERROR: Sfile azimuth must be equal to coordinate system
  //               azimuth"
  ...
  //       attr_id =
  //           H5Aopen(file_id, "Coarsest horizontal grid spacing",
  //           H5P_DEFAULT);
  //     else
  //       attr_id = H5Aopen(file_id, "Finest horizontal grid spacing",
  //       H5P_DEFAULT);
  //     ASSERT(attr_id >= 0);
  ...
  //   // Depending on the precision of the sfile and sw4, need to convert
  //   between
  //   // float and doulbe
  //   void* in_data;
  ...
  //   if (m_myRank == 0) {
  //     ierr =
  //         H5Dread(dataset_id, h5_dtype, H5S_ALL, H5S_ALL, H5P_DEFAULT,
  //         in_data);
  //     ASSERT(ierr >= 0);
  //     H5Dclose(dataset_id);
  //     H5Gclose(group_id);
  ...
  //                Qip2 * gridElev(i0 + 2, j0 - 1, 1);
  //         tj = Qim1 * gridElev(i0 - 1, j0, 1) + Qi * gridElev(i0, j0, 1) +
  //              Qip1 * gridElev(i0 + 1, j0, 1) + Qip2 * gridElev(i0 + 2, j0,
  //              1);
  //         tjp1 = Qim1 * gridElev(i0 - 1, j0 + 1, 1) +
  //                Qi * gridElev(i0, j0 + 1, 1) +
  //                Qip1 * gridElev(i0 + 1, j0 + 1, 1) +
  ...
  //   int prec, str_len;
  //   double az = 0, origin_x = 0, origin_y = 0, hh = 0, alpha = 0, lon0 = 0,
  //          lat0 = 0;
  //   hsize_t dims[2];
  //   char* crs_to = NULL;
  ...
  //   CHECK_INPUT(fabs(alpha - mGeoAz) < 1e-6,
  //               "ERROR: GMG azimuth must be equal to coordinate system
  //               azimuth"
  ...
  //   if (m_myRank == 0)
  //     printf(
  //         "WARNING: sw4 not compiled with hdf5=yes, ignoring read GMG,
  //         abort!\n");
  //   MPI_Abort(MPI_COMM_WORLD, -1);
  // #endif
  // }
  ...
  //-----------------------------------------------------------------------
  void EW::add_to_grad(
      vector<Sarray> & K, vector<Sarray> & Kacc, vector<Sarray> & Um,
  ...
  void EW::get_epicenter(float_sw4 & epiLat, float_sw4 & epiLon,
                         float_sw4 & epiDepth, float_sw4 & earliestTime,
                         int e) {
    epiLat = m_epi_lat[e];
    epiLon = m_epi_lon[e];
    ...
    for (int g = 0; g < mNumberOfGrids; g++) {
      ...
      if (nn > 0 && verbose == 1) {
        ...
        cout << "proc " << m_myRank << " grid " << g << " array " << name
             << " found " << nn << "  nans. First nan at " << cnan << " "
             << inan << " " << jnan << " " << knan << endl;
      }
    }
    return retval;
  ...
  void EW::setup_attenuation_relaxation(float_sw4 minvsoh) {
    // using minvsoh, estimate highest resolved frequency on the mesh. Use this
    // to assign omega_max for attenuation use the number of mechanisms to
    // determine bandwidth of attenuation model. Use this to assign omega_min
    ...
    //     else if (n >= 5)
    //     {
    //       m_min_omega = m_max_omega/2000.; // decent accuracy for 5
    //       mechanisms
    //     }
        ...
        for (int k = m_kStart[g]; k <= m_kEnd[g]; k++)
          for (int j = m_jStart[g]; j <= m_jEnd[g]; j++)
            for (int i = m_iStart[g]; i <= m_iEnd[g]; i++) {
              ...
              if (info != 0) {
                printf(
                    "setup_viscoelastic:: solving for qs=%e, processor=%i, "
                    "dgels "
              ...
              if (info != 0) {
                printf(
                    "setup_viscoelastic:: solving for qp=%e, processor=%i, "
                    "dgels "
                    "returned error code = %i\n",
                    qp, m_myRank, info);
                MPI_Abort(MPI_COMM_WORLD, 1);
              ...
              for (int nu = 0; nu < n; nu++) {
                ...
                mLambdaVE[g][nu](i, j, k) =
                    kappa_tmp - 2 * mMuVE[g][nu](i, j, k);
              }
              // save the unrelaxed value
...
#undef a
  ...
  void EW::setup_viscoelastic_tw() {
    ...
    if (m_number_mechanisms != 1) {
      printf(
          "setup_viscoelastic_tw:: Number of mechanisms must be %i for "
          "twilight "
...
#pragma omp parallel for reduction(min : minvs)
  ...
  void EW::sort_grid_point_sources(vector<GridPointSource*> & point_sources,
    ...
    for (int g = 0; g < mNumberOfGrids - 1; g++) {
      gptr[g + 1] =
          gptr[g] + static_cast<size_t>((m_iEnd[g] - m_iStart[g] + 1)) *
                        (m_jEnd[g] - m_jStart[g] + 1) *
                        (m_kEnd[g] - m_kStart[g] + 1);
    }
    ...
    if (m_twilight_forcing) {
      if (m_anisotropic) {
        ...
        for (g = 0; g < mNumberOfCartesianGrids; g++) {
          ...
          if (m_croutines)
            tw_aniso_force_ci(ifirst, ilast, jfirst, jlast, kfirst, klast,
        ...
        for (g = 0; g < mNumberOfCartesianGrids; g++) {
          ...
          kfirst = m_kStart[g];
          klast = m_kEnd[g];
          h = mGridSize[g];  // how do we define the grid size for the
                             // curvilinear grid?
          zmin = m_zmin[g];
          ...
          if (usingSupergrid()) {
            ...
            float_sw4 omstrz = m_supergrid_taper_z[g].get_tw_omega();
            if (m_croutines)
              forcingfortsg_ci(ifirst, ilast, jfirst, jlast, kfirst, klast,
                               f_ptr, a_t, om, cv, ph, omm, phm, amprho, ampmu,
                               ampla, h, zmin, omstrx, omstry, omstrz);
            else
        ...
        for (g = mNumberOfCartesianGrids; g < mNumberOfGrids; g++) {
          ...
          if (usingSupergrid()) {
            ...
            if (m_croutines)
              forcingfortc_ci(ifirst, ilast, jfirst, jlast, kfirst, klast,
                              f_ptr, a_t, om, cv, ph, omm, phm, amprho, ampmu,
                              ampla, mX[g].c_ptr(), mY[g].c_ptr(),
                              mZ[g].c_ptr());
            else
      ...
      if (m_anisotropic) {
        ...
        for (g = 0; g < mNumberOfCartesianGrids; g++) {
          ...
          if (m_croutines)
            ...
            tw_aniso_force_tt(ifirst, ilast, jfirst, jlast, kfirst, klast,
                              f_ptr, a_t, om, cv, ph, omm, phm, amprho, phc, h,
                              zmin);
        }  // end for all Cartesian grids
            ...
            if (m_use_attenuation) {
              if (m_croutines)
                ...
                forcingttfortsgatt(&ifirst, &ilast, &jfirst, &jlast, &kfirst,
                                   &klast, f_ptr, &a_t, &om, &cv, &ph, &omm,
                                   &phm, &amprho, &ampmu, &ampla, &h, &zmin,
                                   &omstrx, &omstry, &omstrz);
            }
          } else {
            if (m_croutines)
              forcingttfort_ci(ifirst, ilast, jfirst, jlast, kfirst, klast,
                               f_ptr, a_t, om, cv, ph, omm, phm, amprho, ampmu,
                               ampla, h, zmin);
            else
      ...
      for (int k = klow; k <= khigh; k++)
        for (int j = jlow; j <= jhigh; j++)
          for (int i = ilow; i <= ihigh; i++)
            for (int c = 1; c <= a_U[g].m_nc; c++) a_U[g](c, i, j, k) = 0;
    }
  }
  //-----------------------------------------------------------------------
...
#pragma omp parallel for
  ...
  void EW::heat_kernel_filter(vector<Sarray> & u, float_sw4 ep, int nit) {
    ...
    // Two iterations between communication, because always at least 5pt
    // overlap, take care of odd number of iterations first.
    if (nit % 2 == 1) {
      for (int g = 0; g < mNumberOfGrids; g++) {
        Sarray utmp(u[g]);
#pragma omp parallel for
...
#pragma omp parallel for
  ...
  void rhs4th3point(
      ...
      float_sw4* __restrict__ a_stry, float_sw4* __restrict__ a_strz, int i,
      int j, int k);
  //-----------------------------------------------------------------------
