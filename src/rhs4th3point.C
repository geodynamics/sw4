...
#define ghcof(i) a_ghcof[i - 1]
  ...
  int a1 = 0;
  bool upper = onesided[4] == 1 && k <= 6;
  bool lower = onesided[5] == 1 && k >= nk - 5;
  ...
  if (upper) {
    ...
    /* ghost point only influences the first point (k=1) because ghcof(k)=0 for
     * k>=2*/
     ...
     * k>=2 */
    r3 = r3 +
         (mu3zz + ghcof(k) * (la(i, j, 1) + 2 * mu(i, j, 1)) * u(3, i, j, 0));
  ...
  if (lower) {
    ...
    /* xx, yy, and zz derivatives: */
    /* note that we could have introduced intermediate variables for the average
     * of lambda  */
    ...
    /* all indices ending with 'b' are indices relative to the boundary, going
     * into the domain (1,2,3,...)*/
    kb = nk - k + 1;
    /* all coefficient arrays (acof, bope, ghcof) should be indexed with these
     * indices */
