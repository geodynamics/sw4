...
RAJA::forall<DEFAULT_LOOP1_ASYNC> (RAJA::RangeSegment(0,m_npts),[=] RAJA_DEVICE(size_t i){
    lm_data[i] = 0;});
}
...
//       for( size_t i=0 ; i < m_ni*((size_t) m_nj)*m_nk*m_nc ; i++ )
     ...
     RAJA::forall<SARRAY_LOOP_POL1> (RAJA::RangeSegment(0,end),[=] RAJA_DEVICE(size_t i){
	 m_data[i] = ar[i];});
   }
   else if( m_corder )
   {
...
// 	       for( int c=0 ; c < m_nc ; c++ )
    ...
    RAJA::statement::CudaKernel<
      RAJA::statement::Tile<1, RAJA::statement::tile_fixed<4>, RAJA::cuda_block_y_loop,
        RAJA::statement::Tile<3, RAJA::statement::tile_fixed<64>, RAJA::cuda_block_x_loop,
