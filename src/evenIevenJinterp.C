...
#include "sw4.h"
//--------------------- Jacobi ------------------------
void evenIevenJsw4_typeerpJacobi(float_sw4 rmax[6], Sarray &Uf, Sarray &UfNew,
                            Sarray &Uc, Sarray &Morc, Sarray &Mlrc,
                            Sarray &Morf, Sarray &Mlrf, Sarray &Unextf,
                            Sarray &UnextcSw4_Typeerp, sw4_type a_iStart[], sw4_type a_iEnd[],
                            sw4_type a_jStart[], sw4_type a_jEnd[], sw4_type a_kStart[],
                            sw4_type a_kEnd[], sw4_type a_iStartSw4_Type[], sw4_type a_iEndSw4_Type[],
  ...
  SW4_MARK_FUNCTION;
  // tmp
  //  printf("Inside evenIevenJsw4_typeerp! ");
...
#pragma omp simd
    for (sw4_type i = ifb; i <= ife; i += 2) {
      ...
      // for( sw4_type c=1 ; c <= 2 ; c++ ) // update components 1,2 of the ghost
      // point value of Uf
      // {
      // un-roll the c-loop
      sw4_type c = 1;
...
#pragma omp simd
  ...
  rmax[4] = rmax2;
  rmax[5] = rmax3;
}  // end evenIevenJsw4_typeerpJacobi

// ---------- Optimized version -----------
void evenIevenJsw4_typeerpJacobiOpt(
...
#define UfNew(c, i, j, k) \
  ...
  RAJA::kernel<XRHS_POL_ASYNC>(RAJA::make_tuple(c_range, j_srange, i_srange),
...
#undef Uf
#undef UfNew
}  // end evenIevenJsw4_typeerpJacobiOpt

//--------------------- Reference ------------------------
void evenIevenJsw4_typeerp(float_sw4 rmax[6], Sarray &Uf, Sarray &Muf,
                      ...
                      float_sw4 hf, float_sw4 hc, float_sw4 cof,
...
#pragma omp simd
    for (sw4_type i = ifb; i <= ife; i += 2) {
      ...
      for (sw4_type c = 1; c <= 2; c++) {
        ...
        //     + 9*(-Unextc(c,ic-1,jc,  1)+9*(Unextc(c,ic,jc,
        //     1)+Unextc(c,ic+1,jc,  1))-Unextc(c,ic+2,jc,  1)
        //          -Unextc(c,ic-1,jc+1,1)+9*(Unextc(c,ic,jc+1,1)+Unextc(c,ic+1,jc+1,1))-Unextc(c,ic+2,jc+1,1))
      ...
      //     + 9*(-Unextc(3,ic-1,jc,  1)+9*(Unextc(3,ic,jc, 1)+Unextc(3,ic+1,jc,
      //     1))-Unextc(3,ic+2,jc,  1)
      // 	 -Unextc(3,ic-1,jc+1,1)+9*(Unextc(3,ic,jc+1,1)+Unextc(3,ic+1,jc+1,1))-Unextc(3,ic+2,jc+1,1))
