...
#pragma omp simd
    for( int i=ifb ; i <= ife ; i+=2 )
    {
      ...
      jc = j/2;
//      for (int c=1; c<=2; c++){
// updated component 1 &2 of the ghost point value of Uf
//      int c=1;
      
// All Unextc terms
      b1 = i256*
	( Unextc(1,ic-1,jc-1,1)-9*(Unextc(1,ic,jc-1,1)+Unextc(1,ic+1,jc-1,1))+Unextc(1,ic+2,jc-1,1)
	  + 9*(-Unextc(1,ic-1,jc,  1)+9*(Unextc(1,ic,jc,  1)+Unextc(1,ic+1,jc,  1))-Unextc(1,ic+2,jc,  1)  
	       -Unextc(1,ic-1,jc+1,1)+9*(Unextc(1,ic,jc+1,1)+Unextc(1,ic+1,jc+1,1))-Unextc(1,ic+2,jc+1,1))
	  +Unextc(1,ic-1,jc+2,1)-9*(Unextc(1,ic,jc+2,1)+Unextc(1,ic+1,jc+2,1))+Unextc(1,ic+2,jc+2,1) );

// All Uc terms
      b1 = b1 + nuc*a_ghcof[0]*i256*(
	Uc(1,ic-1,jc-1,0)*Morc(ic-1,jc-1,1)-9*(Uc(1,ic,  jc-1,0)*Morc(ic,  jc-1,1)+Uc(1,ic+1,jc-1,0)*Morc(ic+1,jc-1,1)) +
	Uc(1,ic+2,jc-1,0)*Morc(ic+2,jc-1,1)
	+ 9*(
	  -Uc(1,ic-1,jc,0)*Morc(ic-1,jc,1)
	  +9*(Uc(1,ic,  jc,0)*Morc(ic,  jc,1)+Uc(1,ic+1,jc,0)*Morc(ic+1,jc,1))
	  -Uc(1,ic+2,jc,0)*Morc(ic+2,jc,1)  
	  -Uc(1,ic-1,jc+1,0)*Morc(ic-1,jc+1,1)+
	  9*(Uc(1,ic,  jc+1,0)*Morc(ic,  jc+1,1)+Uc(1,ic+1,jc+1,0)*Morc(ic+1,jc+1,1))
	  -Uc(1,ic+2,jc+1,0)*Morc(ic+2,jc+1,1)
	  )
	+ Uc(1,ic-1,jc+2,0)*Morc(ic-1,jc+2,1)
	-9*(Uc(1,ic,  jc+2,0)*Morc(ic,  jc+2,1) +Uc(1,ic+1,jc+2,0)*Morc(ic+1,jc+2,1))
	+Uc(1,ic+2,jc+2,0)*Morc(ic+2,jc+2,1)
	);
      b1 = b1 - Unextf(1,i,j,nkf); 
      a11 = nuf*a_ghcof[0]*Muf(i,j,nkf)/(Rhof(i,j,nkf));
//                  a11 = nuf*a_ghcof[0]*Muf(i,j,nkf)/(Rhof(i,j,nkf))/(strf_x(i)*strf_y(j));
      r3 = Uf(1,i,j,nkf+1); // save old value for relaxation
// update ghost point value Uf(1,i,j,nkf+1)
      Uf(1,i,j,nkf+1) = b1/a11;
      Uf(1,i,j,nkf+1) = relax*Uf(1,i,j,nkf+1)+(1-relax)*r3;
// change in ghost point value
      r3 = r3 - Uf(1,i,j,nkf+1);
//      if( c == 1 )
      rmax1 = rmax1 > fabs(r3) ? rmax1 : fabs(r3);
      // else
      // 	rmax2 = rmax2 > fabs(r3) ? rmax2 : fabs(r3);
...
// work on componet 2 of the ghost point value of Uf
//      c = 2;
// All Unextc terms
      b1 = i256*
	( Unextc(2,ic-1,jc-1,1)-9*(Unextc(2,ic,jc-1,1)+Unextc(2,ic+1,jc-1,1))+Unextc(2,ic+2,jc-1,1)
	  + 9*(-Unextc(2,ic-1,jc,  1)+9*(Unextc(2,ic,jc,  1)+Unextc(2,ic+1,jc,  1))-Unextc(2,ic+2,jc,  1)  
	       -Unextc(2,ic-1,jc+1,1)+9*(Unextc(2,ic,jc+1,1)+Unextc(2,ic+1,jc+1,1))-Unextc(2,ic+2,jc+1,1))
	  +Unextc(2,ic-1,jc+2,1)-9*(Unextc(2,ic,jc+2,1)+Unextc(2,ic+1,jc+2,1))+Unextc(2,ic+2,jc+2,1) );

// All Uc terms
      b1 = b1 + nuc*a_ghcof[0]*i256*(
	Uc(2,ic-1,jc-1,0)*Morc(ic-1,jc-1,1)-9*(Uc(2,ic,  jc-1,0)*Morc(ic,  jc-1,1)+Uc(2,ic+1,jc-1,0)*Morc(ic+1,jc-1,1)) +
	Uc(2,ic+2,jc-1,0)*Morc(ic+2,jc-1,1)
	+ 9*(
	  -Uc(2,ic-1,jc,0)*Morc(ic-1,jc,1)
	  +9*(Uc(2,ic,  jc,0)*Morc(ic,  jc,1)+Uc(2,ic+1,jc,0)*Morc(ic+1,jc,1))
	  -Uc(2,ic+2,jc,0)*Morc(ic+2,jc,1)  
	  -Uc(2,ic-1,jc+1,0)*Morc(ic-1,jc+1,1)+
	  9*(Uc(2,ic,  jc+1,0)*Morc(ic,  jc+1,1)+Uc(2,ic+1,jc+1,0)*Morc(ic+1,jc+1,1))
	  -Uc(2,ic+2,jc+1,0)*Morc(ic+2,jc+1,1)
	  )
	+ Uc(2,ic-1,jc+2,0)*Morc(ic-1,jc+2,1)
	-9*(Uc(2,ic,  jc+2,0)*Morc(ic,  jc+2,1) +Uc(2,ic+1,jc+2,0)*Morc(ic+1,jc+2,1))
	+Uc(2,ic+2,jc+2,0)*Morc(ic+2,jc+2,1)
	);
      b1 = b1 - Unextf(2,i,j,nkf); 
      a11 = nuf*a_ghcof[0]*Muf(i,j,nkf)/(Rhof(i,j,nkf));
//                  a11 = nuf*a_ghcof[0]*Muf(i,j,nkf)/(Rhof(i,j,nkf))/(strf_x(i)*strf_y(j));
      r3 = Uf(2,i,j,nkf+1); // save old value for relaxation
// update ghost point value Uf(2,i,j,nkf+1)
      Uf(2,i,j,nkf+1) = b1/a11;
      Uf(2,i,j,nkf+1) = relax*Uf(2,i,j,nkf+1)+(1-relax)*r3;
// change in ghost point value
      r3 = r3 - Uf(2,i,j,nkf+1);
      // if( c == 1 )
      // 	rmax1 = rmax1 > fabs(r3) ? rmax1 : fabs(r3);
      // else
      rmax2 = rmax2 > fabs(r3) ? rmax2 : fabs(r3);
...
// solve for the ghost point value Uf(3,i,j,nkf+1)
  ...
  rmax[4] = rmax2;
  rmax[5] = rmax3;
} // end evenIevenJinterpOpt

//------------------------------------------------
// void evenIevenJinterpOpt(float_sw4 rmax[6], Sarray &Uf, Sarray &Muf, Sarray &Lambdaf, Sarray &Rhof, 
// 			 Sarray &Uc, Sarray &Muc, Sarray &Lambdac, Sarray &Rhoc,
// 			 Sarray &Morc, Sarray &Mlrc,
// 			 Sarray &Unextf, Sarray &Bf, Sarray &Unextc, Sarray &Bc,
// 			 int a_iStart[], int a_jStart[], int a_iStartInt[], int a_iEndInt[], int a_jStartInt[], int a_jEndInt[],
...
// // updated component 2 of the ghost point value of Uf
//   c=2;
