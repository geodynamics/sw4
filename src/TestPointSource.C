         ...
         for( int k=kb ; k <= ke ; k++ )
            for( int j=jb ; j <= je ; j++ )
			   ...
			   1/pow(R,2) * Gaussian_x_T_Integral(time, R, fr, alpha, beta) ) / (4*M_PI*rho*R) ;
                        ...
                        u(1,i,j,k) = ( (x - x0)*(x - x0)*fx + (x - x0)*(y - y0)*fy + (x - x0)*(z - z0)*fz )*A + fx*B;
                        u(2,i,j,k) = ( (y - y0)*(x - x0)*fx + (y - y0)*(y - y0)*fy + (y - y0)*(z - z0)*fz )*A + fy*B;
                        u(3,i,j,k) = ( (z - z0)*(x - x0)*fx + (z - z0)*(y - y0)*fy + (z - z0)*(z - z0)*fz )*A + fz*B;
                     }
                  }
                  else 
                  {
...
float_sw4 TestPointSource::VerySmoothBump(float_sw4 t, float_sw4 R, float_sw4 c)
{
  float_sw4 temp = R;
...
float_sw4 TestPointSource::d_SmoothWave_dt(float_sw4 t, float_sw4 R, float_sw4 c)
{
  float_sw4 temp = R;
  float_sw4 c0 = 2187./8., c1 = -10935./8., c2 = 19683./8., c3 = -15309./8., c4 = 2187./4.;
...
float_sw4 TestPointSource::d_VerySmoothBump_dt(float_sw4 t, float_sw4 R, float_sw4 c)
{
  float_sw4 temp = R;
  float_sw4 c0 = 1024, c1 = -5120, c2 = 10240, c3 = -10240, c4 = 5120, c5 = -1024;
...
float_sw4 TestPointSource::d_C6SmoothBump_dt(float_sw4 t, float_sw4 R, float_sw4 c)
{
  float_sw4 retval=0;
  if( (t-R/c) > 0 && (t-R/c) < 1 )
...
float_sw4 TestPointSource::SmoothWave_x_T_Integral(float_sw4 t, float_sw4 R, float_sw4 alpha, float_sw4 beta)
{
  float_sw4 temp = R;
...
float_sw4 TestPointSource::VerySmoothBump_x_T_Integral(float_sw4 t, float_sw4 R, float_sw4 alpha, float_sw4 beta)
{
  float_sw4 temp = R;
...
float_sw4 TestPointSource::C6SmoothBump_x_T_Integral(float_sw4 t, float_sw4 R, float_sw4 alpha, float_sw4 beta)
{
  float_sw4 temp = R;
...
//-----------------------------------------------------------------------
float_sw4 TestPointSource::Gaussian(float_sw4 t, float_sw4 R, float_sw4 c, float_sw4 f )
{
  float_sw4 temp = R;
...
//-----------------------------------------------------------------------
float_sw4 TestPointSource::d_Gaussian_dt(float_sw4 t, float_sw4 R, float_sw4 c, float_sw4 f)
{
  float_sw4 temp = R;
...
//-----------------------------------------------------------------------
float_sw4 TestPointSource::Gaussian_x_T_Integral(float_sw4 t, float_sw4 R, float_sw4 f, float_sw4 alpha, float_sw4 beta)
{
  float_sw4 temp = R;
