...
//
//-----------------------------------------------------------------------
MaterialParCartesianVels::MaterialParCartesianVels(EW* a_ew, int nx, int ny,
                                                   int nz, int init,
                                                   char* fname)
    : MaterialParameterization(a_ew, fname) {
  //  VERIFY2( nx > 1 && ny > 1 && nz > 1, "MaterialParCartesianVels: The grid
...
void MaterialParCartesianVels::get_material(int nmd, double* xmd, int nms,
  ...
  // to f.d. grid.
  // 3.  (a_rho,a_mu,a_lambda) := T( mRho+a_rho,mCs+a_cs,mCp+a_cp), where T
  // transforms from (rho,cs,cp) to (rho,mu,lambda)
...
//-----------------------------------------------------------------------
void MaterialParCartesianVels::interpolate_parameters(
  ...
  for (int k = 1; k <= m_nz; k++)
    for (int j = 1; j <= m_ny; j++)
      for (int i = 1; i <= m_nx; i++) {
        ...
        if (csp[indm] < csmin) csmin = csp[indm];
        if (cpp[indm] < cpmin) cpmin = cpp[indm];
        if (rhop[indm] > rhmax) rhmax = rhop[indm];
        if (csp[indm] > csmax) csmax = csp[indm];
...
//-----------------------------------------------------------------------
void MaterialParCartesianVels::get_parameters(
    int nmd, double* xmd, int nms, double* xms, std::vector<Sarray>& a_rho,
  ...
  if (nr == 0) {
    ...
    mpfile.interpolate_to_other(xms, 2, m_nx, m_ny, m_nz, m_hx, m_hy, m_hz,
                                m_xmin, m_ymin, m_zmin);
    for (int i = 0; i < 54; i++) cout << i << " mtrl " << xms[i] << endl;
    if (!mpfile.is_update()) subtract_base_mtrl(nms, xms);
  }
}

//-----------------------------------------------------------------------
void MaterialParCartesianVels::get_gradient(
  ...
  for (int k = 1; k <= m_nz; k++)
    for (int j = 1; j <= m_ny; j++)
      for (int i = 1; i <= m_nx; i++) {
        ...
        //            dfs[3*ind]   =
        //            cs*cs*gmup[indm]+(cp*cp-2*cs*cs)*glambdap[indm]+grhop[indm];
...
//-----------------------------------------------------------------------
void MaterialParCartesianVels::interpolate_pseudohessian(
...
//-----------------------------------------------------------------------
ssize_t MaterialParCartesianVels::parameter_index(int ip, int jp, int kp,
                                                  int grid, int var)
// Ignore grid.
{
...
//-----------------------------------------------------------------------
ssize_t MaterialParCartesianVels::local_index(size_t ind_global) { return -1; }

//-----------------------------------------------------------------------
// void MaterialParCartesianVels::gradient_transformation( std::vector<Sarray>&
...
//}
//-----------------------------------------------------------------------
void MaterialParCartesianVels::set_scalefactors(int nmpars, double* sfs,
