...
subroutine updatememvar( ifirst, ilast, jfirst, jlast, kfirst, &
     klast, alp, alm, up, u, um, omega, dt, domain, pred ) bind(c)
  ...
  do k=k1,k2
     do j=jfirst,jlast
        do i=ifirst,ilast
           do c=1,3
              alp(c,i,j,k) = icp*( -cm*alm(c,i,j,k) + u(c,i,j,k) + pcoeff*i6* ( &
                   dto*dto*u(c,i,j,k) + dto*(up(c,i,j,k)-um(c,i,j,k)) + (up(c,i,j,k)-2*u(c,i,j,k)+um(c,i,j,k))  ))
           enddo
        enddo
     enddo
  enddo
