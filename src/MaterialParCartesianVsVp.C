...
//
//-----------------------------------------------------------------------
MaterialParCartesianVsVp::MaterialParCartesianVsVp(EW* a_ew, int nx, int ny,
                                                   int nz, int init,
                                                   char* fname)
...
void MaterialParCartesianVsVp::get_parameters(
  ...
  if (nr == 0) {
    ...
    mpfile.interpolate_to_other(xms, 4, m_nx, m_ny, m_nz, m_hx, m_hy, m_hz,
...
//-----------------------------------------------------------------------
void MaterialParCartesianVsVp::get_gradient(
...
void MaterialParCartesianVsVp::interpolate_pseudohessian(
  ...
  for (int i = 0; i < 2 * npts; i++) tmp[i] = phs[i];
  MPI_Allreduce(tmp, phs, 2 * npts, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  delete[] tmp;
}

//-----------------------------------------------------------------------
ssize_t MaterialParCartesianVsVp::parameter_index(int ip, int jp, int kp,
                                                  int grid, int var)
// Ignore grid.
{
  if (1 <= ip && ip <= m_nx && 1 <= jp && jp <= m_ny && 1 <= kp && kp <= m_nz &&
      (0 <= var) && (var <= 1))
    return 2 * (ip - 1 + static_cast<ssize_t>(m_nx) * (jp - 1) +
                m_nx * m_ny * (kp - 1)) +
           var;
  else
    return -1;
...
//-----------------------------------------------------------------------
ssize_t MaterialParCartesianVsVp::local_index(size_t ind_global) { return -1; }
...
//}
//-----------------------------------------------------------------------
void MaterialParCartesianVsVp::set_scalefactors(int nmpars, double* sfs,
