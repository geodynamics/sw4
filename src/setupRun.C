...
void EW::setupSBPCoeff() {
  ...
  if (m_croutines) {
    ...
    // get coefficients for difference approximation of 2nd derivative with
    ...
    // NEW: setup stencils that do NOT use ghost points (for visco-elastic
    // memory variables)
...
#pragma omp parallel for
    ...
    for (g = 0; g < mNumberOfCartesianGrids; g++) {
      ...
      if (m_croutines)
        exactmatfort_ci(ifirst, ilast, jfirst, jlast, kfirst, klast, rho_ptr,
                        mu_ptr, la_ptr, omm, phm, amprho, ampmu, ampla, h,
                        zmin);
      else
        exactmatfort(&ifirst, &ilast, &jfirst, &jlast, &kfirst, &klast, rho_ptr,
                     mu_ptr, la_ptr, &omm, &phm, &amprho, &ampmu, &ampla, &h,
                     &zmin);
    ...
    if (topographyExists()) {
      ...
      if (m_croutines)
        ...
        exactmatfortc(&ifirst, &ilast, &jfirst, &jlast, &kfirst, &klast,
                      rho_ptr, mu_ptr, la_ptr, &omm, &phm, &amprho, &ampmu,
...
#pragma omp parallel for
    ...
    if (topographyExists()) {
      int g = mNumberOfGrids - 1;
      anisomtrltocurvilinear(&m_iStart[g], &m_iEnd[g], &m_jStart[g], &m_jEnd[g],
                             &m_kStart[g], &m_kEnd[g], mMetric.c_ptr(),
    ...
    for (g = 0; g < mNumberOfCartesianGrids; g++) {
      ...
      if (m_croutines)
        ...
        tw_ani_stiff(ifirst, ilast, jfirst, jlast, kfirst, klast, h, zmin, omm,
    ...
    if (topographyExists()) {
      ...
      if (m_croutines)
        ...
        tw_ani_curvi_stiff(ifirst, ilast, jfirst, jlast, kfirst, klast, x_ptr,
                           y_ptr, z_ptr, omm, phm, amprho, rho_ptr, phc,
                           cm_ptr);
      if (proc_zero())
    ...
    for (int k = m_kStart[g]; k <= m_kEnd[g]; k++)
      for (int j = m_jStart[g]; j <= m_jEnd[g]; j++)
        for (int i = m_iStart[g]; i <= m_iEnd[g]; i++) {
          ...
          Amat[0] =
              -4 *
              (SQR(mMetric(1, i, j, k)) * la2mu +
               SQR(mMetric(1, i, j, k)) * mu +
               SQR(mMetric(2, i, j, k)) * la2mu +
               SQR(mMetric(3, i, j, k)) * mu + SQR(mMetric(4, i, j, k)) * mu) *
              jinv;
          // A21 = A12
          //	   Amat[1] = -4.*(mQ(1,i,j,k)*mQ(2,i,j,k) +
          // mR(1,i,j,k)*mR(2,i,j,k) + mS(1,i,j,k)*mS(2,i,j,k))*(mu+la);
          Amat[1] = -4. * mMetric(2, i, j, k) * mMetric(3, i, j, k) *
                    (mu + la) * jinv;
          // A31 = A13
          //	   Amat[2] = -4.*(mQ(1,i,j,k)*mQ(3,i,j,k) +
          // mR(1,i,j,k)*mR(3,i,j,k) + mS(1,i,j,k)*mS(3,i,j,k))*(mu+la);
          Amat[2] = -4. * mMetric(2, i, j, k) * mMetric(4, i, j, k) *
                    (mu + la) * jinv;
          // A22
          Amat[3] = -4. *
                    (SQR(mMetric(1, i, j, k)) * mu +
                     SQR(mMetric(1, i, j, k)) * la2mu +
                     +SQR(mMetric(2, i, j, k)) * mu +
                     SQR(mMetric(3, i, j, k)) * la2mu +
                     SQR(mMetric(4, i, j, k)) * mu) *
                    jinv;
          // A32 = A23
          //	   Amat[4] = -4.*(mQ(2,i,j,k)*mQ(3,i,j,k) +
          // mR(2,i,j,k)*mR(3,i,j,k) + mS(2,i,j,k)*mS(3,i,j,k))*(mu+la);
          Amat[4] = -4. * mMetric(3, i, j, k) * mMetric(4, i, j, k) *
                    (mu + la) * jinv;
          // A33
          Amat[5] =
              -4. *
              (SQR(mMetric(1, i, j, k)) * mu + SQR(mMetric(1, i, j, k)) * mu +
               SQR(mMetric(2, i, j, k)) * mu + SQR(mMetric(3, i, j, k)) * mu +
               SQR(mMetric(4, i, j, k)) * la2mu) *
              jinv;
...
void EW::setup_supergrid() {
  ...
  for (int g = 0; g < mNumberOfGrids; g++) {
    ...
    if (m_iStartAct[g] < m_iStart[g]) m_iStartAct[g] = m_iStart[g];
    if (m_jStartAct[g] < m_jStart[g]) m_jStartAct[g] = m_jStart[g];
    if (m_iEndAct[g] > m_iEnd[g]) m_iEndAct[g] = m_iEnd[g];
    if (m_jEndAct[g] > m_jEnd[g]) m_jEndAct[g] = m_jEnd[g];
...
void EW::perturb_velocities(vector<Sarray> &a_vs, vector<Sarray> &a_vp) {
  ...
  for (g = 0; g < mNumberOfGrids; g++) {
    ...
    if (m_croutines) {
      ...
      if (g == mNumberOfGrids - 1 && topographyExists()) {
        randomfield3dc(&ifirst, &ilast, &jfirst, &jlast, &kfirst, &klast, &nx,
                       &ny, &nz, &ghost, pert_ptr, wgh_ptr, &m_random_dist,
                       &m_random_distz, &h, mZ.c_ptr(), m_random_seed,
                       saverand_ptr, &p, &pz);
        perturbvelocityc(&ifirst, &ilast, &jfirst, &jlast, &kfirst, &klast,
                         vs_ptr, vp_ptr, pert_ptr, &m_random_amp,
                         &m_random_amp_grad, mZ.c_ptr(), &plimit);
      } else {
        randomfield3d(&ifirst, &ilast, &jfirst, &jlast, &kfirst, &klast, &nx,
                      &ny, &nz, &ghost, pert_ptr, wgh_ptr, &m_random_dist,
                      &m_random_distz, &h, m_random_seed, saverand_ptr, &p,
                      &pz);
        perturbvelocity(&ifirst, &ilast, &jfirst, &jlast, &kfirst, &klast,
                        vs_ptr, vp_ptr, pert_ptr, &m_random_amp,
                        &m_random_amp_grad, &m_zmin[g], &h, &plimit);
      }
    }
  }
}
