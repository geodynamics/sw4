...
#include "F77_FUNC.h"
extern "C" {
void tw_ani_stiff(sw4_type ifirst, sw4_type ilast, sw4_type jfirst, sw4_type jlast, sw4_type kfirst,
                  sw4_type klast, float_sw4 h, float_sw4 zmin, float_sw4 omm,
                  float_sw4 phm, float_sw4 amprho, float_sw4 *rho,
                  float_sw4 *phc, float_sw4 *cm);
void tw_ani_curvi_stiff(sw4_type ifirst, sw4_type ilast, sw4_type jfirst, sw4_type jlast,
                        sw4_type kfirst, sw4_type klast, float_sw4 *xx, float_sw4 *yy,
                        float_sw4 *zz, float_sw4 omm, float_sw4 phm,
                        float_sw4 amprho, float_sw4 *rho, float_sw4 *phc,
                        float_sw4 *cm);
void exactmatfort(sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, float_sw4 *,
                  float_sw4 *, float_sw4 *, float_sw4 *, float_sw4 *,
                  float_sw4 *, float_sw4 *, float_sw4 *, float_sw4 *,
                  float_sw4 *);
void exactmatfortc(sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, float_sw4 *,
                   float_sw4 *, float_sw4 *, float_sw4 *, float_sw4 *,
                   float_sw4 *, float_sw4 *, float_sw4 *, float_sw4 *,
                   float_sw4 *, float_sw4 *);

void wavepropbop_4(double *, double *, double *, double *, double *, double *,
                   double *);
...
void randomfield3dc(sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *,
                    sw4_type *, sw4_type *, double *, double *, double *, double *,
                    double *, double *, sw4_type *, double *, sw4_type *, sw4_type *);
void perturbvelocity(sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, double *,
                     double *, double *, double *, double *, double *, double *,
                     double *);
void perturbvelocityc(sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, double *,
                      double *, double *, double *, double *, double *,
                      double *);
void checkanisomtrl(sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, double *,
                    double *, double *, double *, double *, double *);
void computedtaniso2(sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, double *,
                     double *, double *, double *, double *);
void computedtaniso2curv(sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, double *,
                         double *, double *, double *, double *);
void anisomtrltocurvilinear(sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, sw4_type *, double *,
                            double *, double *);
void bndryOpNoGhost(double *acof_no_gp, double *ghcof_no_gp,
                    double *sbop_no_gp);
}
...
void EW::setupRun(vector<vector<Source *> > &a_GlobalUniqueSources) {
  ...
  m_testing = (m_twilight_forcing || m_point_source_test || m_lamb_test ||
               m_rayleigh_wave_test || m_energy_test);
  ...
  if (mVerbose && proc_zero()) {
    cout << "  Using Bjorn's fast (parallel)"
         << " IO library" << endl;
    if (m_pfs) {
  ...
  if (!mbcsSet) default_bcs();

  // check if there are any supergrid boundaries. If so, setup the supergrid
  // tapering functions using the parameters m_sg_thickness, m_sg_transition,
  // m_supergrid_damping_coefficient
  setup_supergrid();

  // assign m_bcType and m_onesided based on the global boundary conditions and
  // parallel overlap boundaries.
  assign_local_bcs();
  ...
  // for all sides with dirichlet, free surface, supergrid, or periodic boundary
  // conditions, save the extent of the multi-D boundary window, and allocate
  // arrays to hold the boundary forcing
  sw4_type wind[6], npts;
  // with 2 ghost points, we need twice as many elements in the m_BCForcing
  // arrays!!!

  for (sw4_type g = 0; g < mNumberOfGrids; g++) {
    // tmp
    //    printf("Allocating boundary forcing arrays for grid g=%i, # ghost
    //    points=%i\n", g, m_ghost_points);
    ...
    for (sw4_type side = 0; side < 6; side++) {
      if (m_bcType[g][side] == bStressFree || m_bcType[g][side] == bDirichlet ||
        ...
        if (m_bcType[g][side] == bStressFree) {
          ...
          // when calling side_plane with nGhost=1, you get the outermost grid
          // plane for Free surface conditions, we apply the forcing on the
          // boundary itself, i.e., just inside the ghost points add/subtract
          // the ghost point offset
          if (side == 0) {
            wind[0] += m_ghost_points;
            wind[1] = wind[0];
          } else if (side == 1) {
            wind[0] -= m_ghost_points;
            wind[1] = wind[0];
          } else if (side == 2) {
            wind[2] += m_ghost_points;
            wind[3] = wind[2];
          } else if (side == 3) {
            wind[2] -= m_ghost_points;
            wind[3] = wind[2];
        ...
        npts = (wind[5] - wind[4] + 1) * (wind[3] - wind[2] + 1) *
               (wind[1] - wind[0] + 1);
        // copy wind sw4_typeo m_BndryWindow
        for (sw4_type qq = 0; qq < 6; qq++)
      ...
      // tmp
      // printf("proc=%i side=%i, bc=%i, npts=%i, m_BndryWindow[g][side]=[%i %i
      // %i %i %i %i]\n", m_myRank, side, 	     m_bcType[g][side],
      // m_NumberOfBCPoints[g][side], 	     m_BndryWindow[g][0 + side*6],
      // m_BndryWindow[g][1 + side*6], m_BndryWindow[g][2 + side*6],
      // 	     m_BndryWindow[g][3 + side*6], m_BndryWindow[g][4 + side*6],
      // m_BndryWindow[g][5 + side*6]);
    }  // end for side...
  }    // end for grid...

  // communicate the metric across processor boundaries (one-sided differences
  // were used in setup_metric())
  //  if (topographyExists())
  //  {
  ...
  // 	  find_curvilinear_derivatives_at_point( (double) i, (double) j,
  // (double) k, qX, rX, sX ); 	  dist += SQR(qX[0] - mQ(1,i,j,k)) + SQR(qX[1] -
  // mQ(2,i,j,k)) + SQR(qX[2] - mQ(3,i,j,k))
  // 	    + SQR(rX[0] - mR(1,i,j,k)) + SQR(rX[1] - mR(2,i,j,k)) + SQR(rX[2] -
  // mR(3,i,j,k))
  // 	    + SQR(sX[0] - mS(1,i,j,k)) + SQR(sX[1] - mS(2,i,j,k)) + SQR(sX[2] -
  // mS(3,i,j,k));
  // 	}
  //     double totalDist;
  //     MPI_Allreduce( &dist, &totalDist, 1, MPI_DOUBLE, MPI_SUM,
  //     m_cartesian_communicator ); if (m_mxyRank == 0)
  ...
  // tmp
  if (mVerbose >= 2) {
    sw4_type top = mNumberOfGrids - 1;
    printf(
        "=================Processor #%i index bounds====================\n"
        "m_iStart=%i, m_iEnd=%i, m_global_nx=%i, m_jStart=%i, m_jEnd=%i, "
        "m_global_ny=%i\n",
    ...
    printf(
        ...
        m_myRank, m_bcType[top][0], m_bcType[top][1], m_bcType[top][2],
        m_bcType[top][3], m_bcType[top][4], m_bcType[top][5]);
  }

  // tmp : make sure that all bc prsw4_type statements are done. Then abort
  //   MPI_Barrier(MPI_COMM_WORLD);
  //   ASSERT(0)

  //    if( m_output_load )
  //       prsw4_type_loadbalance_info();

  //  string saved_path = mPath;

  //  sw4_type beginCycle = 1;

  // Initialize IO
  for (sw4_type e = 0; e < m_nevent; e++) create_directory(mPath[e]);
  ...
  if (mVerbose && proc_zero()) cout << "  Assigned material properties" << endl;
  ...
  // Note: SW4 always ends the simulation at mTmax, whether prefilter is enabled
  // or not. This behavior is different from WPP
  ...
  if (mVerbose && proc_zero()) cout << "*** Initialized Images" << endl;

  if (m_output_detailed_timing) time_measure[9] = MPI_Wtime();

  // // is the curvilinear grid ok?
  //   if (topographyExists() && m_minJacobian <=0. && proc_zero()) //
  //   m_minJacobian is the global minimum and should be the same on all
  //   processes
  //   {
  //     printf("FATAL ERROR: min(Jacobian) = %e < 0 in the curvilinear
  //     grid.\n", m_minJacobian); printf("You can try to increase zmax or
  //     decrease order in the topography command\n"); MPI_Abort(MPI_COMM_WORLD,
  //     1);
  //   }
  ...
  // 	   wksize = wksize < mU[g].m_nj*mU[g].m_nk ? mU[g].m_nj*mU[g].m_nk :
  // wksize;
  //      for( sw4_type side=2 ; side <= 3 ; side++ )
  // 	if( m_onesided[g][side] == 1 )
  // 	   wksize = wksize < mU[g].m_ni*mU[g].m_nk ? mU[g].m_ni*mU[g].m_nk :
  // wksize;
  //      for( sw4_type side=4 ; side <= 5 ; side++ )
  // 	if( m_onesided[g][side] == 1 )
  // 	   wksize = wksize < mU[g].m_nj*mU[g].m_ni ? mU[g].m_nj*mU[g].m_ni :
  // wksize;
  //   }
  ...
  // do source specific initialization such as prefiltering, scaling by mu,
  // compute epicenter, etc (see below)
  ...
  if (m_output_detailed_timing) {
    ...
    if (!mQuiet && proc_zero()) {
      ...
      for (sw4_type p = 0; p < no_of_procs(); p++)
        for (sw4_type c = 0; c < 10; c++) avgs[c] += time_sums[10 * p + c];
      for (sw4_type c = 0; c < 10; c++) avgs[c] /= no_of_procs();
      cout << "\n----------------------------------------" << endl;
      cout << "          Setup time summary (average)" << endl;
      //                             6                  9            8 6 7 7 6
      //                             2        5        7
      cout << "SBP+SG      BndryWind  InitPath  SetElastic  EvalResol  "
              "ViscoElastic  CheckPnt     DT     Image     SrcPrep"
           << endl;
      cout.setf(ios::left);
      cout.precision(3);
      cout.width(11);
...
void EW::preprocessSources(vector<vector<Source *> > &a_GlobalUniqueSources) {
  ...
  // 1: computes the epicenter, i.e. the location of the source with the
  // earliest start time 2: if any source returns get_ShearModulusFactor() ==
  // true, it multiplies Mij by the shear modulus 3: evaluate the min and max
  // z-coordinate of all sources 4: sets freq=1/dt in all sources with iDirac
  // time-function 5: modifies the time function if pre-filtering is enabled ***
  ...
  if (!mIsInitialized) {
    if (proc_zero())
      printf(
          "ERROR: Calling preprocessSources before the material model is "
          "initialized\n");
    return;
  }
  ...
  if (!m_twilight_forcing && !m_energy_test && !m_rayleigh_wave_test) {
    ...
    if (m_testing) {
      ...
      if ((m_point_source_test || m_lamb_test) &&
        ...
        if (proc_zero())
          cout << "Error: Point Source Test and Lamb Test must have one single "
                  "source"
               << endl
               << "       The number of defined sources is "
               << a_GlobalUniqueSources.size() << endl;
        sources_ok = false;
      }
      ...
      {
        return;  // not setting mSourcesOK to true should stop the calling
                 // program
      }
      ...
      // find the epicenter, i.e., the location of the source with the lowest
      // value of t0
...
#pragma omp parallel for reduction(|| : need_mu_corr)
        for (sw4_type i = 0; i < a_GlobalUniqueSources[e].size(); i++)
          need_mu_corr =
        ...
        if (need_mu_corr) {
          sw4_type nSources = a_GlobalUniqueSources[e].size();
          // if (proc_zero())
...
#pragma omp parallel for
          for (sw4_type s = 0; s < nSources; s++)
            if (a_GlobalUniqueSources[e][s]->myPoint()) {
              sw4_type is = a_GlobalUniqueSources[e][s]->m_i0;
              sw4_type js = a_GlobalUniqueSources[e][s]->m_j0;
              sw4_type ks = a_GlobalUniqueSources[e][s]->m_k0;
              sw4_type gs = a_GlobalUniqueSources[e][s]->m_grid;
...
#pragma omp parallel for
        ...
        // 	for( sw4_type s=0; s < a_GlobalUniqueSources.size(); s++ )
...
#pragma omp parallel for reduction(max : zMax) reduction(min : zMin)
        for (sw4_type s = 0; s < a_GlobalUniqueSources[e].size(); s++) {
...
//-----------------------------------------------------------------------
void EW::compute_epicenter(vector<Source *> &a_GlobalUniqueSources, sw4_type e) {
  ...
  if (a_GlobalUniqueSources.size() == 0) {
    ...
    for (unsigned sw4_type i = 1; i < a_GlobalUniqueSources.size(); ++i) {
    ...
    computeGeographicCoord(firstSource->getX0(), firstSource->getY0(), epiLon,
                           epiLat);
...
void EW::setupSBPCoeff() {
  ...
  // FTNC  else
  // FTNC  {
  // FTNC// m_iop, m_iop2, m_bop2, m_hnorm never used in code, use local
  // variables: FTNC     float_sw4 hnorm[4], gh2, iop[5], iop2[5], bop2[24];
  // FTNC// get coefficients for difference approximation of 2nd derivative with
  // variable coefficients FTNC     varcoeffs4(m_acof, m_ghcof); FTNC// get
  // coefficients for difference approximation of 1st derivative FTNC
  // wavepropbop_4(iop, iop2, m_bop, bop2, &gh2, hnorm, m_sbop); FTNC// extend
  // the definition of the 1st derivative tothe first 6 points FTNC
  // bopext4th(m_bop, m_bope); FTNC// NEW: setup stencils that do NOT use ghost
  // points (for visco-elastic memory variables) FTNC     bndryOpNoGhost(
  // m_acof_no_gp, m_ghcof_no_gp, m_sbop_no_gp ); FTNC  }
}

//-----------------------------------------------------------------------
void EW::perturb_vels(Sarray &cs, Sarray &cp, Sarray &rndpert) {
  for (sw4_type k = cs.m_kb; k <= cs.m_ke; k++)
    for (sw4_type j = cs.m_jb; j <= cs.m_je; j++)
      for (sw4_type i = cs.m_ib; i <= cs.m_ie; i++) {
...
// values make sense
{
  ...
  if (!m_testing) {
    ...
    if (m_materials.size() > 0) {
      // Sort the m_materials vector in increasing id order...but only if there
      // are more than one element.
      if (m_materials.size() > 1) {
        MaterialProperty *mp;
        for (sw4_type iStart = 0; iStart < m_materials.size() - 1; iStart++) {
          sw4_type idmin = m_materials[iStart]->m_materialID;
          for (sw4_type q = iStart + 1; q < m_materials.size(); q++) {
            if (m_materials[q]->m_materialID < idmin) {
              // record new min value and swap elements q and iStart i
              // m_materials vector
      ...
      if (proc_zero() && mVerbose >= 3) {
        ...
        for (sw4_type q = 0; q < m_materials.size(); q++)
          cout << "Material[" << q << "]->ID=" << m_materials[q]->m_materialID
               << endl;
      }
    }

    // figure out if we may ignore some blocks (i.e. an efile command which is
    // followed by a block command covering all grid points).
    sw4_type lastAllCoveringBlock = 0;
    for (unsigned sw4_type b = 0; b < m_mtrlblocks.size(); b++)
      if (m_mtrlblocks[b]->coversAllPoints()) lastAllCoveringBlock = b;
    // tmp
    if (proc_zero()) {
      if (lastAllCoveringBlock == 0)
        cout << "Considering all material blocks" << endl;
      else
        cout << "Only considering material blocks with index >= "
             << lastAllCoveringBlock << endl;
    }  // end if proc_zero()
    for (unsigned sw4_type b = lastAllCoveringBlock; b < m_mtrlblocks.size(); b++)
      m_mtrlblocks[b]->set_material_properties(mRho, mMu, mLambda, mQs, mQp);

    //   bool linearExtrapolation=false;
    // note that material thresholding for vs and vp happens further down in
    // this procedure
    ...
    // extrapolate to define material properties above the free surface
    // (topography)
    g = mNumberOfGrids - 1;
    ...
    // extrapolate material properties to mesh refinement boundaries (e.g. for
    // doing the LOH cases more accurately)
    if (!mQuiet && proc_zero() && mVerbose >= 3) {
      printf(
          "setMaterials> mMaterialExtrapolate = %i, "
...
#pragma omp parallel for
    ...
    if (m_randomize) {
      ...
      for (sw4_type g = 0; g < mNumberOfGrids; g++) {
        ...
        for (unsigned sw4_type b = 0; b < m_random_blocks.size(); b++)
    ...
    for (g = 0; g < mNumberOfCartesianGrids; g++) {
      ...
      // +     zmin )
      // FTNC	if( m_croutines )
      exactmatfort_ci(ifirst, ilast, jfirst, jlast, kfirst, klast, rho_ptr,
                      mu_ptr, la_ptr, omm, phm, amprho, ampmu, ampla, h, zmin);
      // FTNC	else
    ...
    if (topographyExists()) {
      for (g = mNumberOfCartesianGrids; g < mNumberOfGrids; g++) {
        ...
        ampla = m_twilight_forcing->m_amplambda;
        // FTNC            if( m_croutines )
        exactmatfortc_ci(ifirst, ilast, jfirst, jlast, kfirst, klast, rho_ptr,
                         mu_ptr, la_ptr, omm, phm, amprho, ampmu, ampla, x_ptr,
                         y_ptr, z_ptr);
        // FTNC            else
        // FTNC               exactmatfortc(&ifirst, &ilast, &jfirst, &jlast,
        // &kfirst, FTNC                             &klast, rho_ptr, mu_ptr,
        // la_ptr, &omm, &phm, FTNC                             &amprho, &ampmu,
        // &ampla, x_ptr, y_ptr, z_ptr );

        //   // Need this for Energy testing, random material will not agree on
    ...
    for (g = 0; g < mNumberOfGrids; g++) {
      ...
      // setting all points in the grid to constant + random perturb: No need to
      // extrapolate ghost points!
      for (sw4_type i = 0;
           i < (m_iEnd[g] - m_iStart[g] + 1) * (m_jEnd[g] - m_jStart[g] + 1) *
        ...
        // lambda_ptr[i] = rho0[g]*(vp0[g]*vp0[g] - 2*vs0[g]*vs0[g]) +
        // amp*drand48();
      }
    }
    material_ic(mRho);
...
void EW::set_anisotropic_materials() {
  ...
  if (!m_testing) {
    sw4_type lastAllCoveringBlock = 0;
    for (unsigned sw4_type b = 0; b < m_anisotropic_mtrlblocks.size(); b++)
      if (m_anisotropic_mtrlblocks[b]->coversAllPoints())
        lastAllCoveringBlock = b;

    for (unsigned sw4_type b = lastAllCoveringBlock;
         b < m_anisotropic_mtrlblocks.size(); b++)
...
#pragma omp parallel for
    ...
    for (g = 0; g < mNumberOfCartesianGrids; g++) {
      ...
      // FTNC	 if( m_croutines )
      tw_ani_stiff_ci(ifirst, ilast, jfirst, jlast, kfirst, klast, h, zmin, omm,
                      phm, amprho, rho_ptr, phc, cm_ptr);
      // FTNC	 else
      // FTNC	    tw_ani_stiff(ifirst, ilast, jfirst, jlast, kfirst, klast, h,
      // zmin, FTNC                     omm, phm, amprho, rho_ptr, phc, cm_ptr);
    ...
    if (topographyExists()) {
      for (g = mNumberOfCartesianGrids; g < mNumberOfGrids; g++) {
        ...
        // FTNC            if( m_croutines )
        tw_ani_curvi_stiff_ci(ifirst, ilast, jfirst, jlast, kfirst, klast,
                              x_ptr, y_ptr, z_ptr, omm, phm, amprho, rho_ptr,
                              phc, cm_ptr);
        // FTNC            else
        // FTNC               tw_ani_curvi_stiff(ifirst, ilast, jfirst, jlast,
        // kfirst, klast, x_ptr, y_ptr, z_ptr, FTNC omm, phm, amprho, rho_ptr,
        // phc, cm_ptr);
        if (proc_zero())
    ...
    CHECK_INPUT(false,
                "Error:  the only test for an anisotropic material is twilight"
                    << endl);
  }
...
void EW::check_anisotropic_material(vector<Sarray> &rho, vector<Sarray> &c) {
  ...
  for (sw4_type g = 0; g < mNumberOfGrids; g++) {
    ...
    checkanisomtrl_ci(m_iStart[g], m_iEnd[g], m_jStart[g], m_jEnd[g],
                      m_kStart[g], m_kEnd[g], rho_ptr, c_ptr, rhominloc,
                      rhomaxloc, eigminloc, eigmaxloc);
    // FTNC      else
    // FTNC	 checkanisomtrl(&m_iStart[g], &m_iEnd[g], &m_jStart[g],
    // &m_jEnd[g], FTNC			&m_kStart[g], &m_kEnd[g], FTNC
    // rho_ptr, c_ptr,
    // FTNC			&rhominloc, &rhomaxloc, &eigminloc, &eigmaxloc
    // );
    if (rhominloc < rhomin) rhomin = rhominloc;
    if (rhomaxloc > rhomax) rhomax = rhomaxloc;
    if (eigminloc < eigmin) eigmin = eigminloc;
    if (eigmaxloc > eigmax) eigmax = eigmaxloc;
  }
  rhominloc = rhomin;
  rhomaxloc = rhomax;
  eigminloc = eigmin;
  ...
  if (proc_zero()) {
    ...
    CHECK_INPUT(eigmin > 0,
                "ERROR: tensor of elastic coefficients is not positive "
                "definite. The problem is not well posed"
                    << endl);
  }
}
...
void EW::create_directory(const string &path) {
  if (proc_zero()) {
    ...
    if (err == 0)
      ...
      // fatal error
      cerr << endl
           << "******** Failed to create the directory *******" << endl
           << endl;
  ...
  // the processes need access)
  //    struct stat statBuf;
  //    sw4_type statErr = stat(path.c_str(), &statBuf);
  //    CHECK_INPUT(statErr == 0 && S_ISDIR(statBuf.st_mode), "Error: " << path
  ...
  for (sw4_type e = 0; e < m_nevent; e++) {
    if (mTimeIsSet[e]) {
      ...
      mNumberOfTimeSteps[e] =
          static_cast<sw4_type>((mTmax[e] - mTstart) / mDt + 0.5);
      mNumberOfTimeSteps[e] =
          (mNumberOfTimeSteps[e] == 0) ? 1 : mNumberOfTimeSteps[e];
      // the resulting mDt could be slightly too large, because the
      // numberOfTimeSteps is rounded to the nearest sw4_type When more than one
                                 ...
                                 // curvilinear grids
{
  ...
  for (sw4_type g = 0; g < mNumberOfCartesianGrids; g++) {
    ...
    computedtaniso2_ci(m_iStart[g], m_iEnd[g], m_jStart[g], m_jEnd[g],
                       m_kStart[g], m_kEnd[g], rho_ptr, c_ptr, mCFL,
                       mGridSize[g], dtgrid);
    // FTNC      else
    // FTNC	 computedtaniso2( &m_iStart[g], &m_iEnd[g], &m_jStart[g],
    // &m_jEnd[g], FTNC			  &m_kStart[g], &m_kEnd[g], FTNC
    // rho_ptr, c_ptr, &mCFL, &mGridSize[g], &dtgrid );
    if (dtgrid < dtproc) dtproc = dtgrid;
  }
  if (topographyExists()) {
    for (sw4_type g = mNumberOfCartesianGrids; g < mNumberOfGrids; g++) {
      //      sw4_type g=mNumberOfGrids-1;
      ...
      computedtaniso2curv_ci(m_iStart[g], m_iEnd[g], m_jStart[g], m_jEnd[g],
                             m_kStart[g], m_kEnd[g], rho_ptr, c_ptr, jac_ptr,
                             mCFL, dtgrid);
      // FTNC         else
      // FTNC            computedtaniso2curv( &m_iStart[g], &m_iEnd[g],
      // &m_jStart[g], &m_jEnd[g], FTNC &m_kStart[g], &m_kEnd[g], FTNC rho_ptr,
      // c_ptr, jac_ptr, &mCFL, &dtgrid );
      if (dtgrid < dtproc) dtproc = dtgrid;
    }
  }

  // compute the global minima
  MPI_Allreduce(&dtproc, &mDt, 1, m_mpifloat, MPI_MIN,
                m_cartesian_communicator);
  if (!mQuiet && mVerbose >= 1 && proc_zero()) {
  ...
  for (sw4_type e = 0; e < m_nevent; e++) {
    if (mTimeIsSet[e]) {
      ...
      mNumberOfTimeSteps[e] =
          (mNumberOfTimeSteps[e] == 0) ? 1 : mNumberOfTimeSteps[e];
      // the resulting mDt could be slightly too large, because the
      // numberOfTimeSteps is rounded to the nearest sw4_type
      mDt = (mTmax[e] - mTstart) / mNumberOfTimeSteps[e];
    }
  }
}
...
//-----------------------------------------------------------------------
sw4_type EW::mkdirs(const string &path) {
  ...
  while (token != NULL) {
    ...
    if (statErr == 0) {
      ...
      if (S_ISDIR(statBuf.st_mode)) {
        ...
        cerr << "stat() says: '" << pathsofar.str() << "' is not a directory."
             << endl;
      ...
      if (errno == EACCES) {
        cerr << "Error: **Search permission is denied for one of the "
                "directories in the path prefix of "
             << pathsofar.str() << endl;
        delete[] pathtemparg;
        return -1;
      } else if (errno == ENOTDIR) {
        cerr << "Error: **A component of the path '" << pathsofar.str()
             << "' is not a directory. " << endl;
        delete[] pathtemparg;
        return -1;
        ...
        if (mVerbose >= 2)
          cout << "Info: **stat returned ENOENT (the path does not exist, or "
                  "the path "
               << endl
               << "      is an empty string) " << pathsofar.str() << endl;
      } else {
        if (mVerbose >= 2)
          cout << "Info: **stat returned other error code for path: "
               << pathsofar.str() << endl;
      }
    }
    ...
    if (mkdir(pathsofar.str().c_str(),
              S_IWUSR | S_IXUSR | S_IRUSR | S_IRGRP |
                  S_IXGRP)  // why do we need group permissions?
        == -1) {
      if (mVerbose >= 2) cout << "mkdir() returned an error code." << endl;
      // check error conditions
      if (errno == EEXIST) {
        // can this ever happen since we called stat(), which said that the
        // directory did not exist ???
        if (mVerbose >= 2)
          cout << "Info: ** The directory already exists:" << pathsofar.str()
               << endl;
        ...
        cerr << "Error: **Write permission is denied for the parent directory "
                "in which the new directory is to be added."
             << pathsofar.str() << endl;
      else if (errno == EMLINK)
        cerr << "Error: **The parent directory has too many links (entries)."
             << pathsofar.str() << endl;
      else if (errno == ENOSPC)
        cerr << "Error: **The file system doesn't have enough room to create "
                "the new directory."
             << pathsofar.str() << endl;
      else if (errno == EROFS)
        cerr << "Error: **  The parent directory of the directory being "
                "created is on a read-only file system and cannot be modified."
             << pathsofar.str() << endl;
      else if (errno == ENOSPC)
        cerr << "Error: ** The new directory cannot be created because the "
...
void EW::setup_supergrid() {
  ...
  sw4_type gTop = mNumberOfCartesianGrids - 1;
  //  vector<float_sw4> sg_width(mNumberOfCartesianGrids);
  vector<float_sw4> sg_width(mNumberOfGrids);

  // if the number of grid points was specified in the input file, we need to
  // convert that to a physical width, based on the coarsest grid
  if (!m_use_sg_width) {
  ...
  for (sw4_type g = 0; g < mNumberOfGrids; g++) {
    ...
    m_supergrid_taper_x[g].define_taper((mbcGlobalType[0] == bSuperGrid), 0.0,
                                        (mbcGlobalType[1] == bSuperGrid),
                                        m_global_xmax, sg_width[g]);
    m_supergrid_taper_y[g].define_taper((mbcGlobalType[2] == bSuperGrid), 0.0,
                                        (mbcGlobalType[3] == bSuperGrid),
                                        m_global_ymax, sg_width[g]);
  }
  //  if( topographyExists() )
  //  {
  //     sw4_type g=mNumberOfGrids-1;
  //     m_supergrid_taper_x[g].define_taper( (mbcGlobalType[0] == bSuperGrid),
  //     0.0,
  //                                          (mbcGlobalType[1] == bSuperGrid),
  //                                          m_global_xmax, sg_width[gTop] );
  //     m_supergrid_taper_y[g].define_taper( (mbcGlobalType[2] == bSuperGrid),
  //     0.0,
  //                                          (mbcGlobalType[3] == bSuperGrid),
  //                                          m_global_ymax, sg_width[gTop] );
  //  }
  if (mNumberOfGrids == 1) {
    m_supergrid_taper_z[0].define_taper(
        !topographyExists() && (mbcGlobalType[4] == bSuperGrid), 0.0,
        (mbcGlobalType[5] == bSuperGrid), m_global_zmax, sg_width[0]);
  } else {
    m_supergrid_taper_z[mNumberOfGrids - 1].define_taper(
        !topographyExists() && (mbcGlobalType[4] == bSuperGrid), 0.0, false,
        m_global_zmax, sg_width[gTop]);
    m_supergrid_taper_z[0].define_taper(
        false, 0.0, mbcGlobalType[5] == bSuperGrid, m_global_zmax, sg_width[0]);
    for (sw4_type g = 1; g < mNumberOfGrids - 1; g++)
  ...
  for (sw4_type g = 0; g < mNumberOfGrids; g++) {
    ...
    sw4_type imin = 1 + sgpts, imax = m_global_nx[g] - sgpts, jmin = 1 + sgpts,
        jmax = m_global_ny[g] - sgpts;
    sw4_type kmax = m_global_nz[g] - sgpts;

    // Only grid 0 has super grid boundary at the bottom
    if (g > 0) kmax = m_global_nz[g];

    //     cout << "Active region for backward solver: " << imin+1 << " " <<
    //     imax-1 << " " << jmin+1 << " " << jmax-1
    //	  << " " << 1+1 << " " << kmax-1 << endl;
    ...
    // count.
    if (m_iStartAct[g] > m_iEndAct[g]) m_iStartAct[g] = m_iEndAct[g] + 1;
    if (m_jStartAct[g] > m_jEndAct[g]) m_jStartAct[g] = m_jEndAct[g] + 1;
    if (m_kStartAct[g] > m_kEndAct[g]) m_kStartAct[g] = m_kEndAct[g] + 1;
  }
  ...
  //     m_supergrid_taper_x.prsw4_type_parameters();
  //     m_supergrid_taper_y.prsw4_type_parameters();
  //     m_supergrid_taper_z.prsw4_type_parameters();
  //   }
}
...
void EW::assign_supergrid_damping_arrays() {
  ...
  // new corner taper functions to reduce strength of damping near the edges and
  // corners
...
#define cornerz(k, g) (m_sg_corner_z[g])[k - m_kStart[g]]
  ...
  // Note: compared to WPP2, we don't need to center the damping coefficients on
  // the half-point anymore, because the damping term is now 4th order: D+D-(
  // a(x) D+D- ut(x) )
...
#pragma omp parallel for
        ...
        if (g > topCartesian ||
            (0 < g &&
             g < mNumberOfGrids - 1))  // curvilinear grid or refinement grid.
        {
#pragma omp parallel for
          for (sw4_type k = m_kStart[g]; k <= m_kEnd[g]; k++) {
            dcz(k, g) = 0.;
...
#pragma omp parallel for
    ...
    else {  // standard case starts here

      if (m_energy_test)  // change the eps parameter in the stretching to make
                          // the discrete energy smaller
      {
        for (g = 0; g < mNumberOfGrids; g++) {
...
#pragma omp parallel for
        ...
        if (g > topCartesian ||
            (0 < g &&
             g < mNumberOfGrids - 1))  // Curvilinear or refinement grid
        {
// No supergrid damping in the vertical (k-) direction on a curvilinear or
// refinement grid.
#pragma omp parallel for
          for (sw4_type k = m_kStart[g]; k <= m_kEnd[g]; k++) {
            dcz(k, g) = 0.;
...
#pragma omp parallel for
  ...
  // m_iStart[g], m_iEnd[g]); for ( j = m_jStart[g]; j<=m_jEnd[g]; j++ )
  //    fprintf(fp,"%d %e\n", j, stry(j,g));
  // fclose(fp);
  ...
  // FILE *fp=fopen(fname,"w");
  // printf("Saving tmp file=%s, g=%i, m_iStart=%i, m_iEnd=%i\n", fname, g,
  // m_iStart[g], m_iEnd[g]); for ( i = m_iStart[g]; i<=m_iEnd[g]; i++ )
  //   fprintf(fp,"%i %e %e %e\n", i, cornerx(i,g), strx(i,g),
  //   strx(i,g)*dcx(i,g));
  // fclose(fp);
  ...
  // m_jStart[g], m_jEnd[g]); for ( j = m_jStart[g]; j<=m_jEnd[g]; j++ )
  //   fprintf(fp,"%i %e %e %e\n", j, cornery(j,g), stry(j,g),
  //   stry(j,g)*dcy(j,g));
  // fclose(fp);
  ...
  // m_kStart[g], m_kEnd[g]); for ( k = m_kStart[g]; k<=m_kEnd[g]; k++ )
  //   fprintf(fp,"%i %e %e %e\n", k, cornerz(k,g), strz(k,g),
  //   strz(k,g)*dcz(k,g));
  // fclose(fp);
  // end tmp
...
#pragma omp parallel for
      for (sw4_type j = m_jStart[g]; j <= m_jEnd[g]; j++)
        for (sw4_type i = m_iStart[g]; i <= m_iEnd[g]; i++) {
          ...
          for (sw4_type q = 0; q < m_ghost_points;
               q++)  // only once when m_ghost_points==1
          {
            for (sw4_type c = 1; c <= nc; c++)
              a_mtrl[g](c, i, j, m_kStart[g] + q) =
                  a_mtrl[gc](c, i, j, m_kEnd[gc] - 2 * m_ghost_points + q);
          }
          ...
          {
            for (sw4_type c = 1; c <= nc; c++)
              a_mtrl[gc](c, i, j, m_kEnd[gc] - q) =
                  a_mtrl[g](c, i, j, m_kStart[g] + 2 * m_ghost_points - q);
          }
        }
    }
  }
...
void EW::perturb_velocities(vector<Sarray> &a_vs, vector<Sarray> &a_vp) {
  SW4_MARK_FUNCTION;
  sw4_type g = mNumberOfGrids - 1;
  sw4_type p = m_random_dist / mGridSize[g] + 1;
  sw4_type pz = m_random_distz / mGridSize[g] + 1;
  //   sw4_type p = 2*m_random_dist/mGridSize[g]+1;
  //   sw4_type pz= 2*m_random_distz/mGridSize[g]+1;
  ...
  //
  Sarray saverand(ifirst - p, ilast + p, jfirst - p, jlast + p, kfirst - pz,
                  kfirst + pz);
  ...
  for (g = 0; g < mNumberOfGrids; g++) {
    ...
    kfirst = m_kStart[g];
    klast = m_kEnd[g];
    sw4_type nx = m_global_nx[g];
    sw4_type ny = m_global_ny[g];
    sw4_type nz = m_global_nz[g];
    sw4_type ghost = m_ghost_points;
    ...
    // FTNC      if( m_croutines )
    {
      ...
      {
        randomfield3dc_ci(ifirst, ilast, jfirst, jlast, kfirst, klast, nx, ny,
                          nz, ghost, pert_ptr, wgh_ptr, m_random_dist,
                          m_random_distz, h, mZ[g].c_ptr(), m_random_seed,
                          saverand_ptr, p, pz);
        perturbvelocityc_ci(ifirst, ilast, jfirst, jlast, kfirst, klast, vs_ptr,
                            vp_ptr, pert_ptr, m_random_amp, m_random_amp_grad,
                            mZ[g].c_ptr(), plimit);
      } else {
        randomfield3d_ci(ifirst, ilast, jfirst, jlast, kfirst, klast, nx, ny,
                         nz, ghost, pert_ptr, wgh_ptr, m_random_dist,
                         m_random_distz, h, m_random_seed, saverand_ptr, p, pz);
        perturbvelocity_ci(ifirst, ilast, jfirst, jlast, kfirst, klast, vs_ptr,
                           vp_ptr, pert_ptr, m_random_amp, m_random_amp_grad,
                           m_zmin[g], h, plimit);
      }
    }
    // FTNC      else
    // FTNC      {
    // FTNC	 if( g >= mNumberOfCartesianGrids && topographyExists() ) // NOT
    // verified for several curvilinear grids FTNC	 { FTNC randomfield3dc(
    // &ifirst, &ilast, &jfirst, &jlast, &kfirst, &klast, FTNC
    // &nx, &ny, &nz, &ghost, pert_ptr, wgh_ptr, &m_random_dist, FTNC
    // &m_random_distz, &h, mZ.c_ptr(), m_random_seed, saverand_ptr, &p, &pz );
    // FTNC	    perturbvelocityc( &ifirst, &ilast, &jfirst, &jlast, &kfirst,
    // &klast,
    // FTNC			      vs_ptr, vp_ptr, pert_ptr, &m_random_amp,
    // &m_random_amp_grad, FTNC			      mZ.c_ptr(), &plimit );
    // FTNC	 }
    // FTNC	 else
    // FTNC	 {
    // FTNC	    randomfield3d( &ifirst, &ilast, &jfirst, &jlast, &kfirst,
    // &klast,
    ...
    // &klast,
    // FTNC			     vs_ptr, vp_ptr, pert_ptr, &m_random_amp,
    // &m_random_amp_grad, FTNC			     &m_zmin[g], &h, &plimit );
    // FTNC	 }
    // FTNC      }
  }
}
...
void EW::getDtFromRestartFile() {
  ...
  if (mTimeIsSet[event]) {
    ...
    mNumberOfTimeSteps[event] =
        static_cast<sw4_type>((mTmax[event] - mTstart) / mDt + 0.5);
    mNumberOfTimeSteps[event] =
...
//-----------------------------------------------------------------------
void EW::initial_tw_test(vector<Sarray> &U, vector<Sarray> &Up,
                         vector<Sarray> &F, vector<Sarray> &Mu,
                         vector<Sarray> &Lambda, vector<Sarray> &Lu,
      ...
      getVerbosity() >= 3)  // only do these tests if verbose>=3
  {
    ...
    for (sw4_type g = 0; g < mNumberOfGrids; g++) {
      ...
      for (sw4_type q = 0; q < 6; q++) printf(" bc[%i]=%i", q, m_bcType[g][q]);
      printf("\n");
    }
...
//-----------------------------------------------------------------------
void EW::checkpoint_twilight_test(vector<Sarray> &Um, vector<Sarray> &U,
...
void EW::perturb_rho(Sarray &rho, Sarray &rndpert) {
  for (sw4_type k = rho.m_kb; k <= rho.m_ke; k++)
    for (sw4_type j = rho.m_jb; j <= rho.m_je; j++)
      for (sw4_type i = rho.m_ib; i <= rho.m_ie; i++) {
