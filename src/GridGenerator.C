...
//-----------------------------------------------------------------------
int GridGenerator::metric_ci( int ib, int ie, int jb, int je, int kb, int ke, float_sw4* __restrict__ a_x,
...
// At a grid point, evaluate topography at that point
   ...
   else
   {
      ...
      if( topo.in_range(1,iNear-3,jNear-3,1) &&  topo.in_range(1,iNear+4,jNear+4,1) )
      {
