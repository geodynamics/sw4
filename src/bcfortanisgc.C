...
void EW::bcfortanisg_ci( int ib, int ie, int jb, int je, int kb, int ke, int wind[36], 
...
#pragma omp parallel for 
      ...
      {
	 REQUIRE2( s == 4 || s == 5, "EW::bcfortanisg_ci,  ERROR: Free surface condition"
...
#pragma omp parallel for 
	    for( int j=jb+2 ; j <= je-2 ; j++ )
		  ...
		  size_t ind = i-ib+ni*(j-jb)+nij*(k-kb);
		  float_sw4 du = strx[i-ib]*(d4a*(u[ind+1]-u[ind-1])+d4b*(u[ind+2]-u[ind-2]));
		  float_sw4 dv = strx[i-ib]*(d4a*(u[npts+ind+1]-u[npts+ind-1])+d4b*(u[npts+ind+2]-u[npts+ind-2]));
		  float_sw4 dw = strx[i-ib]*(d4a*(u[2*npts+ind+1]-u[2*npts+ind-1])+d4b*(u[2*npts+ind+2]-u[2*npts+ind-2]));

		  float_sw4 rhs1 = c[ind+2*npts]*du+c[ind+7*npts ]*dv+c[ind+11*npts]*dw;
		  float_sw4 rhs2 = c[ind+4*npts]*du+c[ind+9*npts ]*dv+c[ind+13*npts]*dw;
		  float_sw4 rhs3 = c[ind+5*npts]*du+c[ind+10*npts]*dv+c[ind+14*npts]*dw;

		  du = stry[j-jb]*(d4a*(u[ind+ni]  -u[ind-ni])+
					     d4b*(u[ind+2*ni]-u[ind-2*ni]));
		  dv = stry[j-jb]*(d4a*(u[npts+ind+ni]  -u[npts+ind-ni])+
					     d4b*(u[npts+ind+2*ni]-u[npts+ind-2*ni]));
		  dw = stry[j-jb]*(d4a*(u[2*npts+ind+ni  ]-u[2*npts+ind-ni  ])+
					     d4b*(u[2*npts+ind+2*ni]-u[2*npts+ind-2*ni]));
		  rhs1 += c[ind+7*npts ]*du+c[ind+12*npts]*dv+c[ind+13*npts]*dw;
		  rhs2 += c[ind+9*npts ]*du+c[ind+16*npts]*dv+c[ind+18*npts]*dw;
		  rhs3 += c[ind+10*npts]*du+c[ind+17*npts]*dv+c[ind+19*npts]*dw;
		  ...
		  rhs1 += c[ind+11*npts]*du+c[ind+13*npts]*dv+c[ind+14*npts]*dw-h*bforce5[  3*qq];
		  rhs2 += c[ind+13*npts]*du+c[ind+18*npts]*dv+c[ind+19*npts]*dw-h*bforce5[1+3*qq];
		  rhs3 += c[ind+14*npts]*du+c[ind+19*npts]*dv+c[ind+20*npts]*dw-h*bforce5[2+3*qq];
 // Solve system for ghost point values
		  ...
		  if( info != 0 )
		     cout << "ERROR in bcfortanisosg_ci, call to DGESV returned info " << info << endl;
		  u[       ind-nij] = -s0i*x[0];
...
#pragma omp parallel for 
	    for( int j=jb+2 ; j <= je-2 ; j++ )
		  ...
		  rhs1 += c[ind+11*npts]*du+c[ind+13*npts]*dv+c[ind+14*npts]*dw-h*bforce6[  3*qq];
		  rhs2 += c[ind+13*npts]*du+c[ind+18*npts]*dv+c[ind+19*npts]*dw-h*bforce6[1+3*qq];
		  rhs3 += c[ind+14*npts]*du+c[ind+19*npts]*dv+c[ind+20*npts]*dw-h*bforce6[2+3*qq];
