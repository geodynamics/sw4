...
class TimeSeries {
 public:
  // support for derived quantities of the time derivative are not yet
  // implemented
  enum receiverMode {
    Displacement,
    Div,
    ...
    Strains,
    Velocity,
    DisplacementGradient /*, DivVelo, CurlVelo, StrainsVelo */
  };

  TimeSeries(EW* a_ew, std::string fileName, std::string staName,
             receiverMode mode, bool sacFormat, bool usgsFormat, float_sw4 x,
             float_sw4 y, float_sw4 z, bool topoDepth, int writeEvery,
             bool xyzcomponent = true);
  ~TimeSeries();
  ...
  float_sw4 misfit(TimeSeries& observed, TimeSeries* diff, float_sw4& dshift,
 ...
 private:
  ...
  // ignore this station if it is above the topography or outside the
  ...
  // components (m_xyzcomponent=true).
  bool m_xyzcomponent;
