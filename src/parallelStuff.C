...
void EW::setup2D_MPICommunications() {
  ...
  //    MPI_Type_vector( nj, m_ppadding,    ni,    m_mpifloat,
  //    &m_send_type_2dfinest[0] ); MPI_Type_vector( 1,  m_ppadding*ni, ni*nj,
  //    m_mpifloat, &m_send_type_2dfinest[1] ); MPI_Type_commit(
  //    &m_send_type_2dfinest[0] ); MPI_Type_commit( &m_send_type_2dfinest[1] );
  ...
  //    &m_send_type_2dfinest_ext[0] ); MPI_Type_vector( 1,  extpadding*ni,
  //    ni*nj, m_mpifloat, &m_send_type_2dfinest_ext[1] );
  ...
  // m_mpifloat, &m_send_type_2dy[g] ); 	 MPI_Type_vector( 3*nj, 1,    ni,
  // m_mpifloat, &m_send_type_2dx1p[g] ); 	 MPI_Type_vector( 3,    ni, ni*nj,
  // m_mpifloat, &m_send_type_2dy1p[g] ); 	 MPI_Type_vector( 3*nj, 3,  ni,
  // m_mpifloat, &m_send_type_2dx3p[g] ); 	 MPI_Type_vector( 3,    3*ni, ni*nj,
  // m_mpifloat, &m_send_type_2dy3p[g] );
  //       }
  //       else
  //       {
  // 	 MPI_Type_vector( nj, 3*m_ppadding,    3*ni,    m_mpifloat,
  // &m_send_type_2dx[g] ); 	 MPI_Type_vector( 1,  3*m_ppadding*ni, 3*ni*nj,
  // m_mpifloat, &m_send_type_2dy[g] ); 	 MPI_Type_vector( nj, 3,    3*ni,
  // m_mpifloat, &m_send_type_2dx1p[g] ); 	 MPI_Type_vector( 1,  3*ni, 3*ni*nj,
  // m_mpifloat, &m_send_type_2dy1p[g] ); 	 MPI_Type_vector( nj, 3*3,    3*ni,
  // m_mpifloat, &m_send_type_2dx3p[g] ); 	 MPI_Type_vector( 1,  3*3*ni, 3*ni*nj,
  // m_mpifloat, &m_send_type_2dy3p[g] );
  //       }
...
void EW::setupMPICommunications() {
  ...
  for (int g = 0; g < mNumberOfGrids; g++) {
    ...
    // FTNC      else
    // FTNC      {
    // FTNC	 MPI_Type_vector( nj*nk, 3*m_ppadding, 3*ni, m_mpifloat,
    // &m_send_type3[2*g] ); FTNC	 MPI_Type_vector( nk, 3*m_ppadding*ni,
    // 3*ni*nj, m_mpifloat, &m_send_type3[2*g+1] ); FTNC FTNC MPI_Type_vector(
    // nj*nk, 4*m_ppadding, 4*ni, m_mpifloat, &m_send_type4[2*g] ); FTNC
    // MPI_Type_vector( nk, 4*m_ppadding*ni, 4*ni*nj, m_mpifloat,
    // &m_send_type4[2*g+1] ); FTNC FTNC	 MPI_Type_vector( nj*nk, 21*m_ppadding,
    // 21*ni, m_mpifloat, &m_send_type21[2*g] ); FTNC	 MPI_Type_vector( nk,
    // 21*m_ppadding*ni, 21*ni*nj, m_mpifloat, &m_send_type21[2*g+1] ); FTNC }
...
void EW::communicate_array_2d(Sarray& u, int g, int k) {
  ...
  u2d.copy_kplane(u, k);
  // X-direction communication
  MPI_Sendrecv(&u2d(1, ie - (2 * m_ppadding - 1), jb, k), 1, m_send_type_2dx[g],
               m_neighbor[1], xtag1, &u2d(1, ib, jb, k), 1, m_send_type_2dx[g],
  ...
  MPI_Sendrecv(&u2d(1, ib + m_ppadding, jb, k), 1, m_send_type_2dx[g],
               ...
               m_cartesian_communicator, &status);
  // Y-direction communication
  MPI_Sendrecv(&u2d(1, ib, je - (2 * m_ppadding - 1), k), 1, m_send_type_2dy[g],
               m_neighbor[3], ytag1, &u2d(1, ib, jb, k), 1, m_send_type_2dy[g],
               m_neighbor[2], ytag1, m_cartesian_communicator, &status);
...
//-----------------------------------------------------------------------
void EW::my_node_core_rank(int Cx, int Cy, int Nx, int Ny, int& cx, int& cy,
                           int& nx, int& ny) {
  //
...
int EW::my_node_core_id(int ni, int nj, int proc_max[2]) {
  ...
  if (proc_zero())
    std::cout << " Cx x Cy = " << Cx << " " << Cy << " Nx x Ny = " << Nx << " "
              << Ny << std::endl;
  int cx, cy, nx, ny;
  ...
  int myrank;
  MPI_Comm_rank(MPI_COMM_WORLD, &myrank);
  std::cout << "old 1d= " << myrank << " new (cx,cy,nx,ny)= " << cx << " " << cy
            << " " << nx << " " << ny << std::endl;
