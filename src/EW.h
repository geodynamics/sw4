...
class EW {
 public:
  ...
  // void solve_allpars( vector<Source*> & a_GlobalSources, vector<Sarray>&
  // a_Rho, vector<Sarray>& a_Mu, 		    vector<Sarray>& a_Lambda, vector<TimeSeries*> &
  //a_GlobalTimeSeries, 		    vector<Sarray>& a_U, vector<Sarray>& a_Um,
  //vector<DataPatches*>& Upred_saved_sides, 		    vector<DataPatches*>&
  //Ucorr_saved_sides, bool save_sides );
  ...
  // 			    vector<Sarray*>& a_AlphaVEm,
  // 			    vector<Sarray>& a_Up, vector<Sarray>& a_U, vector<Sarray>&
  // a_Um, double a_t ); void updateMemoryVariablesBndry( vector<Sarray*>&
  // a_AlphaVEp, 			    vector<Sarray*>& a_AlphaVEm, 			    vector<Sarray>& a_Up,
  // vector<Sarray>& a_U, vector<Sarray>& a_Um );
  ...
  bool curvilinear_grid_mapping(float_sw4 q, float_sw4 r, float_sw4 s, int g,
  ...
  // a_Um, Sarray& Unext, 				    int g, int kic, float_sw4 t,
  //std::vector<GridPointSource*> point_sources ); void
  // compute_preliminary_predictor( Sarray& a_Up, Sarray& a_U, Sarray& Unext, 				    int
  //g, int kic, float_sw4 t, std::vector<GridPointSource*> point_sources ); void
  ...
  //Lambdac, Sarray& Rhoc, float_sw4 hc, 	       float_sw4 cof, int gc, int gp, int
  //is_periodic[2] ); void check_corrector( Sarray& Uf, Sarray& Uc, Sarray&
  ...
  // kfirst, int klast, 		      float_sw4* __restrict__ a_alp, 		      float_sw4* __restrict__
  //a_alm, float_sw4* __restrict__ a_up, 		      float_sw4* __restrict__ a_u, float_sw4*
  ...
  void set_to_zero_at_source(vector<Sarray>& a_U,
                             vector<GridPointSource*> point_sources,
 ...
 private:
  ...
  int m_eStart, m_eEnd;
  int m_event_in_proc;  // Event number [0,nevent) in this proc, when using
                        // parallel events
