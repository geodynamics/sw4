...
void CurvilinearInterface2::init_arrays(vector<float_sw4*>& a_strx,
  ...
  for (size_t ind = 0; ind < msize; ind++) {
    ...
    (&three, &m_mass_block[9 * ind], &three, &m_ipiv_block[3 * ind], work,
     &lwork, &info);
    if (info != 0) {
      int j = ind / m_Mass_block.m_ni + m_Mass_block.m_jb;
      int i =
...
void CurvilinearInterface2::impose_ic(std::vector<Sarray>& a_U, float_sw4 t,
  ...
  while (maxres > m_reltol * maxres0 && scalef * maxres > m_abstol &&
    ...
    for (int j = m_Mass_block.m_jb; j <= m_Mass_block.m_je; j++)
      for (int i = m_Mass_block.m_ib; i <= m_Mass_block.m_ie; i++) {
        ...
        float_sw4 x1, x2, x3;
        float_sw4 b1 = residual(1, i, j, 1), b2 = residual(2, i, j, 1),
                  b3 = residual(3, i, j, 1);
        x1 = m_mass_block[9 * ind] * b1 + m_mass_block[9 * ind + 3] * b2 +
             m_mass_block[9 * ind + 6] * b3;
        x2 = m_mass_block[9 * ind + 1] * b1 + m_mass_block[9 * ind + 4] * b2 +
             m_mass_block[9 * ind + 7] * b3;
        x3 = m_mass_block[9 * ind + 2] * b1 + m_mass_block[9 * ind + 5] * b2 +
  ...
  //   if( m_ew->getRank() == 0 )
  //      cout << "maxres " <<  maxres << " scaled " << scalef*maxres  << "
  //      rellim " << m_reltol*maxres0 << " it= " << iter << endl;
