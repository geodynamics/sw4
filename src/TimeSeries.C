...
TimeSeries::TimeSeries(EW* a_ew, std::string fileName, std::string staName,
                       ...
                       bool topoDepth, int writeEvery, bool xyzcomponent)
    : m_ew(a_ew),
      m_mode(mode),
      m_nComp(0),
      m_myPoint(false),
  ...
  if (a_ew->topographyExists()) {
    ...
    if (!a_ew->interpolate_topography(q, r, m_zTopo, true)) {
      cerr << "Unable to evaluate topography for receiver station" << m_fileName
           << " mX= " << mX << " mY= " << mY << endl;
  ...
  if (m_zRelativeToTopography) {
    ...
    m_zRelativeToTopography = false;  // set to false so the correction isn't
                                      // repeated (e.g. by the copy function)
  }
...
float_sw4 TimeSeries::misfit2(TimeSeries& observed) {
  ...
  if (m_myPoint) {
    ...
    misfit = 0;
  // Return the negative misfit, to make it a quantity to minimize, for
  // conformity with the L2 misfit.
  return -0.5 * misfit;
}

//-----------------------------------------------------------------------
TimeSeries* TimeSeries::copy(EW* a_ew, string filename, bool addname) {
  if (addname) filename = m_fileName + filename;

  TimeSeries* retval = new TimeSeries(
      a_ew, filename, m_staName, m_mode, m_sacFormat, m_usgsFormat, mX, mY, mZ,
      m_zRelativeToTopography, mWriteEvery, m_xyzcomponent);
  retval->m_t0 = m_t0;
  retval->m_dt = m_dt;
  ...
  if (m_myPoint) {
    ...
    // Headers are ok, get the data
    {
      ...
      if (!azfail && !incfail) {
        ...
        {
          m_xyzcomponent =
              false;  // note this is format on output file,
