...
{ 
  ...
  for (decltype(start.i) i = start.i+ threadIdx.x + blockIdx.x * blockDim.x; i < end.i;
       i += blockDim.x * gridDim.x)
    for (decltype(start.j) j = start.j + threadIdx.y + blockIdx.y * blockDim.y; j < end.j;
	 j += blockDim.y * gridDim.y)
      for (decltype(start.k) k= start.k + threadIdx.z + blockIdx.z * blockDim.z; k < end.k;
	   k += blockDim.z * gridDim.z)
	f(i,j,k);
