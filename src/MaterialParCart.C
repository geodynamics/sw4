...
MaterialParCart::MaterialParCart(EW* a_ew, int nx, int ny, int nz, int init,
  ...
  for (int g = 0; g < m_ew->mNumberOfGrids; g++) {
    ...
    if (m_myrank == 0 && verbose > 0)
      std::cout << "active region, index = " << m_ew->m_iStartActGlobal[g]
                << " " << m_ew->m_iEndActGlobal[g] << " "
                << m_ew->m_jStartActGlobal[g] << " " << m_ew->m_jEndActGlobal[g]
  ...
  if (!m_global) {
    ...
    //   std::cout<<"IN GLOBAL "<<ncomp<<" "<< (m_ieint - m_ibint + 1)<<"
    //   "<<(m_jeint - m_jbint + 1)<<" "<<(m_keint - m_kbint + 1)<<"\n";
  } else {
    // Global arrays
  ...
  //  std::cout<<"MaterialPArCart ["<<m_global<<"] "<<m_nmd<<"
  //  "<<m_nms<<"\n"<<std::flush;
}
//-----------------------------------------------------------------------
void MaterialParCart::limit_x(int nmd, double* xmd, int nms, double* xms,
  ...
  if (m_variables == 1) {
    std::cout << "MaterialParCart::Limit_x, variables=1 case is not implemented"
              << std::endl;
...
bool MaterialParCart::compute_overlap(bool force_shared) {
  ...
  int icb = 10000000, ice = -100, jcb = 10000000, jce = -100, kcb = 10000000,
      kce = -100;
  ...
  //   std::cout << m_myrank << " First sweep I " << icb << " " << ice <<
  //   std::endl;
    ...
    while (!done && ngh < maxghost) {
      ...
      for (int j = jcb + ngh; j <= jce - ngh; j++)
        for (int i = icb + ngh; i <= ice - ngh; i++) {
          ...
          //               m_ew->computeNearestLowGridPoint( ig, jg, kg, g, x,
          //               y, z );
...
//-----------------------------------------------------------------------
void MaterialParCart::get_parameters(int nmd, double* xmd, int nms, double* xms,
                                     std::vector<Sarray>& a_rho,
                                     std::vector<Sarray>& a_mu,
...
//-----------------------------------------------------------------------
void MaterialParCart::interpolate_gradient(int g, Sarray& a_gradrho,
                                           Sarray& a_gradmu,
                                           Sarray& a_gradlambda,
                                           Sarray& gradc) {
  SW4_MARK_FUNCTION;
...
//-----------------------------------------------------------------------
void MaterialParCart::get_gradient(
  ...
  if (m_global) {
    get_gradient_shared(nms, xms, dfs, a_rho, a_mu, a_lambda, a_gradrho,
                        a_gradmu, a_gradlambda);
  } else {
    if (m_variables != 1) {
        ...
        if (m_ew->m_iStart[g] <= ig && ig + 1 <= m_ew->m_iEnd[g] &&
          ...
          if (cs(i, j, k) < 0 || cp(i, j, k) < 0) {
            std::cout << "Unphysical interpolation: " << wghx << " " << wghy
                      << " " << wghz << std::endl;
            std::cout << "z= " << z << " h= " << h << " g= " << g
                      << " kg= " << kg << std::endl;
            std::cout << "cs = " << cs(i, j, k) << " " << csdiff[g](ig, jg, kg)
                      << " " << csdiff[g](ig, jg, kg + 1) << std::endl;
            std::cout << "cp = " << cp(i, j, k) << " " << cpdiff[g](ig, jg, kg)
  ...
  for (int k = m_kbint; k <= m_keint; k++)
    for (int j = m_jbint; j <= m_jeint; j++)
      for (int i = m_ibint; i <= m_ieint; i++) {
        ...
        //            else
        //            {
        //               std::cout << "MaterialParCart::interpolate_hessian,
        //               error point "
        //                         << ig << " " << jg << " " << kg << " not in
        //                         processor " <<
