...
//-----------------------------------------------------------------------
MaterialIfile::MaterialIfile(EW* a_ew, std::string fileName,
                             bool CartesianFormat) {
  mEw = a_ew;

  // In general an ifile command only specifies properties down to a certain
  // depth
...
void MaterialIfile::set_material_properties(std::vector<Sarray>& rho,
  ...
  if (mEw->m_materials.size() < m_number_material_surfaces) {
    cerr << "set_materials: ERROR: There are " << m_number_material_surfaces
         << " material surfaces but only " << mEw->m_materials.size()
  ...
  {
    sw4_type kLow = mEw->m_kStart[g];
    if (g ==
        mEw->mNumberOfGrids - 1)  // no topography, so k=1 is at the top surface
      kLow = 1;
      ...
      for (sw4_type j = mEw->m_jStartSw4_Type[g]; j <= mEw->m_jEndSw4_Type[g]; j++) {
        for (sw4_type i = mEw->m_iStartSw4_Type[g]; i <= mEw->m_iEndSw4_Type[g]; i++) {
          ...
          if (m_mat_Cartesian) {
            ...
            if (true) {
              // sw4_typeerpolate material surfaces to find which material is at the
              // specified depth
              sw4_type materialID = getCartesianMaterialID(x, y, depth);
            ...
            // need to calculate (lon, lat) from (x, y) before calling
            // getMaterialID
            mEw->computeGeographicCoord(x, y, lon, lat);
            if (inside_material_surfaces(lat, lon)) {
              // sw4_typeerpolate material surfaces to find which material is at the
              // specified depth
              sw4_type materialID = getMaterialID(lat, lon, depth);
              if (0 <= materialID && materialID < mEw->m_materials.size()) {
                MaterialProperty* mprop = mEw->m_materials[materialID];
                definedPoints += 1;
                rho[g](i, j, k) = lookup_Rho(mprop, depth);
...
// Gradient and quadratic terms: Assume given (vp, vs, rho) constants are values
// at the previous depth surface.
//      double zsurf = 0; // set to zero for the first ifile command NEED TO
//      GENERALIZE
#pragma omp parallel for reduction(+ : totalPoints, definedPoints)
      for (sw4_type k = kLow; k <= mEw->m_kEnd[g];
           k++)  // don't attempt querying the ifile above the topography (start
                 // at k=1)
      {
        for (sw4_type j = mEw->m_jStart[g]; j <= mEw->m_jEnd[g]; j++) {
          for (sw4_type i = mEw->m_iStart[g]; i <= mEw->m_iEnd[g]; i++) {
            ...
            if (m_mat_Cartesian) {
              if (inside_cartesian_material_surfaces(x, y)) {
                // sw4_typeerpolate material surfaces to find which material is at
                // the specified depth
                sw4_type materialID = getCartesianMaterialID(x, y, depth);
                if (0 <= materialID && materialID < mEw->m_materials.size()) {
                  MaterialProperty* mprop = mEw->m_materials[materialID];
  ...
  if (mEw->proc_zero())
    cout << "MaterialIfile:: set_material_properties: Total # points="
         << totalPointsSum << ". Defined properties in # points=" << materialSum
         << endl;
...
void MaterialIfile::extractSurfaceFromGridFile(string a_surfaceFileName) {
  ...
  sw4_type Nlon, Nlat, Nmat, i, j, k;

  ifstream gridFile(a_surfaceFileName.c_str());
  if (!gridFile.is_open()) {
    cerr << "# Error opening ifile surface file: '" << a_surfaceFileName << "'"
         << endl;
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  ...
  token = strtok(buffer, " \t");
  if (token != NULL) Nlon = atoi(token);
  if ((token = strtok(NULL, " \t")) != NULL) Nlat = atoi(token);
  if ((token = strtok(NULL, " \t")) != NULL) Nmat = atoi(token);

  VERIFY2(
      Nlon > 0 && Nlat > 0 && Nmat > 0,
      "ERROR: MaterialIfile:: extractSurfaceFromGridFile: Nlon, Nlat, Nmat= "
          << Nlon << " " << Nlat << " " << Nmat);
  ...
  for (j = 1; j <= Nlat; j++)
    for (i = 1; i <= Nlon; i++) {
      ...
      //      gridFile >> m_materialLon[i] >> m_materialLat[j] >>
      //      m_materialDepth(1,i,j,1);
      missingValues = false;
      sw4_type nValues = 0;
      ...
      if (missingValues) {
        printf(
            "extractSurfaceFromGridFile:: Fatal error: Expecting %i values on "
            "line %i, but only found: %i\n",
  ...
  gridFile.close();
  if (mEw->proc_zero()) printf("Nlon=%i Nlat=%i Nmat=%i\n", Nlon, Nlat, Nmat);

  m_materialLonMax = -1e30, m_materialLonMin = 1e30, m_materialLatMax = -1e30,
  m_materialLatMin = 1e30;
  float_sw4 depthMax = -1e10, depthMin = 1e10;
  for (i = 1; i <= Nlon; i++) {
    if (m_materialLon[i] < m_materialLonMin)
      m_materialLonMin = m_materialLon[i];
    if (m_materialLon[i] > m_materialLonMax)
      m_materialLonMax = m_materialLon[i];
  }
  for (i = 1; i <= Nlat; i++) {
    if (m_materialLat[i] < m_materialLatMin)
      m_materialLatMin = m_materialLat[i];
    if (m_materialLat[i] > m_materialLatMax)
      m_materialLatMax = m_materialLat[i];
  }

  for (j = 1; j <= Nlat; j++)
    for (i = 1; i <= Nlon; i++)
      for (k = 1; k <= Nmat; k++) {
        if (m_materialDepth(k, i, j, 1) < depthMin)
          depthMin = m_materialDepth(k, i, j, 1);
        if (m_materialDepth(k, i, j, 1) > depthMax)
          depthMax = m_materialDepth(k, i, j, 1);
      }
  if (mEw->proc_zero())
    printf(
        "Material sw4_typeerface surfaces: lonMin=%e, lonMax=%e\nlatMin=%e, "
        "latMax=%e\ndepthMin=%e, depthMax=%e\n",
        m_materialLonMin, m_materialLonMax, m_materialLatMin, m_materialLatMax,
        depthMin, depthMax);

  // If the lat vector is not in increasing order, we need to reorder it
  if (m_materialLat[1] > m_materialLat[Nlat]) {
    if (mEw->proc_zero()) printf("Reordering the latitude vector...\n");
    for (j = 1; j <= Nlat / 2; j++) {
  ...
  if (m_materialLon[1] > m_materialLon[Nlon]) {
    if (mEw->proc_zero()) printf("Reordering the longitude vector...\n");
    for (i = 1; i <= Nlon / 2; i++) {
...
void MaterialIfile::extractSurfaceFromCartesianFile(string a_surfaceFileName) {
  ...
  gridFile.getline(buffer, 256);
  token = strtok(buffer, " \t");
  if (token != NULL) Nx = atoi(token);
  if ((token = strtok(NULL, " \t")) != NULL) Ny = atoi(token);
  if ((token = strtok(NULL, " \t")) != NULL) Nmat = atoi(token);

  VERIFY2(Nx > 0 && Ny > 0 && Nmat > 0,
          "ERROR: EW:: extractSurfaceFromCartesianFile: Nx, Ny, Nmat= "
              << Nx << " " << Ny << " " << Nmat);
  ...
  for (j = 1; j <= Ny; j++)
    for (i = 1; i <= Nx; i++) {
      // only works for one surface
      //      gridFile >> m_mat_Xvec[i] >> m_mat_Yvec[j] >>
      //      m_materialDepth(1,i,j,1);
      missingValues = false;
      sw4_type nValues = 0;
      ...
      if (missingValues) {
        printf(
            "extractSurfaceFromCartesianFile:: Fatal error: Expecting %i "
            "values on line %i, but only found: %i\n",
  ...
  if (mEw->proc_zero()) printf("Nx=%i Ny=%i Nmat=%i\n", Nx, Ny, Nmat);
  ...
  if (mEw->proc_zero())
    printf(
        "Material sw4_typeerface surfaces: Xmin=%e, Xmax=%e\nYmin=%e, "
        "Ymax=%e\ndepthMin=%e, depthMax=%e\n",
  ...
  if (m_mat_Yvec[1] > m_mat_Yvec[Ny]) {
    if (mEw->proc_zero()) printf("Reordering the Y-coordinate vector...\n");
    for (j = 1; j <= Ny / 2; j++) {
  ...
  if (m_mat_Xvec[1] > m_mat_Xvec[Nx]) {
    if (mEw->proc_zero()) printf("Reordering the X-coordinate vector...\n");
    for (i = 1; i <= Nx / 2; i++) {
...
//-----------------------------------------------------------------------
sw4_type MaterialIfile::getMaterialID(double lat, double lon, float_sw4 depth) {
  // Sw4_Typeerpolate the materialDepth surfaces to find which material is at the
  // specified depth

  // 1. Sw4_Typeerpolate in the grid file to get elevations on the computational grid
  ...
  if (lat > m_materialLatMax || lat < m_materialLatMin ||
    ...
    printf(
        "Warning: MaterialIfile::getMaterialID lon=%e, lat=%e is outside the "
        "material surface grid\n",
        lon, lat);
    //      MPI_Abort(MPI_COMM_WORLD,1);
    return -1;
  }
  i0 = 1 + (sw4_type)((lon - m_materialLonMin) / deltaLon);
  j0 = 1 + (sw4_type)((lat - m_materialLatMin) / deltaLat);
  ...
  // Bi-linear sw4_typeerpolation for all surfaces
  sw4_type materialID = -1;  // default material is unknown
  float_sw4 minDepth = 0, maxDepth;
  for (sw4_type q = 1; q <= m_number_material_surfaces;
       q++)  // surfaces are assumed to be in increasing depth order
  {
    maxDepth = (1.0 - eta) * ((1.0 - xi) * m_materialDepth(q, i0, j0, 1) +
                              xi * m_materialDepth(q, i0 + 1, j0, 1)) +
               eta * ((1.0 - xi) * m_materialDepth(q, i0, j0 + 1, 1) +
                      xi * m_materialDepth(q, i0 + 1, j0 + 1, 1));
...
//-----------------------------------------------------------------------
sw4_type MaterialIfile::getCartesianMaterialID(float_sw4 xP, float_sw4 yP,
                                          float_sw4 depth) {
  // sw4_typeerpolate the materialDepth surfaces to find which material is at the
  // specified depth

  // Sw4_Typeerpolate in the grid file to get elevations on the computational grid
  // allow extrapolation (for ghost points)
  // if (yP > m_mat_Ymax || yP < m_mat_Ymin || xP > m_mat_Xmax || xP <
  // m_mat_Xmin)
  // {
  //    printf("Warning: MaterialIfile::getCartesianMaterialID xP=%e, yP=%e is
  //    outside the material surface grid\n", xP, yP);
  //    //      MPI_Abort(MPI_COMM_WORLD,1);
  //    return -1;
  // }
  float_sw4 deltaY = (m_mat_Ymax - m_mat_Ymin) / m_mat_Ny;
  float_sw4 deltaX = (m_mat_Xmax - m_mat_Xmin) / m_mat_Nx;
  sw4_type i0 = 1 + (sw4_type)((xP - m_mat_Xmin) / deltaX);
  sw4_type j0 = 1 + (sw4_type)((yP - m_mat_Ymin) / deltaY);
  ...
  // find the right sw4_typeerval
  while (i0 >= 1 && i0 < m_mat_Nx &&
         (xP < m_mat_Xvec[i0] || m_mat_Xvec[i0 + 1] < xP)) {
    if (xP < m_mat_Xvec[i0])
      i0--;
    else if (xP > m_mat_Xvec[i0 + 1])
      i0++;
  }
  while (j0 >= 1 && j0 < m_mat_Ny &&
         (yP < m_mat_Yvec[j0] || m_mat_Yvec[j0 + 1] < yP)) {
    if (yP < m_mat_Yvec[j0])
      j0--;
       ...
       q++)  // surfaces are assumed to be in increasing depth order
  {
    ...
    if (maxDepth >= minDepth + .1 && depth <= maxDepth &&
        depth >=
            minDepth)  // maxDepth>=minDepth+.1 removes zero thickness layers
    {
      materialID = q - 1;
      break;
    }
