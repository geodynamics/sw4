...
//-----------------------------------------------------------------------
Source::~Source() {
  if (mNpar > 0) delete[] mPar;
  if (mNipar > 0) delete[] mIpar;
}
...
void Source::correct_Z_level(EW* a_ew) {
  ...
  int i, j, k, g;
  // preliminary determination of the nearest grid point (already did this in
  // the constructor)
  ...
  if (m_myPoint) {
    ...
    //    if (!a_ew->interpolate_topography(q, r, zTopoLoc, true)) // used the
    //    smoothed topography
    if (!a_ew->m_gridGenerator->interpolate_topography(
            a_ew, mX0, mY0, zTopoLoc,
            a_ew->mTopoGridExt))  // used the smoothed topography
    {
      cerr << "Unable to evaluate topography for source at X= " << mX0
           << " Y= " << mY0 << " Z= " << mZ0 << endl;
  ...
  MPI_Allreduce(&zTopoLoc, &m_zTopo, 1, a_ew->m_mpifloat, MPI_MAX,
                a_ew->m_1d_communicator);

  // printf("Proc #%i: z-coordinate of topo = %e, relative-z = %i\n",
  // a_ew->getRank(), m_zTopo, 	 (int) m_zRelativeToTopography);
  ...
  //     int success = a_ew->computeNearestGridPoint2( iloc, jloc, kloc, gloc,
  //     mX0, mY0, mZ0 ); if( success != 1 || i != iloc || j != jloc )
  //     {
  //        std::cout << "ZCorr " << a_ew->getRank() << " i= " << i <<  " j= "
  //        << j <<  " k= " << k << " g= " << g <<  " " << m_myPoint << " " <<
  //        success << " " << mX0
  //               << " " << mY0 << " " << mZ0 << std::endl;
  ...
  //     q0, r0, s0))
  //         //      if (!a_ew->invert_curvilinear_grid_mapping(mX0, mY0, mZ0,
  //         q0, r0, s0))
  //     {
  //	cerr << "Unable to invert curvilinear mapping for source at X= "
  //             << mX0 << " Y= " << mY0 << " Z= " << mZ0 << endl;
  ...
  if (gridrefbndry) {
    ...
    if (lowerbndry) {
      ...
      if (kc == 1) {
        ...
        //wghk[2] << " " << wghk[3] << " " << wghk[4] << " " << wghk[5] << endl;
