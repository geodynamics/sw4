...
void TestTwilight::get_mula(Sarray& mu, Sarray& lambda, Sarray& x, Sarray& y,
  ...
  SW4_MARK_FUNCTION;
  if (m_sw4twilight) {
    for (sw4_type k = mu.m_kb; k <= mu.m_ke; k++)
      for (sw4_type j = mu.m_jb; j <= mu.m_je; j++)
        for (sw4_type i = mu.m_ib; i <= mu.m_ie; i++) {
          mu(i, j, k) =
...
void TestTwilight::get_ubnd(Sarray& u_in, Sarray& x_in, Sarray& y_in, Sarray& z_in,
                            float_sw4 t, sw4_type npts, sw4_type sides[6]) {
  SW4_MARK_FUNCTION;
  ...
  for (sw4_type s = 0; s < 6; s++)
    if (sides[s] == 1) {
      sw4_type kb = u_in.m_kb, ke = u_in.m_ke, jb = u_in.m_jb, je = u_in.m_je, ib = u_in.m_ib,
          ie = u_in.m_ie;
      if (s == 0) ie = ib + npts - 1;
      if (s == 1) ib = ie - npts + 1;
      if (s == 2) je = jb + npts - 1;
...
void TestTwilight::get_ubnd(Sarray& u, float_sw4 h, float_sw4 zmin, float_sw4 t,
                            sw4_type npts, sw4_type sides[6]) {
  SW4_MARK_FUNCTION;
  SYNC_STREAM;
  for (sw4_type s = 0; s < 6; s++)
    if (sides[s] == 1) {
...
void TestTwilight::get_mula_att(Sarray& muve, Sarray& lambdave, Sarray& x,
  ...
  SW4_MARK_FUNCTION;
  if (m_sw4twilight) {
    for (sw4_type k = muve.m_kb; k <= muve.m_ke; k++)
      for (sw4_type j = muve.m_jb; j <= muve.m_je; j++)
        for (sw4_type i = muve.m_ib; i <= muve.m_ie; i++) {
          muve(i, j, k) =
...
//   for (sw4_type s = 0; s < 6; s++)
//     if (sides[s] == 1) {
//       sw4_type kb = AlphaVE_i.m_kb, ke = AlphaVE_i.m_ke, jb = AlphaVE_i.m_jb,
...
void TestTwilight::get_bnd_att(Sarray& AlphaVE_in, Sarray& x_in, Sarray& y_in, Sarray& z_in,
  ...
  for (sw4_type s = 0; s < 6; s++)
    if (sides[s] == 1) {
      sw4_type kb = AlphaVE_in.m_kb, ke = AlphaVE_in.m_ke, jb = AlphaVE_in.m_jb,
