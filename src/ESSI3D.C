...
#else
  if (m_rank == 0)
    std::cout << "Using SSI output but SW4 is not compiled with HDF5" << std::endl;
#endif
  return;
}
    ...
    float_sw4 a_time, Sarray& a_Z )
{
  ...
  if (m_dumpInterval > 0) {
    ...
    if (m_compressionMode > 0) 
      m_hdf5helper->init_write_vel(m_isRestart, nstep, m_compressionMode, m_compressionPar, m_bufferInterval);
    else
      m_hdf5helper->init_write_vel(m_isRestart, nstep, 0, 0.0, m_bufferInterval);
  }
  else{
    if (m_compressionMode > 0) 
      m_hdf5helper->init_write_vel(m_isRestart, m_ntimestep, m_compressionMode, m_compressionPar, m_bufferInterval);
    else
      m_hdf5helper->init_write_vel(m_isRestart, m_ntimestep, 0, 0.0, m_bufferInterval);
  }
