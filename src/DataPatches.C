...
DataPatches::DataPatches(string fname, Sarray& u, int imin, int imax, int jmin,
  ...
  // Side 3
  intersect(ib, ie, jb, je, kb, ke, imin - off, imax + off, jmin - off, jmin,
            kmin, kmax, wind);
  add_patch(wind);

  // Side 4
  intersect(ib, ie, jb, je, kb, ke, imin - off, imax + off, jmax, jmax + off,
            kmin, kmax, wind);
  add_patch(wind);

  // Side 1
  //   u.intersection(imin-off,imin,jmin,jmax,1,kmax,wind);
  intersect(ib, ie, jb, je, kb, ke, imin - off, imin, jmin, jmax, kmin, kmax,
            wind);
  add_patch(wind);

  // Side 2
  //   u.intersection(imax,imax+off,jmin,jmax,1,kmax,wind);
  intersect(ib, ie, jb, je, kb, ke, imax, imax + off, jmin, jmax, kmin, kmax,
            wind);
  add_patch(wind);
