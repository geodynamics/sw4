...
DataPatches::DataPatches(string fname, Sarray& u, int imin, int imax, int jmin,
                         int jmax, int kmax, int layers, int ntsteps, double dt,
                         int npad[6]) {
  m_filename = fname;
  ...
  m_npatches = 0;
  int wind[6];
  intersect(ib, ie, jb, je, kb, ke, imin - off, imax + off, jmin - off,
            jmax + off, kmax, kmax + off, wind);
  add_patch(wind);

  // Side 3
  intersect(ib, ie, jb, je, kb, ke, imin - off, imax + off, jmin - off, jmin, 1,
            kmax, wind);
  add_patch(wind);

  // Side 4
  intersect(ib, ie, jb, je, kb, ke, imin - off, imax + off, jmax, jmax + off, 1,
            kmax, wind);
  add_patch(wind);

  // Side 1
  //   u.intersection(imin-off,imin,jmin,jmax,1,kmax,wind);
  intersect(ib, ie, jb, je, kb, ke, imin - off, imin, jmin, jmax, 1, kmax,
            wind);
  add_patch(wind);

  // Side 2
  //   u.intersection(imax,imax+off,jmin,jmax,1,kmax,wind);
  intersect(ib, ie, jb, je, kb, ke, imax, imax + off, jmin, jmax, 1, kmax,
            wind);
  add_patch(wind);
  ...
  if (m_isnonempty) {
    ...
    for (int p = 0; p < m_npatches; p++) {
      size_t npts = m_ncomp * (m_dims[6 * p + 1] - m_dims[6 * p] + 1) *
                    (m_dims[6 * p + 3] - m_dims[6 * p + 2] + 1) *
    ...
    for (int n = 0; n < m_nsteps; n++) m_data[n] = new double[totdim];
    m_ncurrent = 0;
  ...
  if (m_ncurrent > 0 && !m_error && m_isnonempty) {
    ...
    if (!m_startedsave) {
      ...
      if (m_steps[0] != m_nmax + 1) {
        printf(
            "ERROR DataPatches::save_to_file : Trying to store non-consequtive "
            "time steps\n");
        return;
      } else
        m_nmax = m_steps[m_ncurrent - 1];
      ...
      if (nr == -1) {
        printf(
            "ERROR DataPatches::save_to_file : Could not find beginning of "
            "file\n");
        return;
      }
      nr = write(fd, &m_nmax, sizeof(int));
...
void DataPatches::pop(Sarray& u, int n) {
  ...
  if (m_isnonempty && !m_error) {
    ...
    if (!(m_steps[0] <= n && n <= m_steps[m_ncurrent - 1])) {
      ...
      // Assume solving backwards, read the entire interval:  n-nsteps+1 .., n
      // into 0,..,nsteps-1
      read_from_file(n);
      //	 nloc = m_ncurrent-1;
    }
    ...
    nloc = m_ncurrent - 1;
    while (nloc >= 0 && m_steps[nloc] != n) nloc--;
    //      if( nloc < 0 )
...
void DataPatches::read_from_file(int n) {
  if (!m_error && m_isnonempty) {
    ...
    if (fd == -1) {
      printf(
          "ERROR DataPatches::read_from_file : file %s could not be opened \n",
          m_filename.c_str());
      print_openerr(errno);
      return;
    }
    ...
    // header
    size_t off =
        4 * sizeof(int) + 6 * m_npatches * sizeof(size_t) + sizeof(double);
    ...
    for (int s = 0; s < m_ncurrent; s++) {
      ...
      while (nrread < npts) {
        ...
        if (nread > linuxlimit) nread = linuxlimit;
...
void DataPatches::intersect(int ib, int ie, int jb, int je, int kb, int ke,
                            int ib2, int ie2, int jb2, int je2, int kb2,
                            int ke2, int wind[6]) {
  wind[0] = max(ib, ib2);
  wind[1] = min(ie, ie2);
  wind[2] = max(jb, jb2);
