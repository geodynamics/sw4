...
//-----------------------------------------------------------------------
void projectmtrl( int ib,  int ie,  int jb,  int je,  int kb,  int ke, 
		  int iba, int iea, int jba, int jea, int kba, int kea,
		  float_sw4* __restrict__ rho, float_sw4* __restrict__ mu, 
		  float_sw4* __restrict__ lambda, float_sw4 dt,
		  float_sw4 h, float_sw4 cfl, float_sw4 vsmin, float_sw4 rhoscale,
...
//-----------------------------------------------------------------------
void projectmtrlc(int ib,  int ie,  int jb,  int je,  int kb,  int ke, 
		  int iba, int iea, int jba, int jea, int kba, int kea,
		  float_sw4* __restrict__ rho, float_sw4* __restrict__ mu,
		  float_sw4* __restrict__ lambda, float_sw4 dt,
		  float_sw4* __restrict__ met, float_sw4* __restrict__ jac,
		  float_sw4 cfl, float_sw4 vsmin, 
		  float_sw4 rhoscale, float_sw4 muscale, float_sw4 lascale, int& info )
{
//*********************************************************************
//*
	 ...
	 for( int i=iba ; i <= iea ; i++ )
	 {
	    ...
	    if( lambda[ind] < 0 )
	    {
	       ...
	       zcorr = 1;
            ...
            float_sw4 as2 = SQR(met[ind+nijk])+SQR(met[ind+2*nijk])+SQR(met[ind+3*nijk]);
...
void checkmtrl( int ib,  int ie,  int jb,  int je,  int kb,  int ke,
		...
		float_sw4 dt, float_sw4 h, float_sw4 limits[10] )
{
   //-----------------------------------------------------------------------
...
// routine to enforce material speed limits and positive density
{
   ...
   for( int g=0 ; g < mNumberOfGrids ; g++ )
   {
	 ...
	 projectmtrlc( ifirst,    ilast,    jfirst,    jlast,    kfirst,    klast,
      ...
      {
	 // Cartesian
	 projectmtrl( ifirst,    ilast,    jfirst,    jlast,    kfirst,    klast,
		      ifirstact, ilastact, jfirstact, jlastact, kfirstact, klastact,
		      rhop, mup, lap, mDt, mGridSize[g], mCFLmax, vsmin, rhoscale,
		      muscale, lascale, info );
      }
      if( info != 0 )
...
// bulk-modulus = 2*mu+3*lambda
	 ...
	 if( limits[4] < 0 )
      ...
      //cerr << "limits[6]=" << limits[6] << " mCFLmax^2=" << mCFLmax*mCFLmax << " limits[8]=" << limits[8] << endl;
