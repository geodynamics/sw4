...
void MaterialParCartesian::get_material(int nmd, double* xmd, int nms,
  ...
  for (int g = 0; g < m_ew->mNumberOfGrids; g++) {
    m_ew->interpolate(m_nx, m_ny, m_nz, m_xmin, m_ymin, m_zmin, m_hx, m_hy,
                      m_hz, m_rho, m_mu, m_lambda, g, a_rho[g], a_mu[g],
                      a_lambda[g], true);
  }
}
...
void MaterialParCartesian::interpolate_parameters(
  ...
  m_ew->interpolate_to_coarse(m_nx, m_ny, m_nz, m_xmin, m_ymin, m_zmin, m_hx,
                              m_hy, m_hz, m_rho, m_mu, m_lambda, a_rho, a_mu,
                              a_lambda, true);
  double* rhop = m_rho.c_ptr();
  double* mup = m_mu.c_ptr();
  double* lambdap = m_lambda.c_ptr();
  ...
  for (int k = 1; k <= m_nz; k++)
    for (int j = 1; j <= m_ny; j++)
      for (int i = 1; i <= m_nx; i++) {
        ...
        if (rhop[indm] < rhmin) rhmin = rhop[indm];
        if (mup[indm] < mumin) mumin = mup[indm];
        if (lambdap[indm] < lamin) lamin = lambdap[indm];
        if (rhop[indm] > rhmax) {
          rhmax = rhop[indm];
          //               cout << " rhomax found " << m_myrank << " value " <<
          //               rhop[ind] << " at " << i << " " << j << " " << k <<
          //               endl;
        }
        if (mup[indm] > mumax) mumax = mup[indm];
        if (lambdap[indm] > lamax) lamax = lambdap[indm];
        ind++;
      }
  //   cout << "proc " << m_myrank << " rho,mu,lambda min " << rhmin << " "
  //   <<mumin << " " << lamin << endl; cout << "proc " << m_myrank << "
  //   rho,mu,lambda max " << rhmax << " " <<mumax << " " << lamax << endl;
}
...
void MaterialParCartesian::get_parameters(int nmd, double* xmd, int nms,
  ...
  if (m_init == 0) {
    ...
    //      cout << " hx, hy, hz " << m_hx  <<  " " << m_hy << " " << m_hz <<
    //      endl; cout << " nx, ny, nz " << m_nx  <<  " " << m_ny << " " << m_nz
    //      << endl; cout << " xmin, xmax " << m_xmin  <<  " " << m_xmax <<
    //      endl; cout << " ymin, ymax " << m_ymin  <<  " " << m_ymax << endl;
    ...
    mpfile.interpolate_to_other(xms, 1, m_nx, m_ny, m_nz, m_hx, m_hy, m_hz,
                                m_xmin, m_ymin, m_zmin);
    if (!mpfile.is_update()) subtract_base_mtrl(nms, xms);
  }
}

//-----------------------------------------------------------------------
void MaterialParCartesian::get_gradient(
...
void MaterialParCartesian::interpolate_pseudohessian(int nmpars, double* phs,
  ...
  for (int i = 0; i < 3 * npts; i++) tmp[i] = phs[i];
...
//-----------------------------------------------------------------------
ssize_t MaterialParCartesian::local_index(size_t ind_global) { return -1; }
...
//-----------------------------------------------------------------------
void MaterialParCartesian::project_and_write(std::vector<Sarray>& a_rho,
                                             std::vector<Sarray>& a_mu,
                                             std::vector<Sarray>& a_lambda,
                                             std::string fname) {
  ...
  for (int i = 0; i < m_nx * m_ny * m_nz; i++) xms[3 * i] = projarray[i];

  // Project mu
  projectl2(a_mu, projarray);
  for (int i = 0; i < m_nx * m_ny * m_nz; i++)
    projarray[i] = projarray[i] > mu_min ? projarray[i] : mu_min;
  for (int i = 0; i < m_nx * m_ny * m_nz; i++) xms[3 * i + 1] = projarray[i];
  ...
  for (int i = 0; i < m_nx * m_ny * m_nz; i++) xms[3 * i + 2] = projarray[i];
  MParGridFile mpfile(xms, 1, m_nx, m_ny, m_nz, m_hx, m_hy, m_hz, m_xmin,
                      m_ymin, m_zmin, 0);

  //  for( int i=0 ; i < m_nx*m_ny*m_nz ; i++ )
  //     if( xms[3*i] < 0 || xms[3*i+1] < 0 || xms[3*i+2]<0 )
  //       cout << "0:negval " << i << " " << xms[3*i] <<" " << xms[3*i+1] << "
  //       " << xms[3*i+2] << endl;
...
void MaterialParCartesian::projectl2(std::vector<Sarray>& mtrl,
  ...
  //
  // parameter grid is x_0, x_1,..,x_{nx}, x_{nx+1}, where x_1,..,x_{nx} carry
  // degrees of freedom. The dimensions are such that x_0=xmin, x_{nx+1}=xmax.
  //
  // x_j = j*hx+xmin, j=0,..,nx+1  --> hx=(xmax-xmin)/(nx+1)
  //
  // In the z-direction, z_0 carries a degree of freedom, we set
  //   z_k = k*hz+zmin-hz, k=1,..,nz are still degrees of freedom, k=0 and
  //   k=nz+1 are
  ...
  CHECK_INPUT(nproc == 1,
              "ERROR: Projectl2 only implemented for single processor");
  ...
  for (int i = 0; i < Ntot; i++) {
    rhs[i] = 0;
    for (int j = 0; j < Ntot; j++) mat[j + Ntot * i] = 0;
  }
  double ihx = 1.0 / m_hx;
  double ihy = 1.0 / m_hy;
  double ihz = 1.0 / m_hz;
            ...
            for (int m3 = 1; m3 <= m_nz; m3++)
              for (int m2 = 1; m2 <= m_ny; m2++)
                for (int m1 = 1; m1 <= m_nx; m1++) {
                  ...
                  if (std::abs(xpar1 - x) < m_hx &&
                      std::abs(ypar1 - y) < m_hy &&
                      std::abs(zpar1 - z) < m_hz) {
                    ...
                    for (int l3 = m3l; l3 <= m3u; l3++)
                      for (int l2 = m2l; l2 <= m2u; l2++)
                        for (int l1 = m1l; l1 <= m1u; l1++) {
                          ...
                          if (std::abs(xpar2 - x) < m_hx &&
                              std::abs(ypar2 - y) < m_hy &&
                              std::abs(zpar2 - z) < m_hz) {
                            int l = l1 - 1 + m_nx * (l2 - 1) +
                                    m_nx * m_ny * (l3 - 1);
                            double phil;
  ...
  if (info != 0)
    cout << "ERROR: in MaterialParCartesian::projectl2,  info = " << info
         << " from dgesv " << endl;
  delete[] ipiv;
  delete[] mat;
...
void MaterialParCartesian::subtract_base_mtrl(int nms, double* xms) {
  ...
  m_ew->interpolate_base_to_coarse(m_nx, m_ny, m_nz, m_xmin, m_ymin, m_zmin,
                                   m_hx, m_hy, m_hz, rhobase, mubase, labase);
  size_t ind = 0;
