...
void SuperGrid::define_taper(bool left, float_sw4 leftStart, bool right,
                             float_sw4 rightEnd, float_sw4 width) {
  m_left = left;
  m_x0 = leftStart;
  m_right = right;
  m_x1 = rightEnd;
  ...
  if (m_left || m_right) {
    ...
    CHECK_INPUT(
        m_width > 0.,
        "The supergrid taper width must be positive, not = " << m_width);
    CHECK_INPUT(
        m_width < dlen,
        "The supergrid taper width must be smaller than the domain, not = "
            << m_width);
    CHECK_INPUT(m_trans_width > 0.,
                "The supergrid taper transition width must be positive, not = "
                    << m_trans_width);
    ...
    if (m_x0 > m_x1 - m_width) {
      ...
      CHECK_INPUT(false,
                  ...
                  "than the domain. Here x0 = "
                      << m_x0 << " and x1-width = " << m_x1 - m_width);
    }
  }
}
...
float_sw4 SuperGrid::stretching(
    float_sw4 x) const {  // this function satisfies 0 < epsL <= f <= 1
  return 1 - (1 - m_epsL) * PsiAux(x);  // PsiAux(x) = psi(x) in our papers
}

float_sw4 SuperGrid::PsiAux(float_sw4 x) const {  // PsiAux = psi in our papers
...
float_sw4 SuperGrid::PsiDamp(float_sw4 x) const {  // PsiAux = psi in our papers
  // this function is zero for m_x0+m_width <= x <= m_x1-m_width
  // and one for x=m_x0 and x=m_x1
  float_sw4 f = 0.;
  if (m_left && x < m_x0 + m_width)
    // the following makes the damping transition in 0 < const_width <= x <=
    // const_width+trans_width = m_width constant damping in 0 <= x <=
    // const_width
    f = Psi0((m_x0 + m_width - x) / m_trans_width);
  else if (m_right && x > m_x1 - m_width)
    // the following makes the damping transition in m_x1-m_width < x < m_x1 -
    // const_width < m_x1 constant damping in m_x1 - const_width <= x <= m_x1
...
float_sw4 SuperGrid::Psi0(float_sw4 xi) const {
  ...
  if (xi <= 0.)
    ...
    f = xi * xi * xi * xi * xi * xi *
        (462 - 1980 * xi + 3465 * xi * xi - 3080 * xi * xi * xi +
         1386 * xi * xi * xi * xi - 252 * xi * xi * xi * xi * xi);
  // one-sided C5 fcn (p2)
...
void SuperGrid::set_twilight(float_sw4 omega) { m_tw_omega = omega; }

void SuperGrid::set_eps(float_sw4 new_eps) { m_epsL = new_eps; }
