...
class EtreeFile : public MaterialData {
 public:
#ifdef ENABLE_ETREE
  EtreeFile(EW* ew, const std::string& access, const std::string& file,
            const std::string& xfile, const std::string& model,
            const std::string& log, const std::string& query, const double res);
  ...
  void getbox(double& latmin, double& latmax, double& lonmin,
              double& lonmax) const;
  void getcorners(double& latse, double& lonse, double& latsw, double& lonsw,
                  double& latne, double& lonne, double& latnw,
                  double& lonnw) const;
 ...
 protected:
  // This is a very crude test! The actual box is rotated relative to the North
  // and East directions
  inline bool inside(double lat, double lon, double elev) {
    return m_latmin <= lat && lat <= m_latmax && m_lonmin <= lon &&
           lon <= m_lonmax && m_elevmin <= elev && elev <= m_elevmax;
  }
  ...
  void readEFile(std::vector<Sarray>& rho, std::vector<Sarray>& cs,
                 std::vector<Sarray>& cp, std::vector<Sarray>& qs,
                 std::vector<Sarray>& qp, int& outside, int& material);
