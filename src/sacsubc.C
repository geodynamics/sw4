...
const char *rstr[] = {
    ...
    "EVLO    ", "EVEL    ", "EVDP    ", "FHDR40  ", "USER0   ", "USER1   ",
...
const char *istr[] = {
    ...
    "NXSIZE  ", "NYSIZE  ", "NHDR15  ", "IFTYPE  ", "IDEP    ", "IZTYPE  ",
    "IHDR4   ", "IINST   ", "ISTREG  ", "IEVREG  ", "IEVTYP  ", "IQUAL   ",
...
const char *estr[] = {
    ...
    "IT6     ", "IT7     ", "IT8     ", "IT9     ", "IRADNV  ", "ITANNV  ",
    "IRADEV  ", "ITANEV  ", "INORTH  ", "IEAST   ", "IHORZA  ", "IDOWN   ",
    "IUP     ", "ILLLBB  ", "IWWSN1  ", "IWWSN2  ", "IHGLP   ", "ISRO    ",
...
const char *Istr[] = {
    ...
    "IZTYPE  ", "IZTYPE  ", "IZTYPE  ", "IZTYPE  ", "IZTYPE  ", "IZTYPE  ",
    "IZTYPE  ", "IZTYPE  ", "IZTYPE  ", "IZTYPE  ", "IRADNV  ", "ITANNV  ",
    "IRADEV  ", "ITANEV  ", "INORTH  ", "IEAST   ", "IHORZA  ", "IDOWN   ",
    "IUP     ", "ILLLBB  ", "IWWSN1  ", "IWWSN2  ", "IHGLP   ", "ISRO    ",
    "IEVTYP  ", "IEVTYP  ", "IEVTYP  ", "IEVTYP  ", "IEVTYP  ", "IEVTYP  ",
    "IEVTYP  ", "IQUAL   ", "IQUAL   ", "IQUAL   ", "IQUAL   ", "IQUAL   ",
    "ISYNTH  ", "IDEP    "};
...
*/
{
  ...
  *nerr = 0;
  ...
  if (fread(*data, sizeof(float), maxpts, fptr) != maxpts)
    fprintf(stderr, "ERROR ins sacsubc.C line 192\n");
  fclose(fptr);
  /* safety */
  if (sachdr.ihdr[9] > LN) {
    maxpts = LN;
...
*/
{
  ...
  if (fwrite(data, sizeof(float), LN, fptr) != LN)
    fprintf(stderr, "ERROR in sacsubc.C line 225\n");
  fclose(fptr);
}
...
*/
{
  ...
  *nerr = 0;
  /* this is ugly - SAC ascii was originally a FORTRAN specification
     scanf likes field separators so problems with reading a FORTRAN 4g15.7
  */
...
*/
{
  ...
  j2 = 5;
  for (i = 0; i < 8; i++) {
    for (j = j1; j < j2; j++) fprintf(fptr, "%10d", sachdr.ihdr[j]);
    fprintf(fptr, "\n");
    j1 += 5;
    j2 += 5;
  }
...
/* initialize sac header */
{
  ...
  sachdr.ihdr[7] = 0;
  sachdr.ihdr[8] = 0;
  for (i = 35; i < 40; i++) sachdr.ihdr[i] = 0;
...
int streql(const char *str1, const char *str2) {
  ...
  str_2[8] = '\0';
  if (strncmp(str_1, str_2, 8) == 0) return (True);
  return (False);
}
