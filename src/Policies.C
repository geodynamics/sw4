...
int main() {
  ...
  //   std::cout << "\n";
  //   delete[] d;
  forallasync(0, N, [=] __host__ __device__(int i) { d[i] = i; });
  ...
  forallasync(0, N, [=] __host__ __device__(int i) { a[i] = 3 * b[i] + d[i]; });

  for (int i = 0; i < 25; i++) {
    forallasync(0, N,
                [=] __host__ __device__(int i) { a[i] = 3 * b[i] + d[i]; });
...
#define D(i, j, k) d[i + M * j + M * M * k]
  for (int i = 0; i < 30; i++) {
    ...
    forall3X<1024>(
        0, M, 0, M, 0, M, [=] __host__ __device__(int i, int j, int k) {
          double x = static_cast<double>(i) / N;
          D(i, j, k) = i * i + sin(x) + cos(x) + tan(x) + i % 10 + i % 999;
        });
  }
...
#define B(i, j, k) b[i + M * j + M * M * k]
  for (int i = 0; i < 35; i++) {
    ...
    forall3X<1024>(0, M, 0, M, 0, M,
                   [=] __host__ __device__(int i, int j, int k) {
                     A(i, j, k) = 3 * B(i, j, k) + D(i, j, k);
                   });
  }
  ...
  for (int i = 0; i < N; i++)
    for (int j = 0; j < N; j++) {
      I = (i << 8) ^ j;
      I = i;
      // std::cout<<sizeof(int)<<" "<<i<<" "<<j<<" "<<" In hex
      // "<<std::hex<<I<<"\n";
      I = i << 8;
      // std::cout<<std::dec<<sizeof(int)<<" "<<i<<" "<<j<<" "<<" In hex
      // "<<std::hex<<I<<"\n";
      I = I ^ j;
      // std::cout<<std::dec<<sizeof(int)<<" "<<i<<" "<<j<<" "<<" In hex
      // "<<std::hex<<I<<"\n";
      ...
      int ii = (I >> 8) & 0xff;
      if ((i != ii) || (j != jj)) {
        std::cout << std::dec << sizeof(int) << " OUT  " << ii << " " << jj
                  << " "
                  << " In hex " << std::hex << I << "\n";
      }
    }
