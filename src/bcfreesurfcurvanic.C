    ...
    for (sw4_type j = jfirst + 2; j <= jlast - 2; j++)
      for (sw4_type i = ifirst + 2; i <= ilast - 2; i++) {
        ...
        float_sw4 du = strx[i - ifirst] * (d4a * (u[ind + 1] - u[ind - 1]) +
                                           d4b * (u[ind + 2] - u[ind - 2]));
        float_sw4 dv =
            strx[i - ifirst] * (d4a * (u[npts + ind + 1] - u[npts + ind - 1]) +
                                d4b * (u[npts + ind + 2] - u[npts + ind - 2]));
        float_sw4 dw = strx[i - ifirst] *
                       (d4a * (u[2 * npts + ind + 1] - u[2 * npts + ind - 1]) +
                        d4b * (u[2 * npts + ind + 2] - u[2 * npts + ind - 2]));

        float_sw4 rhs1 = c[ind + 27 * npts] * du + c[ind + 30 * npts] * dv +
                         c[ind + 33 * npts] * dw;
        float_sw4 rhs2 = c[ind + 28 * npts] * du + c[ind + 31 * npts] * dv +
                         c[ind + 34 * npts] * dw;
        float_sw4 rhs3 = c[ind + 29 * npts] * du + c[ind + 32 * npts] * dv +
                         c[ind + 35 * npts] * dw;
        ...
        rhs1 += c[ind + 36 * npts] * du + c[ind + 39 * npts] * dv +
                c[ind + 42 * npts] * dw;
        rhs2 += c[ind + 37 * npts] * du + c[ind + 40 * npts] * dv +
                c[ind + 43 * npts] * dw;
        rhs3 += c[ind + 38 * npts] * du + c[ind + 41 * npts] * dv +
                c[ind + 44 * npts] * dw;
        ...
        rhs1 += c[ind + 12 * npts] * du + c[ind + 13 * npts] * dv +
                c[ind + 14 * npts] * dw - bforce5[3 * qq];
        rhs2 += c[ind + 13 * npts] * du + c[ind + 15 * npts] * dv +
                c[ind + 16 * npts] * dw - bforce5[1 + 3 * qq];
...
#pragma omp parallel for
    for (sw4_type j = jfirst + 2; j <= jlast - 2; j++)
      for (sw4_type i = ifirst + 2; i <= ilast - 2; i++) {
        ...
        rhs1 += c[ind + 12 * npts] * du + c[ind + 13 * npts] * dv +
                c[ind + 14 * npts] * dw - bforce6[3 * qq];
        rhs2 += c[ind + 13 * npts] * du + c[ind + 15 * npts] * dv +
                c[ind + 16 * npts] * dw - bforce6[1 + 3 * qq];
        rhs3 += c[ind + 14 * npts] * du + c[ind + 16 * npts] * dv +
    ...
    cout << "ERROR in bcfreesurfcurvani_ci, b.c. not implemented for side= "
         << side << endl;
}
