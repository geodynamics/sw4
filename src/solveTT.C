...
//std::cout << "mp xmin=" << mp->getXmin() << " hx=" << mp->getDx() << " nx=" << mp->getNX() << std::endl;
   ...
   switch(wave_mode) {
   case 0:  // P-wave only
        a_TimeSeries[ig]->set_window(timep[iz*nx*ny+iy*nx+ix], timep[iz*nx*ny+iy*nx+ix], 
       timep[iz*nx*ny+iy*nx+ix], timep[iz*nx*ny+iy*nx+ix]);
       ...
       if(myrank==0) fprintf(fd, "%d   %d\t%s\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", 
         ...
         break;
   case 1:  // S-wave only
       a_TimeSeries[ig]->set_window(times[iz*nx*ny+iy*nx+ix], times[iz*nx*ny+iy*nx+ix], 
       times[iz*nx*ny+iy*nx+ix], times[iz*nx*ny+iy*nx+ix]);
