...
//
// NEW: loop over all curvilinear grids and allocate space + estimate the number of grid points in z   
//
...
// tmp
      ...
      communicate_array( mMetric[g], g );
      communicate_array( mJ[g], g );
...
//FTNC         gridinfo(&Bx, &Nx, &By, &Ny, &Bz, &Nz, mMetric[g].c_ptr(), mJ[g].c_ptr(), &minJ, &maxJ );
      ...
      if (mVerbose>3 && proc_zero())
         printf("*** Jacobian of metric, curvi grid g=%d: minJ = %e maxJ = %e\n", g, minJglobal, maxJglobal);
...
#pragma omp parallel for reduction(min:mZmin) reduction(max:mZmax)
  for (int j=m_jStart[gTop]; j<=m_jEnd[gTop]; j++)
    for (int i=m_iStart[gTop]; i<=m_iEnd[gTop]; i++)
    {
       float_sw4 hRatio = (mZ[gTop](i,j,k)-mZ[gTop](i,j,k-1))/mGridSize[gTop]; // TEMPORARY FIX
...
//-----------------------------------------------------------------------
bool EW::invert_curvilinear_grid_mapping( float_sw4 X0, float_sw4 Y0, float_sw4 Z0, float_sw4& q, float_sw4& r, float_sw4& s )
{
...
//  Z0 = zTopCart - h; // to make computeNearestGridPoint think we are in the curvilinear grid FIX THIS
//  computeNearestGridPoint(iNear, jNear, kNear, g, X0, Y0, Z0);

  computeNearestTopoGridPoint(iNear, jNear, X0, Y0);
...
//         return false;
    ...
    {
//      computeNearestLowGridPoint(i, j, k, g, X0, Y0, Z0);
      computeLowTopoGridPoint(i, j, X0, Y0);
...
//    }
  ...
  }// end general case: interpolating mTopoGridExt array  
  Z0 = -tau;
  return true;
}
