...
void MaterialSfile::set_material_properties(std::vector<Sarray> &rho,
  ...
  for (sw4_type g = 0; g < m_npatches; g++) {
    ist[g] = (sw4_type)ceil((double)mInterface[g].m_ni / mInterface[g + 1].m_ni);
    jst[g] = (sw4_type)ceil((double)mInterface[g].m_nj / mInterface[g + 1].m_nj);
  }
...
#pragma omp parallel for reduction(+ : material, outside)
      ...
      for (sw4_type j = mEW->m_jStartSw4_Type[g]; j <= mEW->m_jEndSw4_Type[g]; ++j) {
        for (sw4_type i = mEW->m_iStartSw4_Type[g]; i <= mEW->m_iEndSw4_Type[g]; ++i) {
          ...
          if (m_zminloc <= z && z <= m_zmaxloc) {
            ...
            tmph = down_z - z0;
            if (z > down_z) z = down_z;
            if (z < z0) z = z0;

            // Update the current vertical grid height and z-base with the sfile
            // curvilinear grid
            hv = tmph / (m_nk[gr] - 1);
            ...
            if (k0 <= m_kfirst[gr]) k0 = m_kfirst[gr];
            ...
            if (wghz > 1 || wghz < 0) {
              if (wghz > 1.001 || wghz < -0.001)
                printf(
                    "g=%d, sw4 (%d, %d, %d), mat (%d, %d, %d) wghz = %.2f, "
                    "z=%.2f, z0=%.2f\n",
            ...
            if (use_q) {
              xip[g](i, j, k) =
                  (1 - wghz) *
                      ((1 - wghy) *
                           ((1 - wghx) * mMaterial_qp[gr](1, i0, j0, k0) +
                            wghx * mMaterial_qp[gr](1, i0 + 1, j0, k0)) +
                       wghy *
                           ((1 - wghx) * mMaterial_qp[gr](1, i0, j0 + 1, k0) +
                            wghx * mMaterial_qp[gr](1, i0 + 1, j0 + 1, k0))) +
                  wghz *
                      ((1 - wghy) *
                           ((1 - wghx) * mMaterial_qp[gr](1, i0, j0, k0 + 1) +
                            wghx * mMaterial_qp[gr](1, i0 + 1, j0, k0 + 1)) +
                       wghy * ((1 - wghx) *
                                   mMaterial_qp[gr](1, i0, j0 + 1, k0 + 1) +
                               wghx * mMaterial_qp[gr](1, i0 + 1, j0 + 1,
                                                       k0 + 1)));

              xis[g](i, j, k) =
                  (1 - wghz) *
                      ((1 - wghy) *
                           ((1 - wghx) * mMaterial_qs[gr](1, i0, j0, k0) +
                            wghx * mMaterial_qs[gr](1, i0 + 1, j0, k0)) +
                       wghy *
                           ((1 - wghx) * mMaterial_qs[gr](1, i0, j0 + 1, k0) +
                            wghx * mMaterial_qs[gr](1, i0 + 1, j0 + 1, k0))) +
                  wghz *
                      ((1 - wghy) *
                           ((1 - wghx) * mMaterial_qs[gr](1, i0, j0, k0 + 1) +
                            wghx * mMaterial_qs[gr](1, i0 + 1, j0, k0 + 1)) +
                       wghy * ((1 - wghx) *
                                   mMaterial_qs[gr](1, i0, j0 + 1, k0 + 1) +
                               wghx * mMaterial_qs[gr](1, i0 + 1, j0 + 1,
                                                       k0 + 1)));
            }
  ...
  if (mEW->getRank() == 0)
    ...
    //           << endl;
    cout << endl
         << "sfile command: outside = " << outsideSum
         << ", material = " << materialSum << endl;
}
...
void MaterialSfile::read_sfile() {
  ...
  CHECK_INPUT(
      ...
      "ERROR: sfile azimuth must be equal "
  ...
  if (!m_outside) {
    ...
    for (sw4_type p = 0; p < m_npatches; p++) {
      ...
      if (mEW->getVerbosity() >= 3) {
        ...
        cout << "patch nr global size " << m_ni[p] << " x " << m_nj[p] << " x "
             << m_nk[p] << endl;
      }
    }
  } else {
  ...
  for (sw4_type p = 0; p < m_npatches; p++) {
    try {
      if (!m_isempty[p]) {
        ...
        mMaterial_cp[p].define(1, m_ifirst[p], m_ilast[p], m_jfirst[p],
                               m_jlast[p], m_kfirst[p], m_klast[p]);
        mMaterial_cs[p].define(1, m_ifirst[p], m_ilast[p], m_jfirst[p],
                               m_jlast[p], m_kfirst[p], m_klast[p]);
        if (m_use_attenuation) {
          mMaterial_qp[p].define(1, m_ifirst[p], m_ilast[p], m_jfirst[p],
                                 m_jlast[p], m_kfirst[p], m_klast[p]);
          mMaterial_qs[p].define(1, m_ifirst[p], m_ilast[p], m_jfirst[p],
                                 m_jlast[p], m_kfirst[p], m_klast[p]);
        }
      }
    } catch (bad_alloc &ba) {
  ...
  for (sw4_type p = 0; p < m_npatches + 1; p++) {
    ...
    mInterface[p].define(1, 1, (sw4_type)dim[0], 1, (sw4_type)dim[1], 1, 1);
  ...
  for (sw4_type p = 0; p < m_npatches; p++) {
    if (!m_isempty[p]) {
       ...
       * %d, %d), %d points\n\n", */
      /*         mEW->getRank(), start[0], start[1], start[2], local[0],
       * local[1], local[2], */
      ...
      ierr = H5Sselect_hyperslab(filespace_id, H5S_SELECT_SET, h5_start, NULL,
                                 h5_count, NULL);
      ASSERT(ierr >= 0);

      // Read each var individually
      ierr =
          H5Dread(rho_id, h5_dtype, memspace_id, filespace_id, dxpl, in_data);
      ASSERT(ierr >= 0);
      if (prec == 4) {
        mMaterial_rho[p].assign(f_data);
      } else {
        mMaterial_rho[p].assign(d_data);
      }
      ...
      if (prec == 4) {
        ...
        mMaterial_cs[p].assign(d_data);
      }

      if (m_use_attenuation) {
        ierr =
            H5Dread(qp_id, h5_dtype, memspace_id, filespace_id, dxpl, in_data);
        ASSERT(ierr >= 0);
        if (prec == 4) {
          mMaterial_qp[p].assign(f_data);
        } else {
          mMaterial_qp[p].assign(d_data);
        }

        ierr =
            H5Dread(qs_id, h5_dtype, memspace_id, filespace_id, dxpl, in_data);
        ASSERT(ierr >= 0);
        if (prec == 4) {
          mMaterial_qs[p].assign(f_data);
        } else {
          mMaterial_qs[p].assign(d_data);
        }
      }
  ...
  time_end = MPI_Wtime();
  if (mEW->getRank() == 0) {
    /* cout << "MaterialSfile::read_sfile, time to read interface: " << sw4_typef_end
          ...
          for (sw4_type i = mMaterial_cs[p].m_ib; i <= mMaterial_cs[p].m_ie; i++) {
            ...
            if (mMaterial_cs[p](1, i, j, k0) < 0) {
              ...
              if (p < m_npatches - 1) {
                ...
                mMaterial_rho[p](1, i, j, k0) =
                    mMaterial_rho[pd](1, id, jd, kd);
                mMaterial_cp[p](1, i, j, k0) = mMaterial_cp[pd](1, id, jd, kd);
                mMaterial_cs[p](1, i, j, k0) = mMaterial_cs[pd](1, id, jd, kd);
                if (m_use_attenuation) {
                  mMaterial_qp[p](1, i, j, k0) =
                      mMaterial_qp[pd](1, id, jd, kd);
                  mMaterial_qs[p](1, i, j, k0) =
                      mMaterial_qs[pd](1, id, jd, kd);
                }
              } else {
                printf(
            ...
            if (crat < cratmin) {
              ...
              if (prsw4_typesmallcpcs && crat < 1.41) {
                ...
                cout << " material is " << mMaterial_rho[p](1, i, j, k) << " "
                     << mMaterial_cp[p](1, i, j, k) << " "
                     << mMaterial_cs[p](1, i, j, k) << " "
                     << mMaterial_qp[p](1, i, j, k) << " "
                     << mMaterial_qs[p](1, i, j, k) << endl;
              }
            }
