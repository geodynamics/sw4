...
MaterialSfile::MaterialSfile(EW *a_ew, const string a_file,
  ...
  if (a_ew != NULL) m_use_attenuation = a_ew->usingAttenuation();
  read_sfile();
}
    ...
    for (int k = mEW->m_kStart[g]; k <= mEW->m_kEnd[g]; ++k) {
      for (int j = mEW->m_jStartInt[g]; j <= mEW->m_jEndInt[g]; ++j) {
        for (int i = mEW->m_iStartInt[g]; i <= mEW->m_iEndInt[g]; ++i) {
          ...
          // interface
          if (g == mEW->mNumberOfGrids - 1 && k > 0 && z < z_min) z = z_min;
          ...
          if (inside(x, y, z)) {
            ...
            while (gr >= 0) {
              ...
              // The top interface of a sfile patch is 2x larger than lower
              // level except topest interface
              if (gr == 0) {
    ...
    if (mEW->topographyExists() && g == mEW->mNumberOfGrids - 1) {
      ...
      for (int j = mEW->m_jStart[g]; j <= mEW->m_jEnd[g]; j++) {
        for (int i = mEW->m_iStart[g]; i <= mEW->m_iEnd[g]; i++) {
          if (zmin > mEW->mZ(i, j, kb)) zmin = mEW->mZ(i, j, kb);
          if (zmax < mEW->mZ(i, j, ke)) zmax = mEW->mZ(i, j, ke);
        }
      }
    } else {
