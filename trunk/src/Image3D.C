...
{
// note that the require2 macro doesn't generate any code unless compiled with debugging options
   REQUIRE2( mode == UX || mode == UY || mode == UZ ||
...
//-----------------------------------------------------------------------
bool Image3D::timeToWrite( double time, int cycle, double dt )
{
   // -----------------------------------------------
...
// Introduce 'st' to simplify the variable name
   ...
   for( int g=0 ; g < mEW->mNumberOfGrids ; g++ )
   {
	 ...
	 else
	 {
      ...
      {
         double* rho = a_Rho[g].c_ptr();
         double* mu  = a_Mu[g].c_ptr();
         double* la  = a_Lambda[g].c_ptr();
	 if( m_double )
	 {
	    for( int k=mWindow[g][4] ; k <= mWindow[g][5] ; k+=st )
	       for( int j=mWindow[g][2] ; j <= mWindow[g][3] ; j+=st )
...
void Image3D::compute_file_suffix( int cycle, std::stringstream& fileSuffix )
{
   ...
   fileSuffix << cycle << ".3D";
   fileSuffix << "." << m_modestring << ".sw4img";
}

//-----------------------------------------------------------------------
void Image3D::write_image( int cycle, std::string &path, double t,
			   Sarray& a_Z )
{
  //File format: 
  //
  ...
  // offset initialized to header size:
   ...
   if( m_parallel_io[0]->proc_zero() )
   {
      ...
      for(int g = 0; g < ng ;g++ )
      {
         double h = mEW->mGridSize[g]*mImageSamplingFactor;
   ...
   if( gridinfo == 1 )
   {
	 ...
	 m_parallel_io[g]->writer_barrier();
      ...
      {
	char cprec[]="double";
	m_parallel_io[g]->write_array( &fid, 1, zp, offset, cprec );
	offset += npts*sizeof(double);
      }
      else
      {
         int nptsloc = a_Z.npts();
...
void EW::read_volimage( std::string &path, std::string &fname, vector<Sarray>& data )
{
  ...
  // offset initialized to header size:
   ...
   bool iread = false;
   for( int g=0 ; g < ng ; g++ )
      iread = iread || parallel_io[g]->i_write();
   ...
   if( parallel_io[0]->proc_zero() )
   {
      ...
      CHECK_INPUT( ng == ngfile , "EW::read_image: Error number of grids on file " <<
      ...
      CHECK_INPUT( ret == sizeof(double),"EW::read_image: Error reading dummy-coord" );
      ...
      CHECK_INPUT( ret == 25*sizeof(char),"EW::read_image could not read strtimec");
	 ...
	 CHECK_INPUT( ret == sizeof(double),"EW::read_image: Error reading h" );
         CHECK_INPUT( abs(h-mGridSize[g])< 1e-12, "EW::read_image: Error h on file does not"
		      << " match h in computation" << " hfile = " << h << " hcomp= " << mGridSize[g]);
         double zmin;
         ret = read( fid, &zmin, sizeof(double) );
	 CHECK_INPUT( ret == sizeof(double),"EW::read_image: Error reading zmin" );
         CHECK_INPUT( abs(zmin-m_zmin[g])< 1e-12, "EW::read_image: Error zmin on file does not"
		      << " match zmin in computation" << " zmin file = " << zmin <<
		      " zmin comp= " << m_zmin[g] );
	 int globalSize[6];
	 ret = read( fid, globalSize, 6*sizeof(int) );
	 CHECK_INPUT( ret == 6*sizeof(int),"EW::read_image: Error reading global sizes" );
	 CHECK_INPUT( globalSize[0] == 1, "EW::read_image: Error in global sizes, " <<
		      "low i-index is "<< globalSize[0]);
	 CHECK_INPUT( globalSize[1] == m_global_nx[g], "EW::read_image: Error in global sizes, "
		      << "upper i-index is " << globalSize[1]);
	 CHECK_INPUT( globalSize[2] == 1, "EW::read_image: Error in global sizes, " <<
		      "low j-index is "<< globalSize[2]);
	 CHECK_INPUT( globalSize[3] == m_global_ny[g], "EW::read_image: Error in global sizes, "
		      << "upper j-index is " << globalSize[3]);
	 CHECK_INPUT( globalSize[4] == 1, "EW::read_image: Error in global sizes, " <<
		      "low k-index is "<< globalSize[4]);
	 CHECK_INPUT( globalSize[5] == m_global_nz[g], "EW::read_image: Error in global sizes, "
   ...
   for( int g = 0 ; g < ng ; g++ )
   {
      size_t npts = (size_t)(m_global_nx[g])*(size_t)(m_global_ny[g])*(size_t)(m_global_nz[g]);
      size_t nptsloc = (size_t)((m_iEndInt[g]-m_iStartInt[g]+1))*(m_jEndInt[g]-m_jStartInt[g]+1)*
	 ...
	 offset += npts*sizeof(float);
      ...
      data[g].insert_subarray( m_iStartInt[g], m_iEndInt[g], m_jStartInt[g], m_jEndInt[g],
				 m_kStartInt[g], m_kEndInt[g], doubleField );
      delete[] doubleField;
   }
   if( iread )
      close(fid);
