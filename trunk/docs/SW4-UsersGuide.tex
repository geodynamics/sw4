...
\section{The visco-elastic layer over half-space problem: LOH.3 (UPDATE)}
The LOH.3 problem, which adds effects of anelastic attenuation to the LOH.1 problem, is
defined in the input script \verb+examples/scec/loh3/loh3-h50.mr.in+. To reduce any artificial
effects from the far-field boundaries, we use a slightly larger computational domain compared to
LOH.1,
\begin{verbatim}
grid h=50 x=40000 y=40000 z=20000
\end{verbatim}
The visco-elastic modeling is enabled with the command
\begin{verbatim}
...
time-function. Note, however, that for LOH.3, the Gaussian has spread $\sigma=0.05$, corresponding
to the center angular frequency $\omega_0=1/\sigma=20$ and center frequency
$f_0=20/(2\pi)\approx 3.18$ Hz. The source is specified by the command
\begin{verbatim}
source x=20000 y=20000 z=2000 Mxy=1 m0=1e18 t0=0.2 freq=20 \
       type=Gaussian
\end{verbatim}

The solution is recorded in the same array of recievers as before, but all $(x,y)$ coordinates have
been incremented by 5000 meters to be in the same position relative to the source.  Results recorded
at station 10 are shown in Figure~\ref{fig:LOH3}. As for the LOH.1 problem, the velocity is
presented in polar components. We conclude that the numerical solution is in very good agreement
with the semi-analytical solution shown in black. 
\begin{figure}[ht]
  \begin{center}
%    \includegraphics[width=0.90\textwidth]{loh3-m3-mr50.eps} UPDATE!
    \caption{LOH.3: The radial (top), transverse (middle) and vertical (bottom) velocities at
      receiver number 10. Here the numerical and semi-analytical solutions are plotted in red and
      black, respectively.}
    \label{fig:LOH3}
  \end{center}
\end{figure}

As before we can estimate the resolution in terms of the number of grid points per shortest
...
2000$ m/s where the grid size is $h=25$ m, and we arrive at
\[
P = \dfrac{2000}{25\cdot 7.95} \approx 10.06.
\]
