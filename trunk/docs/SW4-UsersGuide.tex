...
\verb+freq=16.6667+ corresponds to the spread $\sigma=0.06$ ({\tt freq} = $1/\sigma$) in the
Gaussian time function. The lines to setup the source and the time duration of the simulation are:
\begin{verbatim}
time t=9
...
\begin{verbatim}
fileio verbose=1 path=LOH1-h50
\end{verbatim}
The \verb+verbose=1+ keyword makes \emph{SW4} output some extra diagnostic information during the
run.
...
identical with the semi-analytical solution when the grid is refined to $h=25$.
\begin{figure}[ht]
  \begin{center}
...
\end{verbatim}
As for the LOH.1 problem, the semi-analytical solution is given in polar components. The soluion
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=1.0\textwidth]{loh3-m3.png}
...
$$
This 
option gives the user an easy way to modify the quality factors, even if they are given on material file formats that are hard
to access manually. Use {\tt qmultiplier} with caution. If the quality factors are small (i.e., {\tt qmultiplier}$<<1$), 
the equations will become ill-posed. \emph{SW4} will detect too small quality factors and terminate before the time stepping starts.
...
\rm None
\end{flushleft}
The {\bf randomize} command makes a random perturbation of the material velocities $V_p$ and $V_s$, according to
$$
 V_p := (1+A(z)*\theta)V_p\qquad V_s := (1+A(z)*\theta)V_s\qquad
$$
where $\theta$ is a random field with $-1 <\theta<1$ and with correlation lengths specified by the {\tt lengthscale}
and {\tt lengthscalez} options. $V_p$ and $V_s$ use the same random field, hence the ratio $V_p/V_s$ is unaffected
...
\begin{center}
\begin{tabular}{|l|p{8cm}|l|l||l|} \hline
\multicolumn{5}{|c|}{\bf globalmaterial command parameters}\\ \hline
\bf{Option}  & \bf{Description} & \bf{Type} & \bf{Units} & \bf{Default} \\ \hline \hline
...
\end{tabular}
\end{center}
\index{randomize parameters!lengthscale, lengthscalez, seed1, seed2, seed3}
...
volumetric data. A reader for volimage files is provided in the matlab/octave function 'tools/readimage3d.m'.
Alternatively, users might want to visualize the data in these files with the open source {\em VisIt}
post processor. \verb+volimage+ files can also be used to define the material properties in
\emph{SW4}, using the \verb+vimaterial+ command. \emph{SW4} volimage files have extension \verb+.3D.mode.sw4img+,
where \verb+mode+ is one of \verb+ux, uy, uz, rho, lambda, mu, p, s, qp, qs+.
\par
The format of the volimage files is the same as the format of image files, with the exception that
the sizes of the data patches are specified by six integers in the volimage files, instead of four 
in the image file format.
The first part of the header of the volimage format is identical to the first part of the image file header
given in the previous section. The plane and coordinate ($\xi$) variables are not needed for three dimensional 
data. They are both set to -1 in a \verb+volimage+ file. 
The number of possible variables to save is more restricted than for the image command. The volume image format
stores one of the following variables with corresponding integer code for the \verb+mode+ variable in the file header:
\begin{center}
...
\}}
\end{flushleft}
If {\tt grdinfo}=0, there is no additional information on the volimage file.
If {\tt grdinfo}=1, the $z$-coordinates of the curvilinear grid are also saved on the volimage file
...
The execution times shown in Table \ref{tab:cputime-usage} were obtained by running \emph{SW4} on problems
with approximately $2.8\times 10^5$ to $3.3\times 10^5$ grid points per processor. Timings were measured 
on 16 processors of Cab, 
a parallel supercomputer at LLNL, in August of 2013. The \emph{SW4} executable was compiled with Intel compilers at
optimization level -O. The execution times are given in seconds per grid point and time step. 
While the absolute numbers in Table \ref{tab:cputime-usage} are machine
dependent and destined to change in the future, the relative cost of various solver
configurations is likely to stay more or less constant. Note that the visco-elastic cases used 3
mechanisms. For the case with topography, about 31\% of the grid points were in the curvilinear
grid.

We note that solving the visco-elastic wave equations with $m=3$, requires around 3 times more CPU
time than the purely elastic case. Using topography requires about 1.5 times more CPU time when 31\% of
the domain is discretized on a curvilinear grid. By extrapolation, if all grid points were in the 
curvilinear grid, the increase in CPU time would have been about a factor 2.7. 
The strong scaling properties are not perfect. Hence, the performance might be degraded when the number 
of processors is increased while keeping the number of grid points fixed. However, the total computational time
always decreases with increasing number of processors. More experimentation is needed 
to better evaluate the strong scaling properties of \emph{SW4}.
\begin{table}
\begin{center}
