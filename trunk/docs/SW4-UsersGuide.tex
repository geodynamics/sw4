...
utcstart & month/day/year:hour:minute:second.millisecond & string & datum & start of execution\\ \hline
\end{tabular}
\end{center}
...
\end{tabular}
\end{center}
\index{volimage options!file, mode, precision, cycle, cycleInterval, time, timeInterval, startTime}
%
...
performaed after each time step.
\begin{center}
\begin{tabular}{|l|p{10cm}|l|l|} \hline
...
volumetric data. A reader for volimage files is provided in the matlab/octave function
'tools/readimage3d.m'.  Alternatively, users might want to visualize the data in these files with
the open source {\em VisIt} post processor. \verb+volimage+ files can also be used to define the
material properties in \emph{SW4}, using the \verb+vimaterial+ command. \emph{SW4} volimage files
have extension \verb+.3D.mode.sw4img+, where \verb+mode+ is one of
\verb+ux, uy, uz, rho, lambda, mu, p, s, qp, qs+.
...
in the image file format.  The first part of the header of the volimage format is identical to the
first part of the image file header given in the previous section. The plane and coordinate ($\xi$)
variables are not needed for three dimensional data. They are both set to -1 in a \verb+volimage+
file.  The number of possible variables to save is more restricted than for the image command. The
volume image format stores one of the following variables with corresponding integer code for the
\verb+mode+ variable in the file header:
\begin{center}
