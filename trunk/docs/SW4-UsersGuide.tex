...
{\tt tools} directory and described in
Chapter~\ref{chap:scripts}.
...
information. You can setup the computational grid to follow this topography by using the commands
\begin{verbatim}
topography input=efile zmax=6e3 order=2
efile query=MAXRES \
      etree=/Users/petersson1/src/sw4/tests/USGSBayAreaVM-08.3.0.etree 
\end{verbatim}
Here, the topography command tells \emph{SW4} to read the topography from the Etree specified by the
\verb+efile+ command. Hence, the topography command must be accompanied by an efile command. The
\verb+order=2+ option specifies the type of stretching to use when making the curvilinear grid. A higher
value makes the curvilinear grid smoother near the bottom, but can cause a larger variation in grid
size near the top. The \verb+zmax=6e3+ option tells \emph{SW4} to extend the curvilinear grid down
to $z=6000$.
...
that this behavior is different \emph{WPP}, which required the user to also define the material in
the ghost points.
...
constructed before the material properties are assigned. In the Cartesian grid, the
properties at the free surface are thus mapped to the top grid surface ($z=0$), and the bottom grid
surface (with $z=z_N$) is assigned material properties for elevation $-z_N$. Elevation values at
intermediate grid points follow from the linear mapping. 
...
simulation. In addition, we always recommend inspecting the material model along a few \verb+image+ planes.
...
\begin{centering}
  ...
  \caption{The {\tt smoothingsize} parameter can be used to average out imprinting from the
    horizontal lattice in a coarse pfile material model. Here we show $V_P$ in the plane $y=5,000$
    as function of $x$ and $-z$. In this case, {\tt smoothingsize}$=1$ in the top left, {\tt
    smoothingsize}$=3$ in the top right, and {\tt smoothingsize}$=5$ in the bottom plot.}
...
An example that uses an ifile material description is discussed in Section~\ref{sec:grenoble}. The
\verb+ifile+ file format is described in Section~\ref{sec:ifile-format}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The vimaterial command}\label{sec:vimat}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The \verb+vimaterial+ command is intended to speed up the reading of the material model for cases
when many simulations use the same computational grid. In order for this command to work, the \verb+grid+
and \verb+topography+ commands must be identical between runs. An initial run must set up the
material model using the commands described in the previous sections (block, efile, pfile,
ifile). The initial run must also save the material model using the \verb+volimage+ command. Each of
these commands saves one scalar property per file, so three files must be saved for an elastic
material. In subsequent simulations the \verb+vimaterial+ command can replace the initial material
model. This approach works the best on machines with a fast (parallel) file system. Also note that
the files can be very large, because the material properties are saved at each grid point.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The fileio command can be used to specify the directory where \emph{SW4} writes its output files.  If
the directory does not exist, \emph{SW4} attempts to create it for you.  The fileio command may also
be used to set the level of diagnostic messages (verbose) and how often the time step information is
printed. For example,
\begin{verbatim}
	fileio path=sw4_dir verbose=1 printcycle=10
\end{verbatim}
causes all output files to be written to the directory "./sw4\_dir", turns on some extra diagnostic
messages (a higher value gives more details), and prints the time step information every 10 time steps.
...
simultaneously write to the same file. These file systems are often mounted on a special
directory. By default, \emph{SW4} assumes a serial file system and will only allow one processor write
to the same file at the same time. If you have access to a parallel file system, the I/O performance
of \emph{SW4} can sometimes be improved by allowing several processors to simultaneously
write a file. You enable this feature by using the {\tt pfs=1} option,
\begin{verbatim}
	fileio pfs=1 path=/p/lscratcha/my_output_directory
\end{verbatim}
Note that many parallel file systems are only accessible from dedicated directories. Enabling {\tt
pfs=1} without re-directing the output to a parallel file system can cause \emph{SW4} to either
crash or hang.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Time-history at a reciever station: the sac command}\label{sec:sac}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\emph{SW4} can save the time-history of the solution at a receiver station located anywhere in the computational
domain. The basic command looks like this:
\begin{verbatim}
	sac x=100e3 y=50e3 z=0 file=sta1
\end{verbatim}
Here, \emph{SW4} saves the three components of the solution at the grid point that is closest to the
specified $(x,y,z)$ location. By default, \emph{SW4} saves the data using the binary Seismic
Analysis Code (SAC)~\cite{Goldstein-et-al} format. In the above case, the {\tt sac} command results
in three files:
\begin{verbatim}
sta1.x  sta1.y  sta1.z
\end{verbatim}
The x,y,z files hold the solution time-history in the corresponding coordinate direction.
Note that a positive $z$-component corresponds to a downward motion.

The location of the reciever station can alternatively be given in geographical (latitude,
longitude, depth) coordinates. Information about the event date, time, and station name is saved in
the header of a SAC file. By default the date and time are set to the date and time at the start of
the simulation; the default station name is the name of the file. The default values of these fields
can be changed by using the {\tt eventData}, {\tt eventTime}, and {\tt sta} options,
%
\begin{verbatim}
...
\end{verbatim}
%%
Note that {\tt depth} specifies the depth of the reciever below the topography. To place a reciever
at elevation $e$ relative to mean sea level ($e$ is negative below sea level) you use the option {\tt z=}$-e$.
...
By default, \emph{SW4} outputs the three components of the solution
$\ub(\xb_r,t)=(u_x,u_y,u_z)^T$. By using the {\tt velocity=1} option, \emph{SW4} instead outputs the
three components of the time-derivative of the solution, i.e., the velocity if \emph{SW4} is setup
to solve for displacements. The {\tt nsew=1} option can also be used to tell \emph{SW4} to rotate
the solution components to the East, North, and vertical (positive up) directions,
\begin{verbatim}
sac lat=38.25 lon=-122.20 depth=0 file=sta1 velocity=1 nsew=1
\end{verbatim}
The angle between North and the $x$-axis is determined by the azimuth ({\tt az=...}) angle in the grid command:
\begin{verbatim}
grid x=100e3 y=50e3 z=30e3 lat=37.5 lon=-122.0 az=135
\end{verbatim}
To remind the user of what quantities are saved in a sac file, we modify the file name extensions according to the
following table:
\begin{center}
\begin{tabular}{|c|c|c|} \hline
   & \verb+velocity=0+  & \verb+velocity=1+ \\ \hline
\verb+nsew=0+ & .x, .y, .z & .xv, .yv, .zv \\ \hline
\verb+nsew=1+ & .e, .n, .u & .ev, .nv, .uv \\ \hline
\end{tabular}
\end{center}
\par
\emph{SW4} can output the time history of the divergence or the curl of the
solution at the reciever station, instead of the solution itself. The {\tt variables} option governs this 
behavior. For example,
\begin{verbatim}
sac lat=38.25 lon=-122.20 depth=0 file=sta1 variables=div
\end{verbatim}
will output a single file named {\tt sta1.div} containing the divergence of the solution. Similarly
\begin{verbatim}
sac lat=38.25 lon=-122.20 depth=0 file=sta1 variables=curl
\end{verbatim}
will output the three files {\tt sta1.curlx}, {\tt sta1.curly}, and {\tt sta1.curlz}. 
The {\tt velocity} and
{\tt nsew} options also work together with divergence and curl. Setting {\tt velocity=1} will output
the time derivative of any quantity selected by the {\tt variables} option. The {\tt nsew} option has no
effect on the scalar divergence field, but {\tt nsew=1} will make \emph{SW4} output a representation of
the curl vector in the East, North, and vertical components.
The file name extensions for the div and curl variables are described in the following table:
\par
\begin{center}
...
\end{center}
\par
\emph{SW4} can output the time history of the strain components of the
solution at the reciever station. The {\tt variables} option governs this 
behavior. For example,
\begin{verbatim}
sac lat=38.25 lon=-122.20 depth=0 file=sta1 variables=strains
\end{verbatim}
will output the six files {\tt sta1.xx}, {\tt sta1.yy}, {\tt sta1.zz}, {\tt sta1.xy}, {\tt sta1.xz}, and
{\tt sta1.yz} in sac format containing the six Cartesian components of the symmetric strain tensor. 
...
\emph{SW4} can also output reciever time-histories in an ASCII text format,
\begin{verbatim}
sac lat=38.25 lon=-122.20 depth=0 file=sta1 sacformat=0 usgsformat=1
\end{verbatim}
The ASCII text file holds all three components (or six when using {\tt variables=strains} ) in a single 
file named {\tt sta1.txt}. When the
\verb+usgsformat=1+ option is used, the file gets extension .txt independently of the \verb+nsew+
and \verb+velocity+ options. Instead the header inside the file is modified to reflect its contents.
Note that you must give the {\tt sacformat=0} option unless you want the solution to be output in
both formats.

\paragraph{Notes on the sac command:}
\begin{itemize}
\item SAC files are treated in the same way on parallel and serial file systems, because the data for each SAC
file originates from one processor and is always written by that processor only.
\item The binary SAC format is described in Section~\ref{sec:sac-format}.
\item The ASCII text format is outlined in the header of those files.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Generating a bird's eye view of the problem domain: the gmt command}\label{sec:gmt}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
is a change from \emph{WPP}. Also note that the \verb+attenuation+ command may be located anywhere
in the command file. The material commands \verb+block+, \verb+ifile+, \verb+efile+, and
...
\verb+globalmaterial+ command are enforced after all other material commands have been applied.
...
\item {\bf input=efile} Read the topography from the Etree database. The Etree database must be
  specified by an {\bf efile} command (see below). The spatial resolution for querying the Etree
  database can be specified by the {\bf resolution=...} parameter.
...
topo & $x$ & $y$ & $z$ & $u^{(x)} - u_{ex}^{(x)}$ & $u^{(y)} - u_{ex}^{(y)}$ & $u^{(z)} -
u_{ex}^{(z)}$  \\ \hline
\end{tabular}\\
\medskip
...
%% For a complete installation that supports the efile command for reading e-tree material databases,
...
%% database files, holding the material model of Northern California, can currently be downloaded from
%% \begin{verbatim}
