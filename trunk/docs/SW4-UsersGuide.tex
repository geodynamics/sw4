...
command. If you need to use these options, you can always recompile \emph{SW4} after the proj.4 and cencalvm libraries
have been installed. See \S~\ref{sec:cencalvm-install} for details.
...
read it if it is available. There are several examples in the \verb+configs+ directory, for example
\verb+make.osx+ for Macs an \verb+make.linux+ for Linux machines. You should copy one of these files
to your own \verb+make.inc+ and edit it as needed. For example, if you are on a Mac, you could do
To build \emph{SW4} under OSX on a Mac, you need to create the
\verb+configs/make.inc+ file could look something like this:
\begin{verbatim}
shell>cd configs
shell>ls make.inc
FC = openmpif77
...
shell> make
\end{verbatim}
If you for some reason wish to remove all object files and the \verb+sw4+ executable, you can do
\begin{verbatim}
shell> cd /enter/your/path/sw4-v1.0
shell> make clean
\end{verbatim}

By default, the \verb+sw4+ executable is located in 
\begin{verbatim}
...
You can also build a debug version of \emph{SW4} by adding the \verb+debug=yes+ option to \verb+make+,
\begin{verbatim}
shell> cd /enter/your/path/sw4-v1.0
