...
building the libraries as \emph{SW4}.
...
\begin{itemize}
\item The Euclid e-tree library
...
\end{verbatim}
As a result a new sub-directory named \verb+sw4-v1.0+ is created, containing several files
and sub-directories:
%
\begin{itemize}
...
supports all commands except \verb+efile+ and the \verb+proj+/\verb+ellps+ options in the \verb+grid+
...
\end{verbatim}
You can also build a debug executable by adding the \verb+debug=yes+ option to \verb+make+,
\begin{verbatim}
shell> cd /enter/your/path/sw4-v1.0
...
\end{verbatim}
It can be convenient to add the corresponding directory to your \verb+PATH+ environment variable (i.e., modify
...
\verb+cencalvm+ and \verb+proj.4+ libraries should be linked to \emph{SW4}. 
...
\end{verbatim}
The \verb+etree+ variable should be set to \verb+yes+ or \verb+no+, to indicate whether the cencalvm
and related libraries are available. The \verb+SW4ROOT+ variable is only used when
...
The cencalvm library was developed by Brad Aagaard at USGS. Both the cencalvm library and the Etree
...
should assign that directory to the SW4ROOT variable. When these three packages are installed, the
...
...
\end{verbatim}
You then need to re-compile \emph{SW4}. Go to the sw4 main directory, clean up the previous object
