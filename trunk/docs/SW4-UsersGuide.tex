...
\appendix
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Installing \emph{SW4} (in progress)}\label{cha:installing-sw4}
\index{installation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
communication on parallel distributed (and shared) memory machines. Note that the MPI library often
includes wrappers for compiling, linking, and running of MPI programs. For example, the {\tt mpich2}
package includes the {\tt mpicxx} and {\tt mpif77} compilers, as well as the {\tt mpirun} script. We
highly recommend using these programs for building and running \emph{SW4}.
...
\end{enumerate}
To avoid incompatibility issues and linking problems, it is advisable to use the same compiler for
building the libraries and \emph{SW4}.

For a complete installation that supports projections from the Proj.4 library and material models
from an e-tree database, you need to install three additional libraries:
\begin{itemize}
\item The Etree library
\item The Proj.4 library
...
the \verb+openmpi+ package. By adding the \verb+gcc47+ variant, you also get a compatible Fortran
compiler. Alternatively, you can use the \verb+mpich2+ package.

The \verb+lapack+ and \verb+blas+ libraries are preinstalled on recent Macs and can be accessed using the
``-framework vecLib'' link option. If that does not work on your machine, you can download \verb+lapack+
and \verb+blas+ from www.netlib.org.

\paragraph{Linux machines}
UPDATE! We here give detailed instructions for installing the third part libraries under 64 bit,
Fedora Core 14 Linux. Other Linux variants use similar commands for installing software packages.
...
know where, you can find them with the following commands:
\begin{verbatim}
cd /
...
\end{verbatim}
Afterwards, you will find a new directory named \verb+sw4-v1.0+ that contains several files
and subdirectories:
%
\begin{itemize}
...
\section{Compiling and linking \emph{SW4} (basic)}\label{sec:basic-install}
...
supports all commands except \verb+efile+ and the \verb+proj+ option in the \verb+grid+
command. If you need to use these options, you can always install the proj.4 and cencalvm libraries
later, and then recompile \emph{SW4}. See \S~\ref{sec:cencalvm-install} for details.
...
\verb+configs+ directory called \verb+make.inc+, but you may also assign them in your \verb+.cshrc+
file. The \verb+Makefile+ will look for this file and read it if it is available. To build
\emph{SW4} under OSX on a Mac, the \verb+configs/make.inc+ file could look something like this:
\begin{verbatim}
FC = openmpif77
CXX = openmpicxx
...
\end{verbatim}
In this case, the \verb+blas+ and \verb+lapack+ libraries are provided by the
\verb+-framework vecLib+ option.
Here you will find a number of configuration files. Examples are provided for some common operating
systems, such as \verb+make.osx+ for Macs, and \verb+make.linux+ for machines running Linux.
\begin{verbatim}
setenv SW4MAKE /enter/your/path/sw4-v1.0/configs/make.amac
\end{verbatim}
If you are using a Bourne, bash, or related shell, you should instead use the command
\begin{verbatim}
export SW4MAKE=/enter/your/path/sw4-v1.0/configs/make.amac
\end{verbatim}
...
\begin{verbatim}
/my/installation/dir/sw4-v1.0/src/sw4
\end{verbatim}
...
\begin{verbatim}
/my/installation/dir/sw4-v1.0/debug_v2.1/sw4
\end{verbatim}
