...
\begin{figure}[th]
\begin{centering}
 \includegraphics[width=0.6\linewidth]{coords.eps}
...
\section{What is the frequency content in my time function?}\label{sec:freq}
...
sense for time functions that tend to zero for large times. Functions such as {\tt GaussianInt},
...
\]
where $C_s$ is the shear velocity of the material and $f_{max}$ is the largest significant frequency
...
downwards on the surface of a homogeneous half-space. To evaluate the accuracy of the numerical
...
$(5000,5000,0)$. The time dependency of the forcing is a ``RickerInt'' (see Figure
\ref{fig:rickers}) with $\omega=1$ Hz, $t_0=2$ s and magnitude $10^{13}$ N. The above setup is
...
source type=RickerInt x=5e3 y=5e3 z=0 fz=1e13 freq=1 t0=2
rec x=5e3 y=6e3 z=0 file=v1 sacformat=0 usgsformat=1
\end{verbatim}

The vertical displacement and error at a reciever at ($x=5000$, $y=6000$, $z=0$) is shown in
Figure~\ref{fig:lambSAC}.
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=0.8\textwidth]{lamb-err-p6.eps}
    \caption{Lamb's problem: Vertical displacement at the reciever $x=5000$, $y=6000$, $z=0$, i.e.,
      1000 meters from the source. The blue
      line shows the exact solution and the green line shows the error in the numerical solutions
      with $h=10000/150$, corresponding to $P=6$ grid points per shortest wave length.}
    \label{fig:lambSAC}
  \end{center}
\end{figure}

The waveform is smooth and the problem appears to be well resolved. We present the max-norm of the
errors in the vertical displacement in Table~\ref{tab:lamb-err}. The center frequency in the RickerInt
time function is $f_0=1$ Hz. Following (\ref{eq:upper-power-freq}), we estimate the highest
significant frequency to be $f_{max}=2.5$ Hz. In this case the formula for the number of grid points
per wave length (\ref{eq:resolutionformula}) becomes 
\[
P=\frac{1000}{2.5 h} = \frac{nx-1}{25}.
\]
\begin{table}
\begin{center}
\begin{tabular}{| c | c | c | c | }
\hline
$N_x$ & $h$   & $P$ & $\| u_z - U_z\|_\infty$ \\ \hline
$101$ & 100   & 4   & $1.792\cdot 10^{-2}$    \\ \hline
$151$ & 66.67 & 6   & $6.523\cdot 10^{-3}$    \\ \hline
$201$ & 50    & 8   & $2.609\cdot 10^{-3}$    \\ \hline
$301$ & 33.33 & 12  & $6.188\cdot 10^{-4}$    \\ \hline
$401$ & 25    & 16  & $2.047\cdot 10^{-4}$    \\ \hline
\end{tabular}
\caption{Max norm errors in the vertical displacement at $x=5000$, $y=6000$, $z=0$.}
\label{tab:lamb-err}
\end{center}
\end{table}
Since the max norm (amplitude) of the exact solution is $0.1675$, the relative max-norm error for
$P=6$ is about 3.9 percent, when the receiver is 1000 meters from the source. Since the center
frequency is $f_0=1$ Hz and $C_s=1000$ m/s, the dominant wave length in the solution is about 1000
meters.

In Figure~\ref{fig:ppw-err} we plot the max error as function of the number of points per wave
length.  The ratio between errors for grid size $2h$ and $h$ approaches 16 for the finest grid,
indicating that the numerical method and the discretization of the point force are fourth order
accurate.
\begin{figure}[th]
\begin{centering}
 \includegraphics[width=0.8\linewidth]{ppw-err.eps}
  \caption{Error in max norm as function of the number of points per wave length. The black line
    indicates the asymptotic decay of the error in a fourth order accurate method.}
  \label{fig:ppw-err}
\end{centering}
\end{figure}

It is well known that the error in numerical solutions of wave propagation problems is dominated by
phase errors~\cite{Gustafsson-Kreiss-Oliger}. To investigate how the accuracy in the numerical
solution depends on the distance between the source and receiver, we move the receiver in the above
experiment to $y=15,000$, i.e., 10 times further away from the source.
