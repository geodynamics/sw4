...
and integrates to zero. The GaussianInt, Brune, BruneSmoothed, Haskell, HaskellSmoothed, and Liu
functions tend to one
...
Sawtooth, Ramp, Smoothwave, Brune, BruneSmoothed, Haskell, HaskellSmoothed, Liu and VerySmoothBump functions are identically
...
\subsection{Haskell}
\label{haskell}
The Haskell function is a generalization of the Brune function with an additional dimensionless
parameter $b$ that can be used to define the amount of "overshoot" in a spherical source (Figure~\ref{fig:haskells}),
\[
 g(t,t_0,\omega,b) = \left\{
\begin{array}{ll}
0, & t < t_0, \\
1 - e^{-\omega(t-t_0)}\left[1+\omega(t-t_0)-b\left(\omega(t-t_0)\right)^2\right], & t \geq t_0.
\end{array}
\right.
\]
The parameter $b$ is specified with the {\tt b} keyword in the {\tt source} command and is required
when either the Haskell or HaskellSmoothed time function is selected (Note: setting $b=0$ makes Haskell
identical to Brune). The Haskell function has only one continuous derivative at $t=t_0$ and can
therefore generate high-frequency numerical noise; use HaskellSmoothed when a smoother startup is
desired.
\begin{figure}
\begin{centering}
  \includegraphics[width=0.4\linewidth]{figures/f-haskell.png}
  \includegraphics[width=0.4\linewidth]{figures/f-haskellsmoothed.png}
  \caption{Haskell (left) and HaskellSmoothed (right) with $\omega=2$ and $t_0=-1$ and $b=1$.}
  \label{fig:haskells}
\end{centering}
\end{figure}  

\subsection{HaskellSmoothed}
The HaskellSmoothed function modifies the Haskell function near $t=t_0$ to obtain the same smoother
startup used by BruneSmoothed. Let $x=\omega(t-t_0)$ and let $h=2.31$. Then
\[
 g(t,t_0,\omega,b) = \left\{
\begin{array}{ll}
0, & t < t_0, \\
1-e^{-x}\left[1+x+\dfrac{1}{2}x^2
 -\dfrac{3q}{h}x^3+\dfrac{3q}{h^2}x^4-\dfrac{q}{h^3}x^5\right],
 & 0\leq x<h, \\
1-e^{-x}\left[1+x-bx^2\right], & x\geq h,
\end{array}
\right.
\qquad q=b+\dfrac{1}{2}.
\]
The fixed value $h=2.31$ is dimensionless. The two branches join smoothly at $x=h$, while the
polynomial startup reduces the high-frequency content generated at $t=t_0$.
...
f_0 = \left\{ \begin{array}{ll}
 ...
 \dfrac{\tt freq}{2\pi},& \mbox{for Liu, Brune, BruneSmoothed, Haskell, HaskellSmoothed, Gaussian, GaussianInt, and GaussianWindow}.
\end{array}\right.
\end{equation}
...
{\tt BruneSmoothed}, {\tt Haskell}, and {\tt HaskellSmoothed} tend towards unity for large times. Their zero frequency (DC) component
...
Note that all pre-defined time functions use the {\bf t0} keyword, and all functions except {\bf
  Dirac} also use the {\bf freq} keyword. The {\bf b} keyword is required for the Haskell and
HaskellSmoothed time functions. The only pre-defined time function that uses the {\bf ncyc} keyword
is {\bf GaussianWindow}.
%
\begin{center}
\begin{tabular}{|l|p{8cm}|l|l|l|} \hline
...
b & coefficient used by Haskell and HaskellSmoothed & real & none & required \\ \hline
...
\index{source parameters!t0, freq, b, type, ncyc, dfile}
The {\bf type} keyword specifies the source time function. It can have
the following values: {\tt GaussianInt}, {\tt Erf}, {\tt Gaussian}, {\tt RickerInt}, {\tt Ricker},
{\tt Ramp}, {\tt Triangle}, {\tt Sawtooth}, {\tt Smoothwave}, {\tt VerySmoothBump}, {\tt Brune},
{\tt BruneSmoothed}, {\tt Haskell}, {\tt HaskellSmoothed}, {\tt GaussianWindow}, {\tt Liu},
{\tt Dirac}, and {\tt C6SmoothBump}. The functions are described in \S~\ref{sec:predefined}.
\index{source time function!GaussianInt, Gaussian, RickerInt, Ricker, Ramp, Triangle, Sawtooth, Smoothwave,
  VerySmoothBump, Brune, BruneSmoothed, Haskell, HaskellSmoothed, GaussianWindow, Liu, Dirac,
  C6SmoothBump}
\begin{center}
\begin{tabular}{|l|p{8cm}|l|l|l|} \hline
