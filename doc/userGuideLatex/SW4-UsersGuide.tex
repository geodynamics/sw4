...
Be aware that these files can be {\em very} large, depending on the
  x,y,z ranges and cycleInterval requested (which could include
  the whole domain AND every time step).
...
The option {\bf cycleInterval=...} is different than with other
  commands; for \verb+essioutput+ the output file will written out
  after that many time steps, and include the both first and last timestep
  data values.
If omitted, the output occurs at every time step of the simulation.
When reading these multiple files, care must be taken to avoid
  duplicate data from the same time step in different files (and simply
  appending the time series values will lead to duplicate data).
Note that if the \verb+checkpoint+ command also uses the
  {\bf cycleInterval=...} option, that value will be used by
  \verb+essioutput+ to guarantee checkpoint and ESSI files are
