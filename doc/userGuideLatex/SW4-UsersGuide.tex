...
\begin{flushleft}
\bf Syntax:\\ \tt
checkpoint cycleInterval=... file=... \\
checkpoint cycleInterval=... file=... restartfile=... restartpath=\\
\bf Required parameter:\\
\rm Time for output: (time, timeInterval, cycle, or cycleInterval).\\
\bf Notes:\\
\rm \verb+fileio+ \verb+path+ argument sets the path for the checkpoint
  file, which can be overridden on restart with \verb+restartpath+.
\end{flushleft}
%
...
\begin{verbatim}
checkpoint cycleInterval=50 file=LOH1_restart
\end{verbatim}

The simulation can be restarted with the input:
\begin{verbatim}
...
Writes/deletes so only last 2 checkpoint files are kept

Affects behavior with \verb+rec+, appends USGS, but not BINARY

File name issues (cycle numbers)?? \verb+file+ argument is only the
  prefix, full checkpoint name is [path]/[prefix].cycle=\%d.sw4checkpoint

\begin{center}
\begin{tabular}{|l|p{8cm}|l|l|} \hline
\multicolumn{4}{|c|}{\bf restart parameters}\\ \hline
\bf{Option} & \bf{Description} & \bf{Type} & \bf{Default} \\ \hline \hline
file & file header name to be prepended on restart files & string & ???  \\ \hline
cycleInterval & sets the interval for writing out restart files & int & ???  \\ \hline
restartfile & restart the code from this file & string & ???  \\ \hline
restartpath & path to restart file & string & ???  \\ \hline
\end{tabular}
\end{center}
