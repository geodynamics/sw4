...
\rm \verb+fileio+ \verb+path+ argument sets the path for the checkpoint
  file, but restart files are read from \verb+restartpath+.
\end{flushleft}
%
...
\end{verbatim}
The command above will create a checkpoint file in the same output
  path as in the \verb+fileio+ \verb+path+ argument:
\begin{verbatim}
LOH1_restart.cycle=050.sw4checkpoint
\end{verbatim}
...
The \verb+rec+ USGS and SAC files are read in, and continued from the
  checkpoint cycle, and written to the output 
  directory \verb+fileio+ \verb+path+.
{\bf Note:} Because SAC files are float (4 byte) instead of double,
